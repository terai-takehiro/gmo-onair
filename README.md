...
**現在のバージョン**: v2.8.34 (dev) — **モバイル スクロール不能 Fix + レスポンシブ強化**: スマホで編集画面が縦スクロールできない問題を修正 (`overflow-hidden` → `overflow-y-auto lg:overflow-hidden`)。CueCardList のセクション ↑↓削除ボタンと行サイドツールバーを iOS HIG タップ最小 44px に拡大 (size-9, w-11, min-h-9)。 // (v2.8.33: InsertGap UI 安定化) v2.8.32 の挿入バーが hover 拡張で layout shift + focus 残留によりレイアウトが崩れる問題を修正。固定高 28px + opacity 切替に変更、デフォルトは小さな + アイコン、hover で 3 種ボタンへフェードイン。モバイルも同視覚言語に統一。挿入機能は同等。 // (v2.8.32: CM/VTR 任意位置挿入 + ロール/行 DnD) 各セクション間に `InsertGap` (ホバー拡張、3 種ボタン: ロール/CM/VTR)。ロール (section) を HTML5 native DnD でリオーダー。ロール内の行 (row) を ↑↓ ボタン + native DnD でリオーダー。モバイル (CueCardList) はタッチ親和性のため ↑↓ ボタンに加え同 InsertGap 配置。CM/VTR セクションは尺 input + 識別色で表示。データ構造変更なし。 // (v2.8.31: マスター + LED 統合 + 壁/床モダン UI): EditorSidebar の `LED` タブを廃止し 3 タブ構成に再編、LED/XR シーン・立ち位置図・マスタデータを `ResourceCard` 統一カードで並列表示。`MasterSection` pill 改善 + `LedSceneSection` 壁/床を semantic 反対色（壁=primary 床=warning）の 2 カラムミニカード化。CueRow / CueRowMobileEditor の壁/床インライン表示も同形式に統一。機能変更なし。
