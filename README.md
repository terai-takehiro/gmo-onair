...
**現在のバージョン**: v2.8.37 (dev) — **モバイル InsertGap タップ展開式に変更**: 常時 3 ボタン表示で散らかって見える問題を修正。デフォルトは小さな + ボタン 1 個 + 点線、タップで pill 形式の `ロール · CM · VTR · ×` を展開。挿入機能は同等。 // (v2.8.36: 機材ダッシュボードにケーブル/コネクタを追加) // (v2.8.35: 機材管理にケーブル管理 + コネクタ管理ページを追加) // (v2.8.34: モバイル スクロール不能 Fix + レスポンシブ強化) // (v2.8.33: InsertGap UI 安定化) // (v2.8.32: CM/VTR 任意位置挿入 + ロール/行 DnD)
