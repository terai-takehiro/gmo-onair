...
**現在のバージョン**: v2.8.36 (dev) — **機材ダッシュボードにケーブル/コネクタを追加**: 機材ダッシュボードのクイックアクションに「ケーブル管理」「コネクタ管理」ボタンを追加 (Cable/Plug アイコン、グリッドを `lg:grid-cols-4 xl:grid-cols-7` に拡張)。新セクション「消耗品サマリー」で**所有合計本数/個数 + 種類数**を `KpiCard` 2 カラム表示。 // (v2.8.35) **機材管理: ケーブル管理 + コネクタ管理ページ追加**: `equipment_cables` / `equipment_connectors` テーブルを新設、サーバー CRUD + クライアントページ + サイドバーに追加。 // (v2.8.34) **モバイル スクロール不能 Fix**: 編集画面の縦スクロール修正 + CueCardList ボタンを iOS HIG タップ最小 44px に拡大。 // (v2.8.33) **InsertGap UI 安定化 (Fix)**: 固定高 28px + opacity 切替で layout shift 排除。 // (v2.8.32) **CM/VTR 任意位置挿入 + ロール/行 DnD**.
...
| バージョン | 内容 |
|---|---|
| **v2.8.36** | **機材ダッシュボードにケーブル/コネクタを追加 (dev)**: 機材ダッシュボード (`DashboardPage.tsx`) のクイックアクションに「ケーブル管理」「コネクタ管理」ボタンを追加 (Cable/Plug アイコン、`text-primary`)。クイックアクションのグリッドを `lg:grid-cols-5` から `lg:grid-cols-4 xl:grid-cols-7` に拡張し、7 ボタン (貸出登録/メンテ記録/機材一覧/ケーブル管理/コネクタ管理/QRスキャン/ラック実装) を画面幅に応じて適切に配置。さらに新セクション「消耗品サマリー」(`grid-cols-1 sm:grid-cols-2`) を追加し、`/equipment/cables` `/equipment/connectors` を React Query で staleTime 60s でフェッチ → quantity を集計し**ケーブル所有本数 + 種類数**、**コネクタ所有個数 + 種類数**を `KpiCard` (`emphasis="info"`) で表示。タップで該当ページへ遷移。データ構造・既存 KPI/アラート/履歴セクション影響なし。 |
