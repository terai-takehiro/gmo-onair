...
**現在のバージョン**: v2.8.35 (dev) — **機材管理: ケーブル管理 + コネクタ管理ページ追加**: `equipment_cables` / `equipment_connectors` テーブルを新設 (機材一覧とは別 DB、共通 `equipment_locations` / `equipment_manufacturers` を引用)。種別 (映像/音声/NW/照明/電源/その他)、設置場所、商品名、メーカー、型名、ケーブルはメートル数・色・本数、コネクタは個数、収納方法、備考を管理。サーバーに CRUD ルート + クライアントに機材一覧と同等の UI/UX (種別タブ + 場所/メーカーフィルター + 検索 + テーブル/モバイルカード両ビュー + 登録/編集ダイアログ)。サイドバーに「ケーブル管理」「コネクタ管理」を追加。 // (v2.8.34) **モバイル スクロール不能 Fix + レスポンシブ強化**: スマホで編集画面が縦スクロールできない問題を修正、CueCardList の各種ボタンを iOS HIG タップ最小 44px に拡大。 // (v2.8.33) **InsertGap UI 安定化 (Fix)**: 固定高 28px + opacity 切替で layout shift を完全排除。 // (v2.8.32) **CM/VTR 任意位置挿入 + ロール/行 DnD**: 各セクション間に `InsertGap`、ロール/行 DnD、モバイル ↑↓ ボタン。データ構造変更なし。
