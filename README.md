...
**現在のバージョン**: v2.8.6 — Q シート Rundown「次の VTR」専用列
...
| バージョン | 内容 |
|---|---|
| **v2.8.6** | **Q シート Rundown view「次の VTR」専用列 (Q シート改修第 4 段)**。台本行のすぐ隣で「この行で何の VTR を流すか」が一目で分かるよう、Rundownstudio 風の専用列を追加。①**Video ブロック分離**: `RundownPage.tsx` で `videoBlock = blocks.find(b => b.type === 'video')` で抽出し、`blocksForRows = activeBlocks.filter(b => b.id !== videoBlock.id)` で重複表示を防ぐ。②**ヘッダ列追加**: 「実尺」列の直後に「▶ 次の VTR」列 (`w-40`)。テーマに応じて `bg-blue-50 text-blue-900` (light) / `bg-blue-950/40 text-blue-200` (dark) で目立たせる。③**行内描画**: 各 cue 行で `extractCellText(cue.row, videoBlock)` の結果を表示。VTR がある行は `bg-blue-50` + `border-l-2 border-l-blue-500` + `font-semibold` + 先頭に `▶` を付与、無い行は薄い `—` で空隙を埋める。Title 属性で全文ホバー表示。④**colSpan 整合**: section header の `colSpan={4 + activeBlocks.length}` を `colSpan={4 + (videoBlock ? 1 : 0) + blocksForRows.length}` に修正。Video ブロックが無いドキュメントは従来通りの動作 (videoBlock=undefined で列追加スキップ)。⑤**列選択トグル無干渉**: 既存の `visibleColumns` (列の表示/非表示) ロジックは Video 列だけは常に表示。台本行の最重要情報なので、ユーザーが Video 列を非表示にできない設計 |
| **v2.8.5** | **Q シート LED/XR シーンマスタ (Q シート改修第 3 段)**。v2.8.4 でセル UI に「シーン select」のプレースホルダだけ用意していたものを実装。①**サイドバーに新セクション**: `EditorSidebar.tsx` のマスタデータブロック直下に `LedXrSceneMasterSection` を新設。S0/S1/SXX 単位で 1 シーン = (`name`, `wallDefault`, `floorDefault`, `effectDefault`) のカード型エントリ。シーン名は `font-mono font-bold` で目立たせ、壁/床/効果デフォルトをカード内 3 列で入力。`Plus` ボタン or Enter で追加、ホバー時のみ × 削除ボタン (確認ダイアログ付き)。②**`Masters` 型拡張**: `EditorPage.tsx` と `EditorSidebar.tsx` の両方で `ledXrScenes?: LedXrScene[]` を追加。`LedXrScene = { id: string (不変、scn_<timestamp>), name: string, wallDefault?, floorDefault?, effectDefault? }`。③**マスタヘルパーの型整理**: 既存 `addMasterItem` / `removeMasterItem` が `keyof Masters` 全体を受け取っていたが `string[]` 列 (persons/video/audio/telop) 専用の `StringMasterKey` 型に絞り、`ledXrScenes` は別経路で更新する設計に。④**セル側でシーン参照**: `CueRow.tsx` の `LedXrCell` で `masters?.ledXrScenes` を select に展開、選択中シーンの `wallDefault` / `floorDefault` / `effectDefault` を override 入力欄の placeholder に表示 (空欄なら継承される設計)。⑤**保存経路はサーバ無修正**: `qsheet_documents.data.masters.ledXrScenes` として JSONB 内に保存され、既存の `PUT /qsheet/documents/:id` でそのまま永続化。新規マイグレーション・新規 API なし、既存 `masters.persons/video/audio/telop` パターンと一貫 |
| **v2.8.4** | **Q シート LED/XR セルを構造化 + 後方互換 (Q シート改修第 2 段)**。①現状の LED/XR セルは `{ value: string }` の自由 textarea のみで、収録現場の指示 (シーン番号・壁/床表示・トリガー・トランジション) を非構造で書いていた。これを以下の構造に置換: `{ value?: string (legacy), sceneId?: string, wallOverride?: string, floorOverride?: string, effectOverride?: string, trigger?: 'v_end'\|'qword_end'\|'cue_d'\|'as_needed', transition?: 'ci'\|'fi', memo?: string }`。②**新コンポーネント `LedXrCell`** (`client-qsheet/src/components/editor/CueRow.tsx`) を新設。3 行構造: 1 行目 = シーン select (v2.8.5 でマスタ実装、v2.8.4 ではプレースホルダ表示) + トリガー 4 択 pill (V明け / Qワード明け / 卓D Cue / 必要に応じて) + トランジション 2 択 pill (C.I. / F.I.)、2 行目 = 壁/床/効果 override (`<input>` 3 列、空ならシーンのデフォルトを placeholder で表示)、3 行目 = memo。③**後方互換**: `cell.value` のみ存在する旧データを開いたとき、新 UI の memo 欄に旧 value をフォールバック表示し「旧形式」薄バッジで移行を促す。ユーザーが memo を編集すると `value` を捨てて `memo` に移行 (データロスなし、マイグレーション不要)。④`<td>` 全体に `bg-violet-50/40 dark:bg-violet-950/20` で薄紫背景、Editor サイドバーの LED/XR ブロック色 (violet-600) と統一感。⑤シーン未選択時はシーンセレクタを `シーン未設定` の灰色バッジで隠してマスタ追加を促す UI |
| **v2.8.3** | **Q シートの画像サムネ拡大 + 動画セル背景強調 (Q シート改修第 1 段)**。①**サムネ拡大**: `EntryImageButton` (`client-qsheet/src/components/editor/CueRow.tsx` 行 78-82) の `<img>` を `h-6 w-6` (24x24px) → `h-12 w-12` (48x48px) に拡大、`rounded` → `rounded-md` でクリッパー違和感を緩和。× ボタン位置も `-top-1 -right-1` → `-top-1.5 -right-1.5` に微調整しサムネ角に被せる。Editor / Rundown / OnAir 全画面で同コンポーネント経由なので統一適用。②**動画セル背景**: Video / Audio / Telop の `<td>` (行 397) に `blk.type === 'video'` 条件で `bg-blue-50/60 dark:bg-blue-950/25` を加算。Audio / Telop は据え置き、Video のみ薄青背景に。既存の `_vtr` セクションヘッダ (indigo グラデ、CueTable.tsx 行 348) と区別しつつ、行内の Video ブロックも一目で分かるように。③Q シート改修要件 4 件 ((1) サムネ視認性 (2) V素材ハイライト (3) LED/XR 構造化 (4) Rundown Video 列) のうち軽量 UI 改善の 2 件を本バージョンでカバー、残り 2 件は v2.8.4 / v2.8.5 / v2.8.6 で順次投入予定 |
