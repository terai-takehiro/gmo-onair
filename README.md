...
**現在のバージョン**: v2.8.38 (dev) — **ケーブル/コネクタ管理ページに表示列・印刷・表編集・コピー・Excel I/O を追加**: 機材一覧と同等のツールバー (Excel インポート/出力 / 印刷プレビュー / 表示列ピッカー / 表編集モード / コピー登録) を実装。サーバーは `/equipment/cables` `/equipment/connectors` 配下に template / export-xlsx / import-preview / import / PATCH エンドポイントを新設。 // (v2.8.37: モバイル InsertGap タップ展開式) // (v2.8.36: 機材ダッシュボードにケーブル/コネクタを追加) // (v2.8.35: ケーブル管理 + コネクタ管理ページ追加)
