...
**現在のバージョン**: v2.8.29 (dev) — Qシート UI/UX モダン化 **Phase 1〜4 完了**。Phase 1+2 (Toast/DADS/Tabs/Sheet/FAB/モバイルフル編集/StageEditor Dialog) に加え、Phase 3 で OnAir/Rundown/Prompter を `<html class="dark">` 強制 + DADS semantic 化 + Socket 切断/再接続 Toast、Phase 4 で PreviewModal に A3 対応 + 印刷 page-break ルール + Ctrl+S グローバル + a11y 詰め (aria-current/keyshortcuts/focus-visible)。Socket.IO + JSONB 構造完全維持。
