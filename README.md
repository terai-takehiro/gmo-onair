...
**現在のバージョン**: v2.8.31 (dev) — **マスター + LED 統合 + 壁/床モダン UI**: EditorSidebar の `LED` タブを廃止し 3 タブ構成に再編、LED/XR シーン・立ち位置図・マスタデータを `ResourceCard` 統一カードで並列表示。`MasterSection` pill 改善 + `LedSceneSection` 壁/床を semantic 反対色（壁=primary 床=warning）の 2 カラムミニカード化。CueRow / CueRowMobileEditor の壁/床インライン表示も同形式に統一。機能変更なし。
