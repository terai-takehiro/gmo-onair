...
**現在のバージョン**: v2.8.20 — 案件管理3機能改善（仮スケジュール飛び日対応・料金表↔概算見積連携・見積書値引きダイアログ）
...
| バージョン | 内容 |
|---|---|
| **v2.8.20** | **案件管理3機能改善**: ①仮スケジュール飛び日対応 — `project_dates` テーブルを新設（マイグレーション074）、案件フォームに「追加の日程（飛び日対応）」セクションを追加し本番・リハ以外の日付（撤去・中日など）を自由に追加可能。一覧画面で `dates_count > 2` の案件に「N日（飛び日）」バッジ表示。②料金表↔概算見積連携 — `BusinessProjectView` の見積編集ダイアログに「料金表から追加」ボタンを追加し既存 PricingItemPicker を統合。estimate でも `projects.expected_amount` を最新値で上書き同期。③見積書値引きUI — 専用 `DiscountDialog` コンポーネント新規作成。各明細行に[%]ボタン、ダイアログ末尾に「全体値引き」ボタン。率(%)/固定額(¥)切替・プレビュー・任意の項目名で値引きマイナス行を自動挿入 |
