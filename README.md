...
**現在のバージョン**: v2.8.19 — セキュリティ修正（QシートPreviewModalのXSS、JWT_SECRETフォールバック撤廃、ADMIN_EMAILの環境変数化）
...
| バージョン | 内容 |
|---|---|
| **v2.8.19** | **セキュリティ修正**: ①QシートPreviewModalで `dangerouslySetInnerHTML` で描画していた `en.html` を HTMLタグ除去 + `white-space: pre-wrap` のテキスト描画に変更しストアドXSSを排除。②`server/src/config.ts` と `server/src/contexts/liveops/crypto.ts` の `'dev-jwt-secret-do-not-use-in-production'` ハードコードフォールバックを撤廃。本番は env 必須、dev は起動時にランダム値生成（再起動でセッション失効）。`docker-compose.yml` の `JWT_SECRET_DEV` も `${JWT_SECRET_DEV:?...}` で必須化。③`seed-admin.ts` から本番管理者メールアドレスのハードコードを撤廃し、`ADMIN_EMAIL` 環境変数で指定する方式に変更（未設定なら no-op）。`seed.ts` の dev 用 admin メールも `admin@onair.local` に変更 |
