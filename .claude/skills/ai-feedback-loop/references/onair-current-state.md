...
**打合せの録音 → 議事録**（v4.0 で実装）。文字起こしは **Whisper**（2026-08 に決定）。
着手時に満たすもの:

| 条件 | どうするか |
|---|---|
| 1 記録 | 文字起こしの**全文**と AI が整形した議事録を `ai_outputs`（kind=`minutes_draft`）に。**音声そのものは持たない**（容量と個人情報。必要なら BOX 側） |
| 2 差分 | 人が議事録を直したら、`minutes` の保存時にサーバーが自動比較（`project-ai-feedback.service` と同じ形） |
| 3 成果 | 「決定事項・未確認事項」から作ったタスクが**期限内に閉じたか**（`task_intake` と同じ導出） |
| 4 還流 | `get_ai_feedback_digest` に `kind=minutes_draft` を足す |
| 5 レビュー | 上記の月1回に含める |

**先に決めておくこと**: 録音の保存先と保存期間（BOX の社内限り／何日で消すか）。
議事録は取引先の発言を含むので、**録っていることを相手に伝える運用**もあわせて決める。
