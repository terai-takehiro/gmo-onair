    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-1.5">
      {items.map((m) => {
        ...
        return (
          ...
          >
            <span className="flex items-center gap-1.5 text-[10px] font-black tracking-wider leading-none">
              <kbd className="inline-flex items-center justify-center min-w-[1.25rem] h-4 px-1 rounded border border-slate-700 bg-slate-800 text-[9px] font-bold text-amber-400">
                {m.keyHint}
              </kbd>
              <span className={active ? 'text-amber-400' : 'text-slate-500'}>{m.label}</span>
            </span>
          </button>
        );
      })}
