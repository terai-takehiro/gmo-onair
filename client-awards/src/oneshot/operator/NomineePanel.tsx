  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="space-y-1.5">
        <div className="flex items-center gap-1.5 text-[10px] font-black tracking-widest text-slate-400 uppercase px-0.5">
          <Trophy className="h-3 w-3 text-amber-500" />
          賞 · Award
          <span className="text-slate-600 normal-case font-medium tracking-wide">
            ({awards.length})
          </span>
        </div>
          ...
          {awards.map((a, i) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'rounded-md border px-2.5 py-1 text-xs transition-all flex items-center gap-1',
                  active
                    ? 'border-amber-400 bg-amber-500/15 text-amber-300 ring-1 ring-amber-500/30'
                    : 'border-slate-700/60 bg-slate-800/40 text-slate-400 hover:bg-slate-700 hover:text-slate-200'
                )}
                title={`${a.award} · ${a.divisions.length}部門 · ${totalItems}名`}
              >
                <span className="font-bold">{a.award}</span>
                <span className="text-[10px] opacity-60 tabular-nums">{totalItems}</span>
              </button>
            );
          })}
        </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {currentAward.divisions.map((d) => {
              ...
              return (
                ...
                >
                  {d.division}
                  <span className="ml-1 text-[10px] opacity-60 tabular-nums">{d.items.length}</span>
                </button>
              );
            })}
          </div>
