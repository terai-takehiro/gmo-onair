# リアルタイムCG — **移行中（→ 制作技術支援＞テロップCG。URLのみ生かす・一覧には出さない）**

⚠️ **2026-09-06 に「凍結」から「移行中」へ整理した。** このアプリは v4 で「据え置く」のではなく、
**制作技術支援のミニアプリ「テロップCG」（`client-techops/src/pages/graphics/`・
`server/src/contexts/graphics/`）へ機能を移している途中の移行元**である。
用語の定義は [docs/v4-plan.md](../docs/v4-plan.md) の「用語」の節。

- **ここには新しい機能を足さない。** 足したい機能はテロップCG側に作る
- **URL・API・Socket.IO・配信・ビルドは生かす**（本番の運用を止めない）。見た目も変えない
- **一覧・アプリ切替・左メニューには出さない**（`shared/src/client/apps.ts` の `frozen: true` は
  「一覧に出さない印」としてそのまま使っている。凍結の意味ではない）
- 機能・UI/UX のゼロベース再設計は [docs/design/v4/graphics-redesign.md](../docs/design/v4/graphics-redesign.md)、
  データ・演出の移植計画と進捗は [docs/design/v4/graphics-awards-migration-plan.md](../docs/design/v4/graphics-awards-migration-plan.md)
- 移行が検証環境で確認できたら、入口をテロップCGへ一本化してこのアプリを「廃止」へ進める（判断はユーザー）

以下は「凍結」だった時点の記録（URL を生かす仕組みは移行中もそのまま）。

ベースパス `/awards/`・ポート 5179。

## いまの状態（2026-08-25〜。2026-09-06 からは「移行中」）

このアプリは v4.0.0 の「凍結」（見た目は変えないが URL は生かす）に戻してある（2026-09-06 に「移行中」へ整理。URL の扱いは同じ）。
