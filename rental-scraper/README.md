...
### 既知の不具合: レンタル費用がかなり取得できていない（2026-08-24で対処）

⚠️ TOC・レスターどちらも一部の商品で価格が「ー」（未取得）のまま、という報告。

**レスター側の主な原因**: `restar_scraper.py` の価格正規表現が
「￥12,000-(税込)」という**1パターンにしか一致しない**厳密な書式だったため、
実ページがこの書式からわずかでも外れる（「(税込)」の注記が無い・￥ではなく円表記・
半角¥ 等）と price_net が取れない。しかも TOC 側にある「価格取得不可」の診断ログが
レスター側には無く、**取れなかったこと自体がログにも残らず**気づきにくかった。

修正: `PRICE_RE`（厳密な書式）にまず当て、駄目なら `PRICE_FALLBACK_RE`
（￥表記／円表記のどちらか最初に見つかったものを拾う緩いフォールバック）を試し、
それでも見つからなければ TOC と同じ形で `価格取得不可` 診断ログ（`spec keys=[...]`）を
残すようにした。回帰テストは `test_restar_scraper.py` の `ParseItemDetailPriceTest`。

**TOC 側**は 2026-08-22 の対処（上記「TOCの価格が出ない」節）以降も一部の商品で
未対処のまま残っている可能性がある。実サイトの構造がサンドボックスから検証できない
制約は変わっていないため、`価格取得不可` ログの `spec keys`・`ページ内の価格らしき数値`
を見て `_extract_price_from_specs` のキーワードを調整するのが引き続き唯一の当たりの
付け方になる。

### 既知の不具合: レスターだけジャンル分けが効いていない（2026-08-24で対処）

⚠️ レスターの商品がカテゴリの絞り込みチップでほとんど1つのカテゴリに固まってしまう、
という報告（TOC はカテゴリ一覧ページのパンくずから正しく分かれている）。

原因: `restar_scraper.py` の category は、商品ページ内で
`/service/solutions/rental` を含む `<a>` タグの**最後の1つ**を機械的に採用していた。
パンくずの大分類「レンタル」自体や、本文中・関連商品欄・フッター等にある無関係な
同ドメインリンク（ページ後方に出やすい）を拾ってしまい、実質ほぼ全商品が
`category = "レンタル"` に潰れていた（画面の「カテゴリで絞り込む」チップは
`qsheet_rental_items.category` を distinct しているだけなので、この潰れが
そのままジャンル分けの機能不全として現れる）。

修正: 商品ページのスペック表（テーブル）に「カテゴリ」「ジャンル」「分類」等の
行があれば、パンくずより先にそちらを使うようにした（TOC の
`_extract_price_from_specs` と同じ考え方のフォールバック。`_extract_category_from_specs`）。
見つからない場合のみ従来のパンくずへ倒す（診断ログ付き）。この機会に TOC・レスター
共通で使うテーブル抽出（旧 `toc_scraper._extract_tables_as_dict`）を `common_db.py` の
`extract_tables_as_dict` へ集約した。回帰テストは `test_restar_scraper.py` の
`ParseItemDetailCategoryTest` / `ExtractCategoryFromSpecsTest`。

⚠️ **実サイトのスペック表に実際に「カテゴリ」「ジャンル」に相当する行があるかどうかは
未確認**（`CATEGORY_KEYS` の候補名も含めて、このセッションのサンドボックスからは
対象サイトへ接続できないため検証できていない）。デプロイ後のログに
`カテゴリはパンくずのフォールバックを使用` が大量に出る場合、実際の見出し名が
`CATEGORY_KEYS` の候補と合っていないということなので、ログの `spec keys` を見て
候補を追加すること。
