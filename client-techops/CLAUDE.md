...
- **2026-09-06 に機能と UI/UX をゼロベースで再設計した。段A（①一覧＋右パネル・④設定1画面・
  ...
  段A・段Bで実装したファイルの一覧・意図した簡略化は同文書 §11「段A 実装メモ」「段B 実装メモ」
  （段Bはマルチエージェントのワークフローで実装・検証し、検証段の批判的レビューが実際に
  安全装置の抜けを検出→修正した経緯も記録してある）。
...
- 残りの実装の段取りは再設計 §11（段C: 台本から取り込む → 段D: スマホ →
  段E: 台本に追従・前の番組からコピー → 段F: 旧リアルタイムCGを畳む）。**段ごとに別 PR**
