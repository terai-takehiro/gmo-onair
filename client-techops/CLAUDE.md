...
- **2026-09-06 に機能と UI/UX をゼロベースで再設計した。段A（①一覧＋右パネル・④設定1画面・
  ...
  OA中の行内操作）・段C（コーナー見出し・台本から取り込む・台本と違いますバッジ）は実装済み。**
  ...
  段A〜Cで実装したファイルの一覧・意図した簡略化は同文書 §11「段A 実装メモ」「段B 実装メモ」
  「段C 実装メモ」（段B・段Cはマルチエージェントのワークフローで実装・検証し、検証段の
  批判的レビューが実際に問題を検出→修正した経緯も記録してある。段Cはさらに、
  検証に使ったシード台本〈`seed-subapps.ts`〉自体に既存のギャップ〈テロップ/映像/音声セルが
  実編集画面と違う形・行にIDが無い〉があることも見つかっている——段D/Eで同じ台本連携を
  検証するときも当たりうる）。
...
- 残りの実装の段取りは再設計 §11（段D: スマホ → 段E: 台本に追従・前の番組からコピー →
  段F: 旧リアルタイムCGを畳む）。**段ごとに別 PR**
