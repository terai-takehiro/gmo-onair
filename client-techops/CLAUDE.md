...
- **2026-09-06 に機能と UI/UX をゼロベースで再設計した。段A（①一覧＋右パネル・④設定1画面・
  左メニューのサブ項目・戻り先の統一・死んだ UI と固定文字列の削除・`GRAPHICS_RE` 修正）・
  段B（②本番モードの作り直し。TAKE／CLEARの2動詞・OA/NEXT・いま出ているものの帯・
  OA中の行内操作）・段C（コーナー見出し・台本から取り込む・台本と違いますバッジ）・
  段D（⑤依頼フォームの3欄化＋台本から選ぶ・⑥スマホ閲覧＋緊急CLEAR）は実装済み。**
  ...
  段A〜Dで実装したファイルの一覧・意図した簡略化は同文書 §11「段A 実装メモ」〜「段D 実装メモ」
  （段B〜Dはマルチエージェントのワークフローで実装・検証し、検証段の批判的レビューが
  実際に問題を検出→修正した経緯も記録してある。段Cはさらに、検証に使ったシード台本
  〈`seed-subapps.ts`〉自体に既存のギャップ〈テロップ/映像/音声セルが実編集画面と違う形・
  行にIDが無い〉があることも見つけている——段D以降で同じ台本連携を検証するときも当たりうる）。
...
- 残りの実装の段取りは再設計 §11（段E: 台本に追従・前の番組からコピー →
  段F: 旧リアルタイムCGを畳む・段A〜Eを検証環境で確認してから）。
  段A〜Dは1本のPRにまとめて出した（開発中は段ごとに commit・push のみで PR は都度出さず、
  「PRを」のご指示を受けてまとめて出す運用にした）
