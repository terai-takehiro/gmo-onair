...
import { downloadPreviewCsv, printPreview } from "./previewExport";
...
export default function PreviewModal({ state, onClose, docUpdatedAt, docCreatedAt, docTitle }: PreviewModalProps) {
  ...
  const handleCsvDownload = () => downloadPreviewCsv(state, docTitle);

  const handlePDF = () => {
    if (!pageRef.current) return;
    printPreview({ pageEl: pageRef.current, paperSize, margin, mono, pageMode, meta: state.meta });
  };
