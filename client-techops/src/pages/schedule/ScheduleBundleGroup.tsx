  ...
  return (
    <section>
      <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1 border-b border-border pb-1.5">
        <h2 className="text-cardtitle text-foreground">{bundle.label}</h2>
      ...
      <div className="mt-2 space-y-2">
        {bundle.schedules.map((s) => (
          <button
            ...
            className="flex w-full min-h-tap flex-col items-start gap-1 rounded-card border border-border bg-card p-4 text-left hover:bg-accent"
          >
            <div className="flex w-full flex-wrap items-center gap-2">
              <Calendar className="h-4 w-4 shrink-0 text-muted-foreground" aria-hidden="true" />
              <span className="text-cardtitle text-foreground">{formatDateJp(s.service_date)}</span>
              {s.location_name && <span className="text-sub text-muted-foreground">・ {s.location_name}</span>}
              <Badge variant={SCHEDULE_STATUS_BADGE_VARIANT[s.status]} className="ml-auto">
                {SCHEDULE_STATUS_LABEL[s.status]}
              </Badge>
            </div>
            <div className="text-sub text-foreground">{s.title || "（無題）"}</div>
