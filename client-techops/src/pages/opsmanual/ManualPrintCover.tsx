...
// 冊子のメタ情報（タイトル・資料番号・案件名/管理番号または番組名・対象日）から自動生成する
// （設計判断3「表紙・目次・柱」）。
//
...
export default function ManualPrintCover({ manual, hasSecrets }: ManualPrintCoverProps) {
  // 案件（管理番号があれば併記）が優先。案件が無い（program_id で作った冊子の）ときは番組名
  ...
  return (
    ...
    >
      ...
      {/* 大見出し（表紙）は25〜34pt（§8-3）。冊子のタイトルそのまま・人の手で打ち直さない。
          ここは印刷される紙の見出し（この冊子のタイトルそのもの）であって、アプリの「画面の名前」
