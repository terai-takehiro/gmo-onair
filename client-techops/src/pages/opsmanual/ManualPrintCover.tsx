...
// マニュアルのメタ情報（タイトル・資料番号・案件名/管理番号または番組名・対象日）から自動生成する
// （設計判断3「表紙・目次・ヘッダー」）。
//
// 取扱注意（§7-2）はヘッダー（ManualPrintHeader）にも出るが、表紙は初めて手に取ったときに
// 必ず目に入る面なので、ここにも大きく独立して出す（§7-2「表紙とヘッダーに…自動で出る」＝
...
export default function ManualPrintCover({ manual, hasSecrets }: ManualPrintCoverProps) {
  // 案件（管理番号があれば併記）が優先。案件が無い（program_id で作ったマニュアルの）ときは番組名
  ...
  return (
    ...
    >
      ...
      {/* 大見出し（表紙）は25〜34pt（§8-3）。マニュアルのタイトルそのまま・人の手で打ち直さない。
          ここは印刷される紙の見出し（このマニュアルのタイトルそのもの）であって、アプリの「画面の名前」
