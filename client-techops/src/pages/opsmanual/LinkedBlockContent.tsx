...
// 振り分けるだけで、キャンバスの操作（つかむ・伸縮・回転・スナップ…）は一切知らない
// （段Bのスコープ分担と同じ考え方）。
//
...
import TechLinkedContent from "./linked/TechLinkedContent";
...
export default function LinkedBlockContent({ block, resolved }: Props) {
  ...
  switch (key) {
    ...
    case "tech.patch":
    case "tech.staff":
      return <TechLinkedContent blockKey={key} data={resolved.data} options={options} />;
    default:
