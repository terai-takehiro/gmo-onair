...
//
// 出る位置の名前は「フルスクリーン」「サイドスーパー」など5字超が普通に混ざるので、
// 既定の 62px 帯では右端が行ごとにずれる。モック（v4-mockup-graphics.dc.html）の
// 位置のバッジは幅 96px 固定なので、`fixedW` で列ごと 96px にそろえる
...
/**
 * 依頼の状態バッジ。「未対応」〜「テロップ化済み」（7字）まで幅が揃うよう
 * `fixedW` で 96px に固定する（`w` は `SlotWidth` の7段から選ぶ列幅・既定 96px）。
 */
export function RequestStatusBadge({ status, w = 96 }: { status: GraphicsRequestStatus; w?: 96 | null }) {
  return <TableBadge label={REQUEST_STATUS_LABELS[status]} w={w} fixedW={96} variant={REQUEST_STATUS_VARIANT[status]} />;
}
