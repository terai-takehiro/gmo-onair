...
// 品目・置いた数・保有数・保管場所の一覧。保有数を超えても止めない（赤字で
  ...
  return (
    <div className="flex h-full min-h-0 flex-col p-3">
      <div className="flex items-center gap-1.5 border-b border-border pb-1.5 text-[10px] font-bold text-muted-foreground">
        ...
        <span className="num w-11 text-right">置いた数</span>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {rows.map(({ key, cat, n }) => {
          ...
          return (
            <div key={key} className="flex items-center gap-1.5 border-b border-border/60 py-1.5 text-[11.5px]">
              ...
              <span className={`num w-11 text-right font-bold ${over ? "text-destructive" : ""}`}>{n}</span>
        ...
        {rows.length === 0 && <p className="pt-3 text-[11px] text-muted-foreground">まだ何も置いていません。</p>}
      </div>
      ...
      <p className="pt-2 text-[10.5px] leading-[1.6] text-muted-foreground">
        保有数を超えても置けます（赤字で知らせるだけ）。同じ日の別の図面との合算はしません。家電4件は動かせないのでこの一覧には出しません。
      </p>
    </div>
  );
}
