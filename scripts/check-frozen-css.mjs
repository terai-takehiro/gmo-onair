 ...
 * ── なぜ要るのか ────────────────────────────────────────────
 *
 * v4.0.0 の決めごとは「**制作資料・計時LIVE は見た目を今日のまま**」です
 * （技術資料は v4.1.8 でアプリごと削除、リアルタイムCG はその後廃止・配信停止した
 * ので、この検査の対象からも外れている）。
...
/**
 * 凍結2アプリ。`client-awards`（リアルタイムCG）は廃止済み・build:all の対象外
 * になったのでここからも外した（`client-awards/CLAUDE.md` 参照）。
 */
const APPS = [
