 ...
 *
 * 一方、書き込む側の上限は**バイト数**（`CLAUDE.md` が文脈に載る費用はバイトで効くため）。
 ...
 *
 * ⚠️ **`vX.Y.Z — ` が付いていても外すこと**（レビューでの指摘・P2・2度目）。
 * 呼ぶ側が持っているのは**版の行**（`v4.1.3 — **見出し**。本文`）なので、前置きを外さないと
 * `^**` に当たらず、**見出しごと本文として数えます** ＝ 比べているのは**行まるごとの長さ**で、
 * 生成側が比べる `description` ではありません。
 * ⚠️ **見出しの長さが違うと逆転します** — 全文の見出しが長いと、
 * **本文では要約のほうが長いのに、行の長さでは全文のほうが長い**組み合わせが作れ、
 * **検査を通り抜けて全文が画面から消えます**。**両方の形を受けます。**
 */
