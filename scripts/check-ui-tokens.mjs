...
const RULES = [
  ...
  },
  {
     ...
     *
     * **「期限なし」も止める**（2026-09-23）。口語ではないが、正は「期限未設定」で（`docs/wording.md`）、
     * 同じ状態が画面によって「期限なし」「期限未設定」と2通りに呼ばれていた（約20か所）。
     *
     * **「期限を過ぎた」「期日を過ぎている」も同じ口語**として止める（2026-09-22・#727 の宿題の回）。
     * 「過ぎています」だけを止めていたので、画面には「期限を過ぎたもの」「返事の期限が過ぎているもの」が
     * 十数か所残っていた（利用者からは「過ぎてますとかそういう表現」と、型ごとのご指摘だった）。
     * 「言い過ぎ」「多過ぎ」のような期限と関係ない語には当たらないよう、**期限を表す名詞 ＋ を/が ＋ 過ぎ**の形だけにしてある。
     *
     * `codeOnly`: コメントは対象外 — 「なぜこの言い方を禁じたか」を
     * コメントに書き残せなくなると、次に触る人が理由ごと消してしまう。
     * 記録（BASELINE）は持たない — 既存分をこの版で全部直したので 0 件から始まる。
     *
     * ── 期限までの残り・当日も同じ規則で止める（PR #727 の宿題③・2026-09-22）──
     * 「過ぎています」系を止めたあとも、残り・当日の側に口語が残っていた:
     * 「あと3日」「あと ${n}日」「今日が期限」「今日まで」「本日返却」、表記揺れの「3日 超過」。
     * 正は `duePartsOf` の **期限超過 ／ N日超過 ／ 残りN日 ／ 本日 ／ 明日 ／ 期限未設定 ／ 期限内**。
     *
     * 当ててはいけない語（誤検出）を必ず避ける形にしてある:
     *  - 「あとで見る」「あとから」「あと何日か」 — 「あと」の直後が**数字・式（`${…}` / `{…}`）＋「日」**
     *    のときだけ当てる。受信箱の機能名「あとで見る」や説明文の「あと何日」は当たらない
     *  - 「あと {mmss(left)} で自動で止まります」（録音の残り時間）— 直後が「日」でないので当たらない
     *  - 「残り」「本日」「期限超過」「N日超過」 — 正しい言い方そのもの。空白入りの「N日 超過」だけを当てる
     */
    re: /過ぎています|過ぎてます|(?:期限|期日|予定日|見直し日|本番日|締め日|返す予定の日)[をが]過ぎ|を過ぎた(?:もの|まま|工程|タスク)|期限ぎれ|期限なし|あと[ \u3000]*(?:\$\{[^}]*\}|\{[^}]*\}|[0-9０-９]+)[ \u3000]*日|今日が期限|今日まで|今日期限|明日が期限|本日返却|日[ \u3000]+超過/,
    why: '期限の言い方は **「期限超過」「N日超過」「残りN日」「本日」「明日」** に固定です（`docs/wording.md` ルール8・9）。'
       + '✕「2日 過ぎています」「返却の日を過ぎています」→ ○「2日超過」「返却期限超過」。'
       + '✕「あと3日」「今日が期限」「今日まで」「本日返却」「3日 超過」→ ○「残り3日」「本日」「本日」「本日」「3日超過」。'
