...
const TREE = [
  ...
  ['Wiki (新規)', [
    ...
    // ⚠️ 会話の画面なので見出しの枠（`ui/pageHeader`）を持たない。**この画面にしかない部品**で判定する
    ['⑨ AI に聞く', '/wiki/ask', 'client-wiki/src/pages/ask/AskPage.tsx', 'AskMaterialsPanel',
      '出典が出せない質問には**答えず**「足りないページ」に登録する（判断8）。' +
      'スレッドの一覧は共通の左メニューの中（**画面の中に2本目のナビの列を作らない**）。' +
      '⚠️ 質問は**語に分けてから**検索にかける —— 質問文のまま渡すと助詞が混ざって必ず0件になり、' +
      '答えられるはずの質問まで「足りないページ」に積まれる'],
    ['⑩ 見直し', '/wiki/review', 'client-wiki/src/pages/review/ReviewPage.tsx', 'ReviewDueTab',
      '月1回の場（見直し予定／足りないページ／AI の直され方）。' +
