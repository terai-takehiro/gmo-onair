...
lines.push('## まだ入っていない機能', '',
  ...
  '- **スマホ ④ 受付／案件の種類をきめる**（Phase 6 の残り1枚）— モックのスマホ 13 枚のうち 12 枚は作りました（⑩ は ⑬ に統合したので画面は 11 枚）。残るのは「段に分かれた案件づくり」で、いまも PC のモーダル1枚が縦に畳まれます。中身は [docs/design/v4/mobile.md](design/v4/mobile.md)',
