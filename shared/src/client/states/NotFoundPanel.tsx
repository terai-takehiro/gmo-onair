  ...
  return (
    <div className={cn('mx-auto max-w-2xl px-4 py-10 sm:py-14', className)}>
      <div className="rounded-card border border-border bg-card p-6">
        ...
        {path && (
          <p className="mt-3 break-all text-sub text-muted-foreground">
            <code className="rounded bg-muted px-1.5 py-0.5 text-sub-sm text-foreground">{path}</code>{' '}
            という行き先はありません。
          </p>
        )}
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {onOpenSiteMap && (
            <button
              ...
              className="min-h-tap inline-flex items-center gap-1.5 rounded-control bg-primary px-4 text-list text-primary-foreground transition-colors hover:bg-primary-800"
            >
          ...
          {home && (
            <button
              type="button"
              onClick={home.onGo}
              className="min-h-tap inline-flex items-center gap-1.5 rounded-control border border-border bg-card px-4 text-list text-foreground transition-colors hover:bg-accent"
            >
