...
export function humanizeError(err: unknown): HumanCause {
  ...
  /*
   * ⚠️ **混み合っている(429)と、時間切れ(504)と、壊れた(5xx)を分ける。**
   * 以前は3つとも「サーバー側で処理が止まりました」の一言だったので、
   * **利用者が何をすれば直るのか分からず、開発者も原因を切り分けられなかった**
   * （財務の絞り込みで頻発していた件。nginx のレート制限は既定 503 を返すので、
   * 上流が壊れたときの 503 と見分けが付かなかった → `nginx/gmo-onair.conf` で 429 にした）。
   */
  if (status === 429) {
    return {
      cause: '短い時間に何度も読み込んだため、少し待つ必要があります。',
      next: '10 秒ほど待ってから、もう一度お試しください。',
    };
  }
  if (status === 504) {
    return {
      cause: '時間内に読み終わりませんでした。',
      next: '期間や案件で絞ってから、もう一度お試しください。',
    };
  }
    ...
    >
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-cardtitle text-foreground">{title}</p>
          <p className="mt-1 text-sub text-foreground/80">{c.cause}</p>
          <p className="text-sub text-foreground/80">{c.next}</p>
          {inputPreserved && (
            <p className="mt-2 text-list text-foreground">入力した内容は残っています。</p>
          )}
          {(onRetry || action) && (
            <div className="mt-3 flex flex-wrap gap-2">
              {onRetry && (
                <button
                  type="button"
                  onClick={onRetry}
                  className="rounded-control bg-card px-4 py-2 text-list text-foreground shadow-sm transition-colors hover:bg-secondary"
                >
                  {retryLabel}
                </button>
              )}
