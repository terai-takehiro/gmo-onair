 ...
 *
 * v4 着手時点で `confirm()` が **58 か所**・`alert()` が **25 か所**残っていた
 * (v4 対象3アプリだけの数)。
  ...
  return createPortal(
    ...
    >
      <div
        ...
        className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-card border bg-card p-5 shadow-lg"
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 id="confirm-title" className="text-cardtitle text-foreground">{req.title}</h2>
            {req.description && (
              <p id="confirm-desc" className="text-sub mt-1.5 whitespace-pre-line text-muted-foreground">
                {req.description}
              </p>
            )}
          </div>
