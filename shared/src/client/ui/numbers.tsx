...
/**
 * 万円に丸めた**数値**。文字にする前の段階が要るとき (グラフの軸など) に使う。
 * 丸め方の決めごとはここ1か所。
 */
export function toMan(value: number | string | null | undefined): number | null {
  if (value === null || value === undefined || value === '') return null;
  const n = Number(value);
  if (!Number.isFinite(n)) return null;
  // 四捨五入は「絶対値で丸めて符号を戻す」形にそろえる (負の数で差が出ないように)。
  // `+ 0` は **-0 を 0 に潰すため**。これが無いと -4,999 円が「¥-0万」と出る
  // (符号だけ残ってゼロ、という読めない表示になる)。
  return Math.sign(n) * Math.round(Math.abs(n) / 10_000) + 0;
}

/**
 * 1万円に満たない額は**そのままの円**で出す形。
 *
 * 日常業務の週報がこの形だった (`Math.abs(n) >= 10000` で切り替え)。**残すのが正しい**:
 * 5,000 円を「¥1万」と出すと**倍に見える**。万円は「大きい額をざっくり比べる」ための
 * 表示なので、小さい額には向かない。
 *
 * `manYen` (常に万円) と使い分ける — 桁がそろっている一覧・グラフの軸は `manYen`、
 * 大小がばらつく単発の額はこちら。
 */
export function compactYen(value: number | string | null | undefined): string {
  if (value === null || value === undefined || value === '') return '—';
  const n = Number(value);
  if (!Number.isFinite(n)) return '—';
  if (Math.abs(n) < 10_000) return `¥${n.toLocaleString('ja-JP')}`;
  return manYen(n);
}
