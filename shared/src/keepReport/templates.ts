 ...
 * 資料のテンプレ（スライドの骨組み）の正。
 *
 * 画面のプレビュー（client-daily の資料ビルダー）と PowerPoint 出力（server の keep-pptx）が
 * **同じ位置・同じ部品**で描くための1か所。位置は 1280×720 の仮想キャンバスに対する %。
 *
 * ── GMO流会議フォーマット Ver.2.5 で守るもの（docs/design/v4/keep-report.md §6.3）──
 * - ヘッダー: 題（左上・青・36pt 相当）と、その下のトークスクリプトの帯（青＝進行担当／発表者、緑＝会議オーナー／全員）
 * - フッター: GMO INTERNET GROUP ロゴ・フォーマット名の青いタグ・Strictly confidential・ページ番号
 * - 色の役割: ポジ青・ネガ赤・マーカー黄・KGI 緑・KPI 黄
 * - 題の書式【カテゴリ｜緊急×重要｜時間】
 * 文字の大きさは目安（表・一覧は小さくてよい）。
 */
import type { SlideTemplateKey } from './types';
