/**
 * 計上会社（SCS / GSS / GMO）— 画面側の小さな純粋関数（値の検査と表示名）。
 *
 ...
 *
 * ⚠️ **どの会社になるかの規則はここに置かない。** 「グループ外→SCS／グループ内→GSS／
...
/**
 * 計上会社の並び。main の `legal_entities.sort_order` と同じ（親会社 SCS → GSS → GMO）。
 * 隔週キープの画面のチップ・計上会社別の表は GSS を先に置く（切替前は全行 GSS）— `client-daily` の `keep/format.ts`
 */
export const BUSINESS_ENTITIES = ['SCS', 'GSS', 'GMO'] as const satisfies readonly BusinessEntity[];
