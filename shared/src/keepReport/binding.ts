 ...
 * 同じ関数の写しが `server/src/contexts/dailyops/services/keep-binding.ts` にあり、
 * `shared/tests/keepReportDeckParity.test.ts` が両方を同じ材料で走らせて同じ答えになることを固定しています。
 * **こちらを直したら写しも直すこと。**
 */
import type { KeepReportPack, SlidePage, SlidePart, ProjectPageData, MonthlyTrendPoint } from './types';
