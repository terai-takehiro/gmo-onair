...
describe('支払期日の急ぎ具合', () => {
  ...
  it('色だけに頼らない — 文字にも残り日数を書く', () => {
    ...
    expect(dueState('2026-08-31', '2026-08-31').text).toBe('8/31（本日）');
    expect(dueState('2026-09-02', '2026-08-31').text).toBe('9/2（残り2日）');
    // 先のものは日付だけ（急がないものに毎回「残り62日」と書かない）
...
describe('押せるのに 403 にしない（受領書類）', () => {
  it('「仕入・販管費に登録」は sales の editor にだけ出す', () => {
    // 2026-09（migration 281）: 一覧を「ひとつづり」で出すようにしたので、
    ...
    // **見積書には登録ボタンを出さない**（migration 281 で一覧に出るようになったぶん、
  ...
  it('却下した書類に辿り着ける（片づいたものの束に残る）', () => {
    /*
      ...
      **辿り着く道が1本も無いと、間違えて却下したものは画面から二度と見えない。**
      ...
      2026-09（migration 281）で一覧を「ひとつづり」にしたので、道の作り方が変わった:
