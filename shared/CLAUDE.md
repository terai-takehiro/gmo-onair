...
### スマホの上辺バーは「アプリ名」（モックに戻した・M10）

**モックの端末枠は、上辺バーが2種類**です:

| どこ | 中身 |
| --- | --- |
| トップ | **ロゴ** ／ 余白 ／ ベル ／ 本人（`☰` もアプリ名も出さない） |
| ほかの画面 | `☰` ／ **アプリ名のチップ** ／ 余白 ／ 検索（ロゴは出さない） |

**上辺バーが答えるのは「どのアプリにいるか」で、「どの画面か」は本文の見出し**が答えます。

- M7 でいちど**逆**にしていました（上辺バーにページ名・本文の大見出しは消す）。
  同じ名前が2回出るのを1回にする狙いでしたが、**モックに戻す**とご判断いただき、
  仕組みごと外しました（`PageTitleSlotContext` / `data-shell-title` /
  `data-shell-applabel` / `data-page-title` / `[data-shell-switch]` を消す CSS）。
  **本文の見出しが約 90px 戻ります**
- **残したのは説明文の1行化だけ**（`[data-page-sub]`）。「43件 ・ 全員が同じものを
  見ています」が2行に折り返して隣の切替ボタンを潰していたためで、
  上辺バーの話とは別です
- **ロゴはトップのスマホにだけ出します**。ほかの画面では `☰` ＋ アプリ名 ＋
  ベル ＋ 本人 が並ぶので、足すとアプリ名が切れます
- **ベルと本人はスマホでも残します**（モックの非トップ端末枠には検索アイコンしか
  ありませんが、**本人メニューがログアウトとマニュアルの唯一の入口**なので消せません）
