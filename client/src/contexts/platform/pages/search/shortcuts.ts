/**
 * ⑪ 探す の「すぐできること」と「場所」（モックの `spSearch`）
 *
 * **「やること」とは呼ばない** — タスクの画面（⑧）と同じ名前になり、
 * 押した人がタスクの一覧を期待する。ここに置くのはその場で終わる仕事だけ。
 *
 * ── なぜ表にするのか ────────────────────────────────────────
 *
 ...
 * ── モックとの違い（作り話をしない） ──────────────────────────
 *
 * モックの「すぐできること」は4つ（やり取りを記録／機材QR／予定を入れる／案件をつくる）、
...
/** すぐできること — **その場で終わる仕事**だけ置く（読みに行くものは「場所」へ） */
