...
import { NextAction, type NextActionActions } from './NextAction';
import {
...
export function ThreadCard({ a, today, canEdit, onRedo, redoing, nextActionActions }: {
  a: ActivityLog;
  today: string;
  canEdit?: boolean;
  ...
  redoing?: boolean;
  /** 次回アクションの完了・延期。渡さないと（未指定なら）ボタンを出さない */
  ...
  return (
    <article className="rounded-card border border-border bg-card p-4 shadow-sm lg:p-5">
      ...
      {a.next_action && (
        <NextAction a={a} overdue={!!overdue} canEdit={canEdit} actions={nextActionActions} />
      )}
