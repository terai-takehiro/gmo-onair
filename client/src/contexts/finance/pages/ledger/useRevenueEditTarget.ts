 ...
 * `stage=a_won,r_delivered,s_completed`（受注済み以降だけ）で絞っているので、
...
function submitBlockReason(editing: RevenueRow | null, selectedProjectId: string, customerId: string | null): string {
  ...
  if (editing?.group_id) {
    return 'この売上は按分グループに入っています。案件管理 > 按分グループの画面から直してください。';
  }
  if (!customerId) {
    return editing
      ? 'この売上に請求先（顧客）が入っておらず、案件にも顧客が設定されていないため保存できません。案件の編集画面から顧客を設定してください。'
      : '選んだ案件に顧客が設定されていないため保存できません。案件の編集画面から顧客を設定してください。';
  }
  return '';
}
