  ...
  return (
    <div className="rounded-card flex flex-wrap items-center gap-x-3 gap-y-2 border border-border bg-card px-3.5 py-2.5">
      <span className="flex shrink-0 items-center gap-1">
        {(Object.keys(VIEW_LABEL) as CalView[]).map((v) => (
          <button
            key={v}
            type="button"
            onClick={() => onView(v)}
      ...
      <span className="flex flex-wrap items-center gap-1">
        {layerDefs.filter((l) => l.show).map((l) => {
      ...
      <button type="button" onClick={onPickUsers} className={cn(ctl, userCount > 0 ? 'border-primary bg-primary-surface text-primary' : 'border-border bg-card text-secondary-foreground')}>
        <UserSearch className="h-3.5 w-3.5 shrink-0" aria-hidden="true" />
        {userCount > 0 ? `人で絞る（${userCount}）` : '人で絞る'}
      </button>
      <button type="button" onClick={onPickRooms} className={cn(ctl, roomCount > 0 ? 'border-primary bg-primary-surface text-primary' : 'border-border bg-card text-secondary-foreground')}>
        <DoorOpen className="h-3.5 w-3.5 shrink-0" aria-hidden="true" />
        {roomCount > 0 ? `部屋で絞る（${roomCount}）` : '部屋で絞る'}
      </button>
