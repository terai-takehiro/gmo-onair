...
/**
 * 「残り3日」「本日」「3日超過」。**本番日を越えたものを「残り -3日」と出さない**。
 *
 * 言い方は営業活動記録の期限（`activityLog/dueState.ts` の `duePartsOf`）と揃える
 * （`docs/wording.md` ルール8・9）。✕「あと3日」「今日」「3日前」は口語・相対表現で、
 * 画面ごとに言い方が割れる元になっていた。越えたものを「N日前」と書くと
 * 「本番が3日前に終わった」と読めてしまい、**押さえたまま残っている**ことが伝わらない。
 */
