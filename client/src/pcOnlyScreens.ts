...
export const CLIENT_PC_ONLY: PcOnlyEntry[] = [
  ...
  */
  ...
  /*
    ⚠️ **この段落は当時の記録（消さないこと）。** 前は「営業活動記録は v4 で
    作り直したので `CLIENT_MOBILE_OK` へ外した」だったが、統合先の分析3タブ
    （旧営業レビュー）が「3列を並べて打合せの場で映す」前提の PC 専用画面だったため、
    **統合にあわせて画面全体を PC 専用に戻した**（ご指示）。タブごとにスマホ対応が
    割れると `useIsMobile()` の判定を画面の中に書くことになり、PC専用の判定を1か所
    （この表）に集める方針が崩れる、というのが当時の理由。

    **いまの判断（2026-09・利用者からのご指摘でスマホ対応した）:**
    「営業活動記録・営業レビューが PC 専用でスマホから開けない」とご指摘をいただいた。
    営業活動記録は**外で開く画面**（訪問の前後に、その案件で次に何をするかを確かめる）で、
    記録タブは統合前からスマホで使えていたのに、分析3タブを同じ画面に足したせいで
    **記録タブごと開けなくなっていた**。「PC 向きのタブがあるから画面ごと PC 専用」は
    理由として通らない（⑥ 案件詳細も PC 向きのタブを持つが、画面は開ける）。
    → `CLIENT_MOBILE_OK` へ移し、画面の中の `PcOnlyPanel` の門も外した。
    分析3タブの中身のスマホ対応は別途行う。理由は下の `CLIENT_MOBILE_OK` を参照
  */
  ...
  */
  ...
  /*
    **売上・仕入・販管費の3つの台帳（`/budget/{revenues,purchases,sga}`）は
    ここから外し、`CLIENT_MOBILE_OK` へ移した**（v4ネイティブUI化）。
    元の理由「金額の桁を縦にそろえて読む表なので、畳むと桁が比べられません」は
...
export const CLIENT_MOBILE_OK: string[] = [
  ...
  */
  ...
  /*
    ── 営業活動記録・営業レビュー（この回・2026-09・利用者からのご指摘）──
    「スマホから開けない」とのご指摘で PC 専用から戻した（経緯は `CLIENT_PC_ONLY`
    の ⚠️ の段落）。記録タブは**営業担当の立場で作り直した**:
    ・既定を**案件別**にし、1案件＝1つの白い面（`activityLog/ByProjectRows.tsx`）。
      `Row stackOnMobile` ＋ `RowSlot hideOnMobile`（「記録された活動」列を落とす）で
      375px でも横スクロールが出ない
    ・期限の絞り込みは `FilterChips`（横スクロール対応・`min-h-tap`）
    ・**色帯のカードは使わない**（面は塗らず、赤いのは期限超過の日付の文字だけ）
    分析3タブ（ファネル・失注分析・営業評価）の中身のスマホ対応は別途行う —
    画面の入口を閉じる理由にはしない（⑥ 案件詳細が PC 向きのタブを持ったまま
    開けるのと同じ扱い）
  */
  '/sales/activity-logs',               // 営業活動記録（記録・分析の4タブ）
  ...
  */
  ...
  /*
    ── ③④⑤ 売上・仕入・販管費の台帳（v4ネイティブUI化・2026-09）──────
    PC 専用だった理由は「**金額の桁を縦にそろえて読む表**なので、畳むと桁が
    比べられない」だった。**これは表をそのまま縮めた場合の話**で、逃げずに
    正面から解いた:

    ・**金額は畳まない。** カードの右端に `MoneyCell width={128}` の
      **同じレール**を置くので、¥ はレールの左端・**数字の右端は画面左から
      常に同じ位置**（375px で 349px）に来る。`font-number`（等幅数字）で
      桁幅もそろうため、**カードを縦に並べただけで PC の金額列と同じ
      「そろった列」ができる**
    ・**カードの高さを一定にする。** 3行（コード・計上月／見出し／相手先・状態）を
      値が無くても必ず描くので高さが約90pxで固定され、金額が等間隔・同じ横位置に
      並ぶ。ここが可変だと桁は読めても目が縦に流れない
    ・**合計は上に貼り付ける**（`LedgerTotalBar`。スクロールで消えない）。
      金額の下に万円（`manYen`）を添えて、桁を1つ読み違えたときに気づけるようにした
    ・落とした値（税区分・支払期日・備考・精算番号など）は**捨てていない** —
      カードを押すと開く詳細シート（`LedgerDetailSheet`）に移してある

    **入れ子で押せるものは1つも置いていない**（カード全体が1つのボタン。
    状態・申請URL・案件への行き先はシートの下端に縦に並ぶ）。
    **Excel の取込・書き出しと削除はスマホに出さない** — どちらも取り消せない
    操作で、`/budget/vendors` で Excel を落とした前例に揃えた。
    実装は `contexts/finance/pages/ledger/`（`LedgerList` が幅の分岐を1か所で持つ）。
  */
  '/budget/revenues',                   // ③ 売上の台帳
  '/budget/purchases',                  // ④ 仕入の台帳
  '/budget/sga',                        // ⑤ 販管費の台帳
  /*
