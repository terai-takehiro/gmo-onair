...
> 開発用のコンテナのブラウザは Google Fonts に出られないので、`verify:ui` は最初に
> **書体を `curl` で落として手元に取り置き**（`.cache/google-fonts/`・約20MB・初回だけ10秒）、
> それをブラウザに差し込んでから測ります。**取り置きが無いと代替書体の字幅で測ることになり**、
> 桁揃い（±0.5px）の判定が本番と食い違います。取り置きだけ作りたいときは `npm run verify:font`。
...
```bash
npm test          # shared の純関数だけ (1秒未満)
```

**画面のテストは書きません。** 見た目は `npm run verify:ui` が実ブラウザで測るほうが確実で、
73画面ぶんのテストを入れても3か月で誰も直さなくなります。ここに置くのは
**「画面を見ても間違いに気づけない計算」だけ**です。

最初の1本は金額の丸め（`manYen` / `compactYen`）。着手時点で**万円に丸める実装が4通りあり、
同じ画面の中で食い違っていました**（KPI カードが `¥1,234.568万`、同じ画面のグラフが `¥1235万`）。
型でも lint でも `verify:ui` でも捕まらない種類の間違いです。
...
> ⚠️ **この決めごとは前からありましたが、守られていませんでした。**
> 2026-08 の2週間で `CLAUDE.md` 55 コミット・`package.json` 45・`README.md` 42 が動き、
> **PR のたびにこの3か所を書き換えて**いました（そして並行した2本は毎回ぶつかりました）。
> **文章で決めるだけでは足りなかった**ので、機械で止めます:
>
> - 作業 PR は **`docs/changelog.d/<枝の名前>.md`** に載せたい文を置くだけ（衝突しません）
> - `npm run lint` の `check-changelog.mjs` が、**作業 PR で版の3か所を触っていたら落とします**
> - リリースは **`npm run release:notes -- X.Y.Z`** が下の3か所を全部やります
>   （`RELEASE=1` か `release/` で始まる枝でだけ通ります）
>
> 決めごとの全文: [docs/branching.md「版の番号は、作業 PR では取らない」](docs/branching.md)
...
`CLAUDE.md` に残すのは**最新3件だけ**。4件目に押し出された版は
`docs/version-history.md` の「## 過去のバージョン」直下へ移してください
（**`npm run release:notes` が自動でやります**）
