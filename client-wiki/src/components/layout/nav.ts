...
const TOP: PlannedNavItem[] = [
  ...
  { label: '見直し',    to: '/review', icon: ListChecks,                 ready: true  }, // 段F で作った
];
 ...
 * 「やること」は Wiki では見直し（§6-⑦）で、段F で作った。「検索」は §6-④ を段D で作った。
 * **2つに満たないときはタブそのものを出さない**（1つだけのタブは行き先が1つしか
 * ない棒になって場所を取るだけ）。
 *
 * ⚠️ これは画面の下に出る横のタブで、**ナビの列ではない**。
 * 「サイドタブが増えすぎて窮屈」（2026-09-22 のご指摘）に当たるのは左右の列のほうで、
 * 下タブは共通シェルが1本だけ持つ。
 */
const TABS: Array<ShellMobileTab & { ready: boolean }> = [
  { label: 'ホーム',   to: '/',       icon: LayoutDashboard, end: true, ready: true },
  { label: 'やること', to: '/review', icon: ListChecks,                 ready: true  }, // 段F で作った
  { label: '検索',     to: '/search', icon: Search,                     ready: true  }, // 段D で作った
];
