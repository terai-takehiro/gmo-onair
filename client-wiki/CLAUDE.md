...
| **スペース管理** | `/spaces` | スペースの追加・編集（名前・説明・閲覧範囲・担当）・削除と、「メンバーだけ」のスペースのメンバー。**manager だけ・PC の画面**（`pages/spaces/`） |

**設計 §6-① の画面はこれで全部そろった。** 見直し（`pages/review/`・§6-⑦）とコメント（`components/comment/`・§6-②）は段F、スペース管理はその後で作った。
...
**段F で「見直し」を、その後「スペース管理」を `ready: true` にした。いまは1つも外れていない。**
新しい項目を足すときも、画面を作るまでは `ready: false` のままにすること。

⚠️ **「管理」の見出しの下（テンプレート・書き出しと取り込み・スペース管理）は権限で出し分けていない。**
権限の無い人が押すと、その画面が「権限が要る」案内を出す。出し分けるなら3つまとめて（1つだけ隠すと並びが人によって変わる）。

### スペース管理（`pages/spaces/`・`components/spaces/`）

- **管理の画面でも読めないスペースは出さない**（§8「存在ごと見えない」）。manager でも、入っていない
  「メンバーだけ」のスペースは一覧に出ず、直そうとすると 404。system_admin はすべて直せる
- **担当は必ずメンバーに加える**（サーバー `wiki-space-admin.service.ts`）。担当がメンバーでないと、
  見直しの通知もコメントの通知も届かない（送る時点で読めるかを見るため・#735）。
  **担当と自分はメンバーから外せない**
- **URL に使う名前（`key`）はあとから変えない。** 削除したスペースの名前も使い回さない（貼られたリンクが切れる・別のスペースを指す）
- **ページが1件でも残っているスペースは削除できない**（下書きを含む）
- 名前・説明の欄は `BufferedInput`（値を外へ渡すのは打つのが止まって 500ms 後）。**送るときは `FormData` で DOM の値を読む** —
  state を読むと、打ってすぐ Enter を押したときに最後の数文字が落ちる

**編集画面のツールバーの「AI で整える」は段E で置いた。** 設計書 §6-⑨ のスマホの
ツールバー5つめは、段B のあいだ「その他」（ブロックの一覧）を仮に置いていたが、
段E で「AI で整える」に差し替えた（その他のブロックは行頭 `/` から同じ一覧が開く）。
...
| `components/spaces/spaceAdminApi.ts` | スペース管理（一覧・追加・編集・削除・メンバー。manager） |
...
| `components/spaces/` | スペース管理のシート（追加・編集）とメンバーの欄 |
...
| `pages/` | 画面。`home/` `page/` `database/` `editor/` `history/` `templates/` `search/` `importexport/` `review/` `spaces/` |
