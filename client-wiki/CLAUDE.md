...
| 画面 | ルート | 中身 |
| --- | --- | --- |
| ホーム | `/` | 見直し期限切れの案内・スペースのタイル・最近更新・お気に入り |
| スペース | `/s/:key` | 左＝ツリー／中央＝直下のページの一覧 |
| ページ | `/p/:id` | 左＝ツリー／中央＝本文（Markdown）／右＝情報・目次・履歴 |
...
列は `hidden lg:flex` / `xl:flex` で出し分け、スマホではツリーを `Sheet` に入れ、
情報と目次を本文の下に積む。
...
ホームの最近更新・お気に入り・期限切れは別々の問い合わせで、失敗してもその区画の中だけで
受ける（1つの 404 でホーム全体が白紙にならない）。期限切れの案内は**0件・読み込み中・
...
| `lib/` | `wikiApi`（URL と react-query）`wikiFormat`（第N版・見直し期限）`wikiTree`（ツリーの組み立て）`lineDiff`（版の差分） |
