 ...
 *
 * `state` の5つ（未仕分け / あとで見る / タスク / 案件 / 見送り）は
 * DB の値なので変えていません。変えたのは**見せ方**で、
 * 「もう終わったもの」3つを1つのタブにまとめ、
 * 空いた場所を**「あとで見る」を机に戻す仕掛け**に使っています。
 *
 * ── モックとの違いは1つだけ ──────────────────────────────
 *
 * モックのタブは 未仕分け / あとで見る / タスク / 見送り の4つですが、
...
export function actionsFor(state: InquiryState): InquiryAction[] {
  ...
  // 「あとで見る」からは**見直す日を決め直せる**（migration 247）。
  // 「今日見たけれどまだ動けない」を受け止める先が無いと、
  // 机に出た「あとで見る」は毎日出続けるか、見送りにされるかのどちらかになる
 ...
 *
 * ⚠️ **`desk` と `stock` は重なります**（見直しの日が来た「あとで見る」は両方に出る）。
