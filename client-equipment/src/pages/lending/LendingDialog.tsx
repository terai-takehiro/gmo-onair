    ...
    <Dialog open={open} onOpenChange={(o) => { if (!o) { reset(); onClose(); } }}>
      <DialogContent className="flex max-h-[90vh] flex-col gap-0 p-0 sm:max-w-2xl">
        <DialogHeader className="shrink-0 border-b border-border px-5 pb-3 pt-5">
          <DialogTitle>{step === 'select' ? '持ち出す機材を選ぶ' : '借りる人と日付'}</DialogTitle>
        </DialogHeader>

        {error && (
          <p className="shrink-0 border-b border-destructive-border bg-destructive-surface px-5 py-2 text-sub text-destructive">
            {error}
          </p>
        )}
