/**
 * ラック図の印刷（v4 大④でファイルから切り出し・**中身は1行も変えていない**）
 *
 * ── ここは触らない ──────────────────────────────────────────
 *
 * 1U あたりの px（`PRINT_U_H`）と A4 縦に割り当てる高さ（`PRINT_RACK_BODY_BUDGET_PX`）は
 * **実機の印刷で合わせた値**です。少しでも大きくすると**1ページ目で切れます**。
 * 画面の作り直し（大④）では位置だけ動かし、この計算には手を入れていません。
 */
import { TYPE_BG } from "@/lib/constants";
import {
  PRINT_U_H, PRINT_U_H_MIN, PRINT_RACK_BODY_BUDGET_PX, PRINT_RACK_W,
  slotToColumn, type CellConfig, type RackConfig,
} from "./printConstants";
