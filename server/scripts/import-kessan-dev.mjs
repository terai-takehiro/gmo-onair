...
async function main() {
  ...
  try {
    ...
    // project_numbers に number の現役行が無ければ1行足す (kessan-import.service.ts の
    // ensureNumberHistory と同一ロジック)。ensureProject が案件を解決する3つの経路
    // (見つかった／削除済みから復活／新規作成) のどれでも、この関数が入る前に作られた
    // 案件は project_numbers に1行も残っていない可能性があり、放置すると後でこの案件が
    // 改番されたとき「退役させる現役の番号」が見つからず旧番号が完全に失われる
    // (Codex レビュー指摘)。既に (現役・退役いずれかの) 行がある number には触らない。
    async function ensureNumberHistory(projectId, number) {
      const existing = await client.query('SELECT 1 FROM project_numbers WHERE number=$1 LIMIT 1', [number]);
      if (existing.rows[0]) return;
      // scheme/entity_code は番号の見た目から判定する (kessan-import.service.ts と同じ)。
      ...
      await client.query(
        ...
        [randomUUID(), projectId, number, numberEntityCode, numberEntityCode ? 'entity' : 'gls', userId]
      );
    }
    async function ensureProject(key, name, customerId, isFixed = false) {
      ...
      if (cached) {
        // キャッシュ命中でもバックフィルは毎回試みる。事前照合レポートの findProject が
        // 先にキャッシュへ入れた「見つかった案件」はここで即returnしてしまうため、下の
        // 本体 (見つかった／復活／新規作成のどの経路でも通る箇所) まで一度も辿り着けない
        // (Codex レビュー指摘: この関数が入る前に作られた案件のバックフィルが効かない穴)。
