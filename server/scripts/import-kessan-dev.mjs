...
async function main() {
  ...
  try {
    ...
    // Phase 3-2a/3-2b: projects.customer_id / purchases.vendor_id は companies.id を
    // 直接指す。Phase 3-3-7〜9 で customers/vendors テーブル自体を削除したため、
    // company_id の解決は companies を直接引く（それ以前は customers/vendors 経由だった）
    ...
    // **`companies` に is_customer/is_vendor の行を作る**（`company-directory.service.ts`
    // の createCustomerRecord/createVendorRecord と同じ形。Phase 3-3-7〜9 で
    // customers/vendors テーブル自体が無くなったため companies 単独で完結する）
