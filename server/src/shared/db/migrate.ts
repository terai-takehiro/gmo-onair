...
export async function runMigrations(): Promise<void> {
   ...
   * migration 280 が必ず作る土台。無いと計上会社まわりの API が全部落ちるが、
  ...
  if ((legalEntitiesCount.rows[0]?.n ?? 0) !== 3) {
    console.error(
      [
        ...
        '  （migration 280 が未適用、または誰かが削除した環境）。',
        '  計上会社まわりの API・画面が動きません。',
        '  直したら手動で: migration 280_legal_entities.sql の内容を再実行してください。',
      ].join('\n')
    );
  }
  ...
  if ((orgTransitionCount.rows[0]?.n ?? 0) !== 1) {
    console.error(
      [
        '⚠️ [org_transition] \'default\' 行がありません（migration 280 が未適用の環境）。',
