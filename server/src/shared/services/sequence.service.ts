...
export async function generateGlsNumber(): Promise<string> {
  ...
  if (!row) {
    counter = 1;
    await execute('INSERT INTO sequences (seq_name, prefix, year_month, counter) VALUES (?, ?, ?, ?)', [seqName, 'GLS', '000000', counter]);
  } else {
    counter = (row.counter as number) + 1;
    await execute('UPDATE sequences SET counter = ? WHERE seq_name = ?', [counter, seqName]);
  }
