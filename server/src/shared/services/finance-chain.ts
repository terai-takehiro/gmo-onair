 ...
 * `shared/src/utils/financeDocChain.ts` を使うので、**同じ計算が2か所にあります**。
 *
 * **片方だけ直すと、画面が出す支払期日と台帳に入る支払期日が別の日になります。**
 * 同じであることは `shared/tests/financeDocChainParity.test.ts` が
 * 両方を import して突き合わせて固定しています（制作資料 AI の往復テストと同じ形）。
 */
