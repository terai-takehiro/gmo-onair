// shared/utils/secret-box.ts — 秘密文字列の暗号化・復号・マスク（AES-256-GCM）
//
// 元は `server/src/contexts/liveops/crypto.ts` に置かれていた（`liveops_settings` の
// API キー・トークン用）。08（収録設定・配信設定）がストリームキーの暗号化に
// **同じ仕組み**を使うにあたり、qsheet コンテキストから liveops コンテキストを
// import すると「コンテキストをまたぐ」ため、ここ（shared/utils）へ引き上げた。
//
// ⚠️ **鍵の導き方・暗号文の形式（`iv:authTag:ciphertext`、全部 hex）を1文字も変えていない。**
// 変えると `liveops_settings` に既に入っている暗号文が読めなくなる（`decrypt` は
// 失敗すると `null` を返すため、壊れても例外にならず**黙って壊れる**）。
//
// `server/src/contexts/liveops/crypto.ts` は中身をここへの再輸出だけにしてある。
