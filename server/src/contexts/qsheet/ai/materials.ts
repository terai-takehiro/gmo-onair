...
export interface GenerationMaterials {
  ...
  /** `getFeedbackDigest(kind).advice` の先頭6〜8件（04-ai.md §2-1） */
  advice: string[];
...
async function fetchAdvice(kind: string): Promise<string[]> {
  try {
...
export async function gatherMaterialsForDocument(
  ...
  const [project, episode, advice, similar] = await Promise.all([
    fetchProject(projectId),
    fetchEpisode(episodeId),
    fetchAdvice(opts.kind),
    findSimilarDocs({
  ...
  return {
    project, episode, venue: null, scheduleItems: [], advice,
    segmentKey: buildSegmentKey(project?.projectCategory ?? null, null),
    similar,
  };
}
...
export async function gatherMaterialsForSchedule(
  ...
  const [project, episode, venue, scheduleItems, advice] = await Promise.all([
    fetchProject(projectId), fetchEpisode(episodeId), fetchVenue(locationId),
    fetchScheduleItems(scheduleId), fetchAdvice(opts.kind),
  ]);
  return {
    project, episode, venue, scheduleItems, advice,
    segmentKey: buildSegmentKey(project?.projectCategory ?? null, locationId),
    similar: [],
  };
}
