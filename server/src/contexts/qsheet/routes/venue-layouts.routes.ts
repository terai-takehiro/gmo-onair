...
router.post('/venue-layouts', requirePermission('qsheet', 'editor'), wrap(async (req, res) => {
  ...
  const copyFromLayoutId = typeof b.copy_from === 'string' ? b.copy_from : null;
  // レビュー指摘（P1・IDOR）: `copy_from` は他人の図面IDを差し込んでも `items` を丸ごと
  // 読み出せてしまっていた（`/copy` エンドポイントは requireAccessible を通るが、
  // 作成エンドポイントの `copy_from` はここでしか検査できない）。`/copy` と同じ
  // `canAccessVenueLayout` で複製元への可視性を確認してから渡す（存在秘匿のため404）。
  if (copyFromLayoutId) {
    const source = await getVenueLayoutRaw(copyFromLayoutId);
    if (!source) throw new NotFoundError('複製元の図面が見つかりません');
    const canReadSource = await canAccessVenueLayout(req.user!, source.id as string, (source.created_by as string) ?? null);
    if (!canReadSource) throw new NotFoundError('複製元の図面が見つかりません');
  }
...
router.get('/venue-layouts/:id', wrap(async (req: Request, res: Response) => {
  ...
  res.json({ success: true, data: mapDetailRow(row) });
}));
