...
router.get('/events', (req, res) => {
  ...
  const projects = queryAll(`SELECT p.id, p.gls_number, p.name, p.event_start, p.event_end, p.stage, c.name as customer_name FROM projects p LEFT JOIN customers c ON c.id = p.customer_id ${where}`, params);
  const events: any[] = [];
