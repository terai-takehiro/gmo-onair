...
router.post('/', requireAuth, async (req, res) => {
  ...
  const id = uuidv4();
  await execute('INSERT INTO vendors (id, name, address, vendor_type, invoice_registration_number, notes, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
router.put('/:id', requireAuth, async (req, res) => {
  ...
  const { name, address, vendor_type, invoice_registration_number, notes } = req.body;
  await execute(`UPDATE vendors SET name=?, address=?, vendor_type=?, invoice_registration_number=?, notes=?, updated_at=NOW(), updated_by=? WHERE id=?`,
