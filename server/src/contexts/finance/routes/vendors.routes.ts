...
router.get('/', (req, res) => {
  ...
  if (search) { where += ` AND (name LIKE ? OR vendor_type LIKE ?)`; params.push(`%${search}%`, `%${search}%`); }
  const total = (queryOne(`SELECT COUNT(*) as c FROM vendors ${where}`, params) as any).c;
  const rows = queryAll(`SELECT * FROM vendors ${where} ORDER BY name LIMIT ? OFFSET ?`, [...params, limit, offset]);
  res.json(paginatedResponse(rows, total, page, limit));
});

router.get('/:id', (req, res) => {
  const row = queryOne('SELECT * FROM vendors WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
...
router.delete('/:id', requireAuth, (req, res) => {
  execute(`UPDATE vendors SET deleted_at=datetime('now'), updated_by=? WHERE id=? AND deleted_at IS NULL`, [req.user!.id, req.params.id]);
