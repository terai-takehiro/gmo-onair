...
router.get('/', async (req, res) => {
  ...
  if (search) { where += ` AND (pu.description ILIKE ? OR v.name ILIKE ?)`; params.push(`%${search}%`, `%${search}%`); }
  if (projectId) { where += ` AND pu.project_id = ?`; params.push(projectId); }
  if (episodeId) { where += ' AND pu.episode_id = ?'; params.push(episodeId); }
  const total = ((await queryOne(`SELECT COUNT(*) as c FROM purchases pu LEFT JOIN vendors v ON v.id = pu.vendor_id ${where}`, params)) as any).c;
