...
// （`customers.routes.ts` と同じ設計）。Phase 3-3-7〜9 で `customers` テーブルを
// 削除したため、基本情報（名前・連絡先）の読み書きも `companies` 単独で完結する。
...
export function registerCustomerTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    async (args) => runTool(async () => {
      ...
      // （company-directory.service.ts）。グループの印は社名から見立てる
      // （migration 192）。AI は印を渡さないので、ここで入れないと AI が
      // 登録した会社だけグループ外のまま残る。
      //
  ...
  server.registerTool(
    ...
    async (args) => runTool(async () => {
      // args.id は companies.id（Phase 3-2a）。Phase 3-3-7〜9 で `customers`
      // テーブルを削除したため、以降は `companies` を直接読み書きする
