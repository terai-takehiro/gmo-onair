...
// 社内取引（SCS⇄GSS） — 2026年10月の事業再編・P2 Round 2（docs/reorg-2026-10-plan.md §4.12）。
...
export function registerIntercompanyTools(server: McpServer): void {
  server.registerTool(
    ...
    {
      title: '社内取引の作成（GSS→SCSの社内発注）',
      description:
        'SCS の案件が GSS のスタジオ・人員・機材を使うときの社内取引を記録する（重要操作）。' +
      ...
      inputSchema: {
        project_id: z.string().min(1).describe('買い手（SCS）の案件 ID'),
    ...
    async (args) => runTool(async () => {
      if (!args.confirm) {
        ...
        return preview(
          `GSS → SCS の社内取引（${args.amount.toLocaleString()}円）を作る`,
          [
            'GSS の社内売上（revenues）と SCS の社内仕入（purchases）が1本ずつ作られる（案件は増えない）',
            '両方の相手先は自社行（GSS の売上先＝SCS の自社行／SCS の仕入先＝GSS の自社行）',
