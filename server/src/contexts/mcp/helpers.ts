 ...
 *
 * **推測で書かせない。** ここは AI が自由記述で書ける欄なので、メール本文などから
 * 漢字を推測すると誤った人名が記録として残る。実際に「寺井 赳博」さんが
 * **「寺井武大」** として記録され、画面に「指示: 寺井武大」と出ていた (v3.0.6 で
 * 画面表示をやめ、AI がやったことだけを出す形にした)。
 * 記録そのものは監査のため残すが、**確認できない名前は入れないほうが良い**。
 */
export const REQUESTED_BY = {
  requested_by: z.string().max(100).optional()
    .describe(
      '監査ログ用: この操作を依頼した人。**推測で書かないこと。**'
      + ' list_users で実在を確認できたときだけ、名簿どおりの表記で渡す。'
      + ' メール本文や会話から読み取った漢字は誤りやすいので (「赳博」を「武大」と'
      + ' 書いた事故がある)、確認できないときは省略する。'
      + ' なお画面にはこの値を人名として出さず、AI が実行したことだけを表示する。',
    ),
};
