...
export async function getAppBadges(access: Access): Promise<AppBadges> {
  ...
  const row = (await queryOne(
    `SELECT
       ...
       ${wantStudio ? `(SELECT COUNT(*) FROM studio_bookings b
           WHERE b.deleted_at IS NULL AND b.booking_type = 'hold'
             AND substr(b.start_time, 1, 10) >= to_char(NOW(), 'YYYY-MM-DD'))` : 'NULL'} AS studio,
  ...
  if (wantStudio) out.studio = Number(row?.studio ?? 0);
