...
export const INTEGRITY_CHECKS: IntegrityCheck[] = [
  ...
  {
    key: 'won_no_gls',
    label: '受注しているのに管理番号が無い',
    why: '受注に上げると管理番号は自動で採るはずですが、'
       + '案件分類（スタジオ / ビジネス）が未設定だと自動発番だけ失敗し、'
       + '受注そのものは通ってしまいます。仕入・売上などの実務入力'
       + '（案件プルダウン）は受注済みかどうかで判定するので支障はありませんが、'
       + '「回の追加」「既存GLSへの付け替え」「費用を分け合うグループ」は'
       + '管理番号そのものが無いと選べません。',
    how: 'まず案件編集で案件分類を選んでから、案件詳細を開いて「GLS を発番」。'
       + 'まとめては直せません — 番号は1件ずつ採るものだからです。',
  ...
  },
  {
    ...
    why: '標準工程の期限は実施日からの逆算で出すので、空だと逆算の工程が'
       ...
       + '（レビューでの指摘）— そもそも「実施日」という概念を持たず、'
       + '代わりに開始日・終了日（`started_on`/`ends_on`）を持ちます。'
       + 'この2つを `event_start`/`event_end` の代わりに数えると、GLS-B の'
       + '案件（`GMO-0001`「GMO第3本社プロジェクト」等）が実施日の欄を'
       + '持ったことが一度も無いのに毎回「無い」と数えられ続けます。',
    how: '選んで「まとめて直す → 実施日」。'
