...
router.post('/', requireAuth, async (req, res) => {
  ...
  const id = uuidv4();
  await execute('INSERT INTO customers (id, name, short_name, notes, created_by) VALUES (?, ?, ?, ?, ?)',
...
router.put('/:id', requireAuth, async (req, res) => {
  ...
  const { name, short_name, notes } = req.body;
  await execute(`UPDATE customers SET name=?, short_name=?, notes=?, updated_at=NOW(), updated_by=? WHERE id=?`,
