...
/**
 * Phase 3-3-9（`customers` テーブル削除）以降、この画面（案件作成の「お客様」
 * ドロップダウン兼顧客一覧）は `companies`（`is_customer = TRUE`）だけを正として
 * 読み書きする。`customers` テーブル自体・legacy id（移行前の `customers.id`）の
 * 解決（旧 `resolveLegacyCustomerId`/`findCustomerRow` の legacy 分岐）は
 * 削除した — 互換確認期間（`docs/reviews/phase3-2-plan.md`）中、legacy id 経由の
 * アクセスは観測期間中0件だったことを確認済み。
 */
...
/** `companies.id`（正）で引く。`is_customer = TRUE` かつ生きている行だけ返す。 */
...
router.get('/:id/overview', async (req, res) => {
  ...
  const id = customer.id as string; // companies.id
...
router.post('/', requirePermission('sales', 'owner'), async (req, res) => {
  ...
  // チェックボックスで外せます。`createCustomerRecord` は companies.id を返す
  // （Phase 3-3-9 より前は customers.id を返し、company_id を引き直していた）。
...
router.put('/:id', requirePermission('sales', 'owner'), async (req, res) => {
  // :id は companies.id。Phase 3-3-9（`customers` テーブル削除）以降、
  // このテーブルへの読み書きは一切ない — companies を直接引いて直接更新する。
  const existing = await queryOne(
    'SELECT id, is_gmo_group FROM companies WHERE id = ? AND is_customer = TRUE AND deleted_at IS NULL',
    [req.params.id],
...
router.delete('/:id', requirePermission('sales', 'manager'), async (req, res) => {
  // :id は companies.id。`customers` テーブルが無くなったので、この画面（顧客一覧）
  // の削除は `companies.is_customer` を FALSE にするだけでよい（companies 行自体・
  // 仕入先ロールは触らない）。旧実装と同じく、対象が既に無い/削除済みでもエラーには
  // しない（`execute` は影響行数を返さないため、以前から存在チェックはしていない）。
  await execute(
    `UPDATE companies SET is_customer = FALSE, updated_at = NOW(), updated_by = ?
     WHERE id = ? AND is_customer = TRUE AND deleted_at IS NULL`,
