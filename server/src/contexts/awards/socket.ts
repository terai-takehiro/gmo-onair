...
// v2.9.2: 旧 Standalone Poll は廃止、新「アンケート/クイズCG」モジュール (/quiz namespace) に移管。
...
export function initAwardsSocketIO(io: Server): void {
  ...
  awardsNs.on('connection', (socket: Socket) => {
    // v2.9.2: 旧 ?pollRoom=xxx (Standalone Poll) ハンドラは廃止、新「アンケート/クイズCG」モジュール (/quiz namespace) に移管。
