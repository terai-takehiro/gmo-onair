// 機材コネクタ管理 CRUD + Excel I/O + Inline PATCH
...
const SELECT_SQL = `
  SELECT c.id, c.kind, c.location_id, c.name, c.manufacturer_id, c.model_number,
         c.quantity, c.storage_method, c.notes, c.sort_order,
         c.created_at, c.updated_at,
         loc.name AS location_name,
         mfr.name AS manufacturer_name
  FROM equipment_connectors c
...
const EXCEL_COLUMNS = [
  ...
  { key: 'quantity',           header: '所有個数', width: 8 },
...
router.post('/', requirePermission('equipment', 'editor'), wrap(async (req, res) => {
  ...
  const {
    ...
    quantity, storage_method, notes, sort_order,
  } = req.body;
  const id = uuid();
  await execute(
    `INSERT INTO equipment_connectors
...
router.put('/:id', requirePermission('equipment', 'editor'), wrap(async (req, res) => {
  ...
  await execute(
    `UPDATE equipment_connectors
     SET kind=$1, location_id=$2, name=$3, manufacturer_id=$4, model_number=$5,
         quantity=$6, storage_method=$7, notes=$8, sort_order=$9, updated_at=NOW()
...
const PATCHABLE_FIELDS = new Set([
  'name', 'model_number', 'storage_method', 'notes',
  'quantity', 'kind', 'location_id', 'manufacturer_id',
]);
router.patch('/:id', requirePermission('equipment', 'editor'), wrap(async (req, res) => {
  ...
  for (const [k, v] of Object.entries(req.body || {})) {
    ...
    let val: unknown = v;
    if (k === 'quantity') val = toNumOrNull(v);
  ...
  await execute(
    `UPDATE equipment_connectors SET ${setSql}, updated_at=NOW() WHERE id=$${updates.length + 1} AND deleted_at IS NULL`,
    params,
  );
  const row = await queryOne(`${SELECT_SQL} AND c.id = $1`, [req.params.id]);
  res.json({ success: true, data: row });
...
router.delete('/:id', requirePermission('equipment', 'manager'), wrap(async (req, res) => {
  await execute('UPDATE equipment_connectors SET deleted_at = NOW() WHERE id = $1', [req.params.id]);
  ...
  const buf = buildExcelWorkbook([
    {
      ...
      rows: [
        { kind_label: '映像', location_name: 'サブ1', name: 'BNCコネクタ オス', manufacturer_name: 'CANARE', model_number: 'BCP-B25HD', quantity: 50, storage_method: 'パーツケース B-3', notes: '' },
        { kind_label: '音声', location_name: 'サブ1', name: 'XLR3 メス', manufacturer_name: 'NEUTRIK', model_number: 'NC3FXX', quantity: 30, storage_method: '', notes: '' },
      ],
    },
    {
      name: '記入要領',
      ...
      rows: [
        ...
        { col: '型名', desc: '型番・モデル名' },
        { col: '所有個数', desc: '整数。例: 50' },
        { col: '収納方法', desc: '保管箱・棚の名称など自由記述' },
        { col: '備考', desc: '自由記述' },
      ],
    },
  ]);
  excelResponse(res, 'コネクタ_テンプレート.xlsx', buf);
}));
...
router.get('/export-xlsx', requirePermission('equipment', 'exporter'), wrap(async (_req, res) => {
  ...
  const buf = buildExcelWorkbook([{ name: 'コネクタ', columns: EXCEL_COLUMNS, rows }]);
