...
router.get('/', (req, res) => {
  ...
  const rows = queryAll(`SELECT p.*, c.name as customer_name FROM projects p LEFT JOIN customers c ON c.id = p.customer_id ${where} ORDER BY COALESCE(p.event_start, p.created_at) DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
...
router.post('/', requireAuth, (req, res) => {
  const { name, customer_id, opportunity_id, rehearsal_start, rehearsal_end, event_start, event_end, status, notes } = req.body;
  ...
  execute(`INSERT INTO projects (id, gls_number, name, customer_id, opportunity_id, rehearsal_start, rehearsal_end, event_start, event_end, status, notes, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [id, glsNumber, name, customer_id, opportunity_id || null, rehearsal_start || null, rehearsal_end || null, event_start || null, event_end || null, status || 'tentative', notes || null, req.user!.id]);
...
router.put('/:id', requireAuth, (req, res) => {
  ...
  const { name, customer_id, rehearsal_start, rehearsal_end, event_start, event_end, status, application_form, logo_permission, notes } = req.body;
  execute(`UPDATE projects SET name=?, customer_id=?, rehearsal_start=?, rehearsal_end=?, event_start=?, event_end=?, status=?, application_form=?, logo_permission=?, notes=?, updated_at=datetime('now'), updated_by=? WHERE id=?`,
    [name, customer_id, rehearsal_start || null, rehearsal_end || null, event_start || null, event_end || null, status, application_form ? 1 : 0, logo_permission ? 1 : 0, notes || null, req.user!.id, req.params.id]);
