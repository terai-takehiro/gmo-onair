...
router.post('/categories', requireAuth, (req, res) => {
  ...
  const row = queryOne('SELECT * FROM pricing_categories WHERE id = ?', [id]);
  res.status(201).json({ success: true, data: row });
});

// PUT /pricing/categories/:id - Update category
router.put('/categories/:id', requireAuth, (req, res) => {
  const existing = queryOne('SELECT id FROM pricing_categories WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
  if (!existing) throw new AppError(404, 'NOT_FOUND', 'カテゴリが見つかりません');
  const { name, sort_order } = req.body;
  execute(
    `UPDATE pricing_categories SET name=?, sort_order=?, updated_at=datetime('now'), updated_by=? WHERE id=?`,
    [name, sort_order ?? 0, req.user!.id, req.params.id]
  );
  const row = queryOne('SELECT * FROM pricing_categories WHERE id = ?', [req.params.id]);
...
router.delete('/categories/:id', requireAuth, (req, res) => {
  execute(
    `UPDATE pricing_categories SET deleted_at=datetime('now'), updated_by=? WHERE id=? AND deleted_at IS NULL`,
...
router.post('/items', requireAuth, (req, res) => {
  ...
  const category = queryOne('SELECT id FROM pricing_categories WHERE id = ? AND deleted_at IS NULL', [category_id]);
  ...
  const row = queryOne('SELECT * FROM pricing_items WHERE id = ?', [id]);
  res.status(201).json({ success: true, data: row });
});

// PUT /pricing/items/:id - Update item
router.put('/items/:id', requireAuth, (req, res) => {
  const existing = queryOne('SELECT id FROM pricing_items WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
  if (!existing) throw new AppError(404, 'NOT_FOUND', '料金項目が見つかりません');
  const { name, sub_label, unit_price, calc_type, sort_order } = req.body;
  execute(
    `UPDATE pricing_items SET name=?, sub_label=?, unit_price=?, calc_type=?, sort_order=?, updated_at=datetime('now'), updated_by=? WHERE id=?`,
    [name, sub_label || null, unit_price, calc_type, sort_order ?? 0, req.user!.id, req.params.id]
  );
  const row = queryOne('SELECT * FROM pricing_items WHERE id = ?', [req.params.id]);
  res.json({ success: true, data: row });
});
...
router.delete('/items/:id', requireAuth, (req, res) => {
  execute(
    `UPDATE pricing_items SET deleted_at=datetime('now'), updated_by=? WHERE id=? AND deleted_at IS NULL`,
    [req.user!.id, req.params.id]
  );
  res.json({ success: true, message: '削除しました' });
});
