...
router.get('/', (req, res) => {
  ...
  const rows = queryAll(`SELECT pu.*, p.name as project_name, p.gls_number, v.name as vendor_name FROM purchases pu LEFT JOIN projects p ON p.id = pu.project_id LEFT JOIN vendors v ON v.id = pu.vendor_id ${where} ORDER BY pu.recognition_date DESC, pu.created_at DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
  res.json(paginatedResponse(rows, total, page, limit));
});

router.get('/:id', (req, res) => {
  const row = queryOne(`SELECT pu.*, p.name as project_name, p.gls_number, v.name as vendor_name FROM purchases pu LEFT JOIN projects p ON p.id = pu.project_id LEFT JOIN vendors v ON v.id = pu.vendor_id WHERE pu.id = ? AND pu.deleted_at IS NULL`, [req.params.id]);
  if (!row) throw new AppError(404, 'NOT_FOUND', '仕入が見つかりません');
  res.json({ success: true, data: row });
});

router.post('/', requireAuth, (req, res) => {
  const { project_id, episode_id, vendor_id, settlement_method, settlement_number, tax_category, invoice_qualified, amount, description, recognition_date, inspection_date, payment_due_date, notes } = req.body;
  if (!project_id || !vendor_id) throw new AppError(400, 'VALIDATION_ERROR', '案件と仕入先は必須です');
  const id = uuidv4();
  execute(`INSERT INTO purchases (id, project_id, episode_id, vendor_id, assigned_to, settlement_method, settlement_number, tax_category, invoice_qualified, amount, description, recognition_date, inspection_date, payment_due_date, notes, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [id, project_id, episode_id || null, vendor_id, req.user!.id, settlement_method || null, settlement_number || null, tax_category || 'tax10', invoice_qualified !== undefined ? (invoice_qualified ? 1 : 0) : 1, amount || 0, description || null, recognition_date || null, inspection_date || null, payment_due_date || null, notes || null, req.user!.id]);
