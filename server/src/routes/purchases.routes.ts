...
router.post('/', requireAuth, (req, res) => {
  ...
  const id = uuidv4();
  execute(`INSERT INTO purchases (id, project_id, vendor_id, assigned_to, settlement_method, settlement_number, tax_category, invoice_qualified, amount, description, recognition_date, inspection_date, payment_due_date, notes, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [id, project_id, vendor_id, req.user!.id, settlement_method || null, settlement_number || null, tax_category || 'tax10', invoice_qualified !== undefined ? (invoice_qualified ? 1 : 0) : 1, amount || 0, description || null, recognition_date || null, inspection_date || null, payment_due_date || null, notes || null, req.user!.id]);
