...
export async function seed() {
  ...
  const projectData: [string, string, string, string | null, string | null, string | null, string | null, string, string, string][] = [
    ['GLS001', 'GMO IR説明会 2026春', 'GMO-IG', '2026-03-18', '2026-03-18', '2026-03-19', '2026-03-19', 'completed', 'live', 'youtube'],
    ['GLS002', 'テレ東 特番収録「未来の技術」', 'テレ東', '2026-03-20', '2026-03-20', '2026-03-22', '2026-03-22', 'confirmed', 'recording', 'terrestrial_tv'],
    ['GLS003', 'ABEMA 生放送「Tech Night」', 'ABEMA', null, null, '2026-03-25', '2026-03-25', 'confirmed', 'live', 'net_media'],
    ['GLS004', 'GMO-PG 新サービス発表会', 'GMO-PG', '2026-03-28', '2026-03-28', '2026-03-30', '2026-03-30', 'tentative', 'live', 'zoom'],
    ['GLS005', 'Netflix ドキュメンタリー撮影', 'Netflix', '2026-04-02', '2026-04-03', '2026-04-05', '2026-04-07', 'confirmed', 'recording', 'net_media'],
    ['GLS006', 'CA 社内イベント中継', 'CA', null, null, '2026-04-10', '2026-04-10', 'tentative', 'live', 'teams'],
    ['GLS007', 'TBS 番組パイロット撮影', 'TBS', '2026-04-12', '2026-04-12', '2026-04-14', '2026-04-15', 'tentative', 'recording', 'terrestrial_tv'],
    ['GLS008', 'フジ CM撮影「春キャンペーン」', 'フジ', '2026-03-10', '2026-03-10', '2026-03-12', '2026-03-12', 'completed', 'recording', 'terrestrial_tv'],
  ];
  ...
  const oppData: [string, string, string, string, string, number, number, string, string | null][] = [
    ...
    ['OPP-202603-0010', 'GMO IR説明会 2026春', 'GMO-IG', 'hybrid_event', 'a_won', 100, 4000000, '2026-03-19', 'GLS001'],
    ['OPP-202603-0011', 'テレ東 特番収録「未来の技術」', 'テレ東', 'recording', 'a_won', 100, 7500000, '2026-03-22', 'GLS002'],
    ['OPP-202603-0012', 'ABEMA 生放送「Tech Night」', 'ABEMA', 'live_broadcast', 'a_won', 100, 5500000, '2026-03-25', 'GLS003'],
  ...
  const revenueData: [string, string, string, number, string, string][] = [
    ['GLS001', 'GMO-IG', 'tax10', 4000000, '2026-03-31', 'スタジオ利用+技術費'],
    ['GLS002', 'テレ東', 'tax10', 5000000, '2026-03-31', 'スタジオ利用料'],
    ['GLS002', 'テレ東', 'tax10', 2500000, '2026-03-31', '技術スタッフ費'],
    ['GLS003', 'ABEMA', 'tax10', 3500000, '2026-03-31', 'スタジオ利用+配信技術'],
    ['GLS003', 'ABEMA', 'tax10', 2000000, '2026-03-31', '追加機材レンタル'],
    ['GLS004', 'GMO-PG', 'tax10', 2800000, '2026-04-30', 'スタジオ利用料（仮）'],
    ['GLS005', 'Netflix', 'tax10', 8000000, '2026-04-30', '撮影スタジオ3日間'],
    ['GLS005', 'Netflix', 'tax10', 4000000, '2026-04-30', '技術スタッフ+機材'],
    ['GLS006', 'CA', 'tax10', 1500000, '2026-04-30', 'スタジオ利用料'],
    ['GLS007', 'TBS', 'tax10', 3000000, '2026-04-30', 'パイロット撮影費'],
    ['GLS007', 'TBS', 'tax10', 1500000, '2026-04-30', '編集室利用料'],
    ['GLS008', 'フジ', 'tax10', 3200000, '2026-03-31', 'CM撮影一式'],
  ];
  ...
  // Episodes - テレ東特番(GLS002): 12話一括 + 1話追加
  ...
  const teltoProject = PROJECTS['GLS002'];
  for (let i = 1; i <= 13; i++) {
    const epId = uuidv4();
  ...
  // Episodes - ABEMA生放送(GLS003): 4話(生放送なので収録日=放送日)
  const abemaProject = PROJECTS['GLS003'];
  for (let i = 1; i <= 4; i++) {
    const epId = uuidv4();
  ...
  // Netflix(GLS005): 6話
  const netflixProject = PROJECTS['GLS005'];
  for (let i = 1; i <= 6; i++) {
    const epId = uuidv4();
