...
### Phase B — 決めてから直すもの（ユーザー判断が要る）**✅ 実装済み（2026-08-27）**

| 論点 | 選択肢 | 決定 |
|---|---|---|
| `logo_permission` | (a) 請求発行の gate に加える / (b) スイッチごと消す | **(b) 削除**。migration 243 |
| `tags` | (a) 廃止 / (b) 正式機能に昇格 | **(a) 廃止**。台帳の一括編集・MCP・Excel取込出力・バックアップ出力から全撤去。migration 243 |
| `lessons_learned` | (a) 列ごと廃止 / (b) 失注ダイアログに欄を戻す | **(a) 廃止**。営業レビューの「教訓・学び」パネルごと削除。migration 243 |
| `assigned_to` | 方針文書を実態に合わせて書き直す | [client/CLAUDE.md](../client/CLAUDE.md) の「v4 の設計判断」を書き直し済み（旧「案件担当者という概念を持たない」→「主担当を持つ。実務の割り当てはタスク単位」） |

`lost_reason_note`（表示ゼロのまま集め続けるか・失注パネルに出すか）は今回の指示に
含まれなかったため**未着手のまま残す**。

**実装で判明した追加の掃除**: 棚卸し時点では見えていなかった書き手・読み手が実装時に見つかった。
`tags` は Excel 取込（`excel.routes.ts` のインポート/エクスポートテンプレート）・バックアップ
xlsx 出力（`backup.routes.ts`）・制作技術支援 AI の生成コンテキスト（`qsheet/ai/materials.ts`、
取得するが未使用のフィールドだった）にも及んでいた。すべて同じ棚卸しの延長として削除した。

### Phase C — 持ち場所の一本化（設計変更・別計画で詳細化）**🔄 詳細設計・一部実装中（2026-08-27〜）**
...
**詳細設計・実装状況は [docs/project-ledger-phase-c-design.md](project-ledger-phase-c-design.md)。**
4テーマをマルチエージェントで深掘り調査した結果、金額の集計統一（1）と実施日の DATE 型化（3）は
**本番データの事前監査が必須**（このセッションからは本番DBへアクセスできず監査不可）と判明し、
それ以外の安全な項目（受注/失注日時の集約・書く口の一部集約・関連する独立バグの修正）から
段階的に実装を進めている。
