...
**入手元**: Box folder `381450796408` (寺井さん共有)
**ファイル**: `損益計算書_残高試算表_20260507_1651.csv` (9.4KB) + `総勘定元帳_20260507_1652.csv` (189KB)
**取得経路**: Box MCP `get_file_content` で UTF-8 デコード後を VM 内 `docs/accounting-import/samples/` に保管（**`.gitignore` 追加済 — 機微情報のため git に push しない**）

### 10.1 損益計算書 (P/L) 観察

- 行数: 158 行（要件書記載と一致）
- 階層判定パターン（要件書 §2.1.2 通り）が実データで成立:
  - レベル 0 (大分類見出し): `"売上高"` / `"売上原価"` / `"販売費及び一般管理費"` / `"営業外収益"` / `"営業外費用"` / `"特別利益"` / `"特別損失"` / `"当期純損益"`
  - レベル 3 (集計行): `"売上高合計"` / `"売上原価合計"` / `"売上総損失"` / `"営業損失"` / `"経常損失"` / `"税引前当期純損失"` / `"当期純損失"`
  - レベル 1/2: `\d{4}` コード + 名前
- 補助科目の二段表記実例: `"8000 ｸﾞﾙｰﾌﾟ業務委託 :100 GP"` — 要件書 §2.1.3 の正規表現 `^(\d+)\s+(.+?)(?:\s*:\s*(\d+)\s+(.+))?$` で正しくパース可
- マイナス値の実例: `"7111 賞与" = -24792` (賞与引当金取崩のため正常マイナス)
- 構成比 `184.9%` のような 100% 超過値あり (売上原価 vs 売上高)
- ヘッダ動的部分: `"開始月:2026-03-01"` / `"終了月:2026-03-31"` → ここから period_ym=`2026-03` を抽出

### 10.2 総勘定元帳 観察

**行数**: 1,199 行（ヘッダ 1 + データ 1,198）

**ヘッダ構成 (18 列、要件書 §2.2.1 と完全一致)**:
```
...
**取引先列 (5列目)**: 実データでも**全行空文字**。取引先名は **相手取引先 (10列目)** に入る。要件書 §2.2.1 記述通り。

**GLS 番号 (摘要欄から抽出、15 件)**:
```
GLS098, GLS127, GLS128, GLS140, GLS143, GLS146, GLS148, GLS149,
GLS150, GLS151, GLS152, GLS153, GLS154, GLS155, GLS161
```
→ **要件書 §10 Definition of Done で言及されている GLS127 / GLS140 / GLS148 / GLS150 / GLS151 はすべて実在を確認**。Phase 5 検証時にこれらの GLS が `projects.gls_number` に登録済みかどうかを事前確認する必要あり。

**XP 番号 (摘要欄から抽出、115+ 件)**:
- `XP106213` 〜 `XP164955` までの 6 桁 ID
- 正規表現 `XP\d{6,}` で全件抽出可能
- 既存 `purchases.settlement_method='xpoint'` + `settlement_number` との完全一致 JOIN で突合できる

**税区分 (実データで出現する 6 種、要件書記載と一致)**:
- `対象外`
- `共-課仕 10%` / `共-課仕 (軽)8%`
- `課仕 10%` / `課仕 (軽)8%`
- `課売 10%`

**インボイス区分**: `80%控除` のみ（要件書記載通り）

**タグの揺れ (10 種)**:
- 整った: `1010 インフラ関連IT/GMO-IG`, `1020 インフラ関連IT/GMO-IG`, `1110 業務関連IT/GMO-IGグル`, `1210 情報関連IT/GMO-IGグル`, `1220 情報関連IT/GMO-IGグル`
- 不完全: `1220` (コードのみ), `9999` (コードのみ)
- 未分類: `9999 該当なし`, `9000 不明`
- 空文字: `""`

→ タグパースは `^(\d{4})(\s+(.+))?$` で寛容に。コードのみの行も保持できる構造に。

**摘要欄の追加パターン (要件書 §2.2.3 補強)**:
- `ｸﾚｶ決済/ANTHROPIC: CLAUDE TEAM` / `ｸﾚｶ決済/SP KITBASH3D` 等 — `ｸﾚｶ決済/` 以降がベンダー名（半角カナ + 英字混在）
- `支払/昭和ﾘｰｽ` / `支払/ｾｺﾑ` — `支払/` 以降が振込先（半角カナ）
- `26/03社員経費支払11697` — `\d{2}/\d{2}社員経費支払\d+` で社員番号埋め込み（11697 = 社員 ID）
- `東京都/都道府県民税/均等割` — `都道府県名/税目/区分` の階層
- `事業所税納付　加算金` — 全角スペース込み

→ 摘要パーサは **GLS / XP** の 2 種を必須として抽出。それ以外のパターン（ベンダー名等）は v1 では正規化せず生文字列のまま保持し、将来の AI 抽出フェーズで補強する方針が安全。

**勘定科目コード**: 4 桁数字、90 種類が実データで出現
- 主要なグループ: 1101 普通預金 / 30xx 未払金等 / 50xx 売上高 / 64-65xx 売上原価 / 71-75xx 販管費 / 82xx 営業外収益 / 84xx 営業外費用 / 98xx 法人税

### 10.3 重要な技術的注意点

**MCP `get_file_content` は UTF-8 デコード済みコンテンツを返している**:
- 実 Box ファイルは CP932 だが、Phase 0 分析用に取得したのは markdown 抽出版（UTF-8）
- **Phase 1 パーサのユニットテストでは CP932 バイト列で検証する必要**がある
- Phase 1 着手時の対応:
  1. Box SDK の `getReadStream(fileId)` 等で生バイト列を取得する経路を別途実装
  2. または寺井さんから直接 dev 環境にアップロードしてもらい、サーバーに到達したバイト列でテスト

### 10.4 Phase 5 検証フェーズで事前確認すべきこと

- [ ] 本番 / dev DB に `projects.gls_number` として GLS127 / GLS140 / GLS148 / GLS150 / GLS151 が登録されているか
- [ ] XP 番号 115+ 件のうち、`purchases.settlement_number` と一致するものが何件あるか（突合精度の事前把握）
- [ ] 元帳に登場する社員 ID（`26/03社員経費支払11697` 等）が `users` テーブルとどう繋がるか（または無関係か）— 立替経費の処理範囲を決めるため
...
## 付録: Phase 0 レビュー後の決定事項 (2026-05-12 追記)

**A 案で確定**: 経理 CSV 由来データは ETL 解決後、既存 `revenues` / `purchases` / `sga_expenses` に `source='accounting'` で INSERT（`revenues` と `purchases` にも `source` 列を新設）。生データは別表 `accounting_pl_lines_raw` / `accounting_ledger_entries_raw` にアーカイブ保持。既存 BudgetDashboardPage / monthly-summary API は `source` フィルタを追加するだけで予算 vs 実績の切替に対応。

**その他確定事項**:
- **権限モジュール**: 既存 `budget` を拡張（新規 `accounting` モジュールは作らない）
- **CSV 原本保管**: DB BYTEA + Box ミラーのハイブリッド
- **経理由来ベンダー**: `vendors` + `companies` 両方に登録（migration 061 移行方針と整合）

**実 CSV 入手元**: Box folder `381450796408` (`get_file_content` 経由で取得済、Phase 0 §10 に観察結果を反映)
