...
**`status` は無くします。プロジェクトの状態も `stage` 1 本で持ちます。**

書いているうちに分かったのですが、この 2 つは**別の軸ではなく、同じ軸の粗さ違い**です
（引き合い → 提案 → 受注 → 完了）。両方持つと「同じことを 2 か所で持つ」ことになり、
**必ず片方だけ更新される日が来ます**。

- **画面の「いまの状態」は案件と同じ 7 段**（ネタ／仮押さえ／提案中／内示／受注済／完了／見送り）
- **一覧の絞り込みだけ 4 つに束ねる**（準備中／進行中／完了／見送り）。
  **束ねるのは読むときだけ**で、保存するのはいつも `stage` そのものです
  — 4 段で保存して 7 段に戻すと、**触っていないのにステージが動きます**
  （`neta` → 「準備中」→ 保存 → `c_proposal` になる）
- **工程が動いているかは `gpm_phases` から出す**（決め④で親を付け替えるので引ける）。
  進み具合はもともと列に持っていません
- 取り込み時の初期値は **`a_won`（受注済）**。`gpm_projects` にある行は
  「やると決まって工程を切ったもの」で、ヨミではありません。
  `status='done'` の行だけ `s_completed` にします

> `planning` / `onhold` だった行は `a_won` に倒れます。**黙って倒しません** —
> migration が `RAISE NOTICE` で名前を列挙し、`gpm_projects.status` も残るので
> あとから突き合わせられます（migration B までは残る）。
...
- **`/gpm/*` はすべて行の `gls_category = 'B'` を確かめる**（`gpm.service.ts` の
  `IS_PROJECT` と `index.ts` の `isGpmProject()`）。案件（A）の id を渡すと 404 / 403 になる。
  → **`gpm` の権限しか無い人が案件に触れる道はない**
- **`/projects/*` は今までどおり `sales` 権限で、A も B も返す。**
  ここを行で 403 にすると**財務が壊れます** — 売上台帳の案件選択（`RevenueDialog`）・
  決算取込・検索・MCP がこの口を通っており、経理は `gpm` を持ちません（決め⑧の
  「外さないもの」と正面から衝突する）。**GLS-B は前からここに居た**ので、
  そのままにしても見える範囲は増えません
- **案件管理の画面が `gls_category=A` を渡す**（一覧・ダッシュボード）。
  サーバー側も受付・タスク一覧・営業 KPI に同じ絞り込みを入れてあるので、
  画面だけで隠している状態にはならない
- **案件詳細で GLS-B を開いたら、プロジェクト管理の同じ id へ転送する。**
  404 にすると「移ったのか消えたのか」が分からない（id は変わらない）
...
## 実装の記録（2026-08-08・migration A まで）

### 入れたもの

| 段 | 何をしたか |
| --- | --- |
| 1 | **migration 179**（`projects` に 5 列 ／ 自社の顧客 1 行 ／ 子テーブル 3 本の親を `project_id` に差し替え ／ `project_tasks.project_id` の埋め ／ `estimates` の排他 CHECK を外して `gpm_project_id` を落とす） |
| 2 | **サーバー**: `contexts/gpm` を `projects`（`gls_category='B'`）へ。`estimate.service` / `billing.routes` / `salesOverview` / `gpm-estimate.service` の絞り込みを `gpm_project_id` から `gls_category` へ |
| 3 | **案件管理を GLS-A に**: 案件一覧・ダッシュボードの「動いている案件」・受付（期限超過 / AI 起票 / 申込書）・タスク一覧・営業 KPI（進行中 / 今週の実施 / 止まっている / 今月の受注 / 今月の売上 / 見積の返事待ち） |
| 4 | **画面の移動**: 確定案件（ビジネス）→ `/gpm/projects` へ転送、月次請求（月締め）→ プロジェクト詳細の「請求」タブ、案件詳細で GLS-B を開いたら転送、案件フォームの A/B トグルを廃止（発番済みを「プロジェクト管理へ移す」道は残す） |
| 5 | **言葉**: 「ビジネス案件」→「プロジェクト」。`docs/wording.md` に追記し、`check-ui-tokens` の `forbidden-wording` に入れて書き戻りを機械で止めた（残り 97 → 63 件） |

### 途中で見つけて直したもの（どちらも実物を動かして見つけた）

1. **migration の順番**: `estimates` の排他 CHECK を**外す前に** `project_id` を埋めていたため、
   行があると **その UPDATE 自身が CHECK に弾かれる**。空の DB では通ってしまうので、
   実データを入れて試すまで気づけなかった
2. **`/gpm/estimates/summary` が常に 404 だった**（これは今回の変更より前からの不具合）。
   `/gpm/estimates/:id` が先に宣言されており、`summary` が id として食われていた。
   ダッシュボードの見積 KPI 2 枚が最初から出ていなかったことになる。
   型にも lint にも出ない形なので、**実 API を叩いて見つけた**

### 検証

**① 集計が動かないこと**（決め⑧の「外さないもの」）。migration 178 の状態に
GLS-A と GLS-B のデータ（売上・仕入・販管費・見積・タスク・工程・体制・未確認事項）を入れ、
179 の前後で数えた:

```
売上合計(確定) 1,000,000     → 1,000,000     同じ
仕入合計       4,240,000     → 4,240,000     同じ
販管費合計        50,000     →    50,000     同じ
見積(sent)合計 23,220,000    → 23,220,000    同じ
売上件数 / タスク数            変化なし
案件数(A) 1 → 1               変化なし
案件数(B) 1 → 2               ← 案件に結んでいなかったプロジェクトが案件になった分（意図どおり）
```

**② 実 DB + 実サーバーで 10 項目**（migration を当てた Postgres にサーバーを立てて実際に叩いた）:
GPM 一覧が GLS-B だけを返す ／ 束ねたステージ（`planning` / `active`）で絞れる ／
**知らないステージは空**（素通ししない）／ GPM の詳細に案件（A）の id を渡すと 404 ／
GPM のタスク一覧 ／ GPM の見積とまとめ ／ 案件管理ダッシュボードにプロジェクトの見積が
混ざらない ／ 案件の見積一覧に GLS-B が出ない ／ 受付が GLS-A だけ ／
**GPM の口から案件（A）の見積明細を書こうとすると 403**。

**③ 全9ワークスペースのビルド通過。** 型チェック（7アプリ + server）exit 0、
`shared` のテスト 209 件通過、`eslint` 0 errors / 61 warnings（着手前と同数）。

### まだやっていないこと

- **migration B（`gpm_projects` を落とす）は入れていません。** 落とす前に、
  検証環境で 1 周まわして「やはり要る」が出ないことを確かめます
- **`gpm_projects` の `status` 列は残っています。** 倒したステージを突き合わせるための
  控えなので、migration B で一緒に消えます
