...
**理由**: これらは「GLS-B は売上を持つ案件である」ことに依存しています。
...
1. **自社構築は GLS-B の番号を採るか。** 売上を生まない社内の設備投資に案件番号を振ると、
   財務の一覧に「売上 0・仕入あり」の行が並びます（決め④）
2. **既存の `gpm_projects` に実データがあるか。** 本番に行があれば移行が要り、
   0 行なら**畳んで作り直すほうが安全**です（移行スクリプトを書かずに済む）。
   **手元で確かめられたのはここまで**: 検証用 DB は `gpm_projects` **0 行**、
   `seed.ts` / `seed-tasks.ts` も GPM の行を 1 つも作りません
   （`gpm_templates` の 2 行だけは migration が入れる既定の雛形）。
   **本番と検証環境の実データは、この場からは見られません**
3. **`gls_category` の名前を変えるか。** 画面から「ビジネス案件」の語が消えるので、
   `A`/`B` のままにするか、意味の分かる名前にするか（`docs/wording.md` の決めごと）

---

## 進め方（この順でないと途中で壊れる）

1. **上の「要確認」3 点を決める**（特に 2 — 実データの有無で段取りが変わる）
2. **migration**: `projects` に 5 列追加 ／ 子テーブル 3 本の親を付け替え ／
   `estimates` の CHECK を外して `gpm_project_id` を落とす ／
   `gpm_projects` の行を `projects` へ取り込む ／ `gpm_projects` を落とす
   （**落とすのは最後の migration に分ける** — 取り込みが間違っていたとき戻せるように）
3. **サーバー**: `contexts/gpm` のサービス 14 か所を `projects`（`gls_category='B'`）へ向ける。
   権限は行で判定する（決め⑦）
4. **案件管理側**: 一覧・受付・タスク・ダッシュボードに `gls_category='A'` を入れる。
   **1 か所ずつ、入れるたびに件数が変わることを確かめる**（忘れると GLS-B が混ざったまま）
5. **画面の移動**: 確定案件（ビジネス）と月次請求をプロジェクト管理へ。旧 URL は転送
