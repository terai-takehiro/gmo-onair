...
> **状態**: 現役の決めごと（段1〜4 は実装済み）
> **最終確認**: 2026-09-08（v4.6.10）
> **位置づけ**: 隔週キープの数字・資料ビルダーのロジックの正。画面は `client-daily/src/pages/weekly/{keep,deck}/`、型と純粋関数は `shared/src/keepReport/`、DB は migration 291。残りは §11.1（Slack への投稿と反応の回収）
...
> 計上会社 `entity_code`（SCS／GSS／GMO）・`legal_entities`・`org_transition`・帳簿の行の `entity_code` は **main のものをそのまま読む**。
...
| `SCS` | GMOサムライコンテンツスタジオ（合弁） | お客様がグループ外で、実施日が切替日以降 |
| `GMO` | GMOインターネットグループ本体（コストセンター・`kind='cost_center'`） | プロジェクト（旧 GLS-B）。売上は無く費用だけ |

- 値の型は [`shared/src/keepReport/types.ts`](../../../shared/src/keepReport/types.ts) の `BusinessEntity = 'SCS' | 'GSS' | 'GMO'`
...
- **`org_transition.state = 'off'` のあいだは規則が効かず、全行が GSS。** 計上会社別の表は SCS の列が空（目標 null・実績 0）で出る。
  切替（`cutover`）のあとに登録・改番された案件から SCS／GMO の数字が入り始め、そこから「計上会社別」が意味を持つ
...
- **「全体（統合）」は3社の行の単純合計**。2社間の社内取引（GSS→SCS の売上と SCS の仕入・reorg §4.12）は相殺しないので、
...
KeepReportPack
  ...
  scope: { entity: all|SCS|GSS|GMO, customer_segment: all|internal|external }   … entity の値は計上会社の entity_code（§4）
  landing:  { all, SCS, GSS, GMO? }  … 当月 着地。計上会社ごとの表（6行 × 目標/実績/差/比/判定・鍵は entity_code）＋ 全体（統合）＋ 未確定の売上
  forecast: { all, SCS, GSS, GMO? }  … 翌月 着地見込（確定 ＋ 確度加味）。同じ形
...
会社ごとの表（`SCS`／`GSS`／`GMO`）はその会社の行だけで同じ式。
...
- **前回の構成から組む**: 前回の `KeepDeck` を写し、`auto: true` のページは新しいパックで組み直し、人が足したページ・上書きした文・並び順・消した印（`removed`）は引き継ぐ。部品の位置・大きさは**人が右の「このページ」で直したもの（`options.moved`）だけ**引き継ぎ、ほかはテンプレの今の値にする（無条件に引き継ぐと、テンプレの枠を直しても次の会議の資料が古い位置のまま組まれる — 2026-09-07 の実測）。
...
|---|---|
| **ヘッダーの形を踏襲**: 題の位置・大きさ・青、その下のトークスクリプトの帯（青＝進行担当・発表者、緑＝会議オーナー・全員）。**位置・大きさは実物の XML から読んだインチ**（下の「実物から読んだ寸法」・`FORMAT_CHROME`） | 各ページ |
| **フッターの形を踏襲**: GMO INTERNET GROUP のロゴ（**実物の SVG**）・フォーマット名の青いタグ・「Strictly confidential for internal use only」・ページ番号。罫線は無い。**表紙にも出る** | スライドマスター |
...
#### 実物から読んだ寸法・色・絵（2026-09-07・ご依頼「ヘッダー・フッターは添付の実物から抽出、ロゴデータもちゃんと取る」）

出どころは **`GMO流会議フォーマット_Ver_2_5.pptx`（適用開始 2025/10/24）そのもの**。unzip して
`ppt/slideMasters/slideMaster1.xml`（フッター）・`ppt/slideLayouts/*.xml`（題・表紙・Appendix・締め）・
`ppt/slides/slide15.xml`（報告ページの帯とアイコン）の `<a:off>`/`<a:ext>` を **EMU ÷ 914400 でインチ**にした（小数 3 桁）。
正は `shared/src/keepReport/templates.ts` の `FORMAT_CHROME`（server の写しは `keep-templates.ts`・一致は
`shared/tests/keepReportDeckParity.test.ts`）。**pptx はこのインチをそのまま使い、画面のプレビューは % に換算して描く**
（1280×720 に置くと 1in = 96px・1pt = 1.333px。`client-daily/.../renderers/slideStyle.ts` の `px()`）。
**手で測り直さない・「見た感じ」で寄せない** — 変えるときは実物の pptx から読み直す。

| 要素 | 位置（x, y）・大きさ（w × h）in | 書式 | 出どころ |
|---|---|---|---|
| 題 | (0.234, 0.124)・9.817 × 0.707 | 36pt 太字 `#005BAC` 左・上詰め。長い題は PowerPoint の自動調整（normAutofit）で 1 行に縮む | レイアウト「8．アジェンダ」の title placeholder（マスターは 28pt だがレイアウトが 36pt に上書き） |
| トークスクリプトの帯 | x 0.234・w 13.003・h 0.449。上の帯 y 0.86／下の帯 y 1.422。角丸 0.075（roundRect の既定） | 18pt `#262626`。文の前に全角空白 3 つ（アイコンの分）。注記は赤 `#D62825` 右寄せ | 報告ページ（slide15）の 2 つの角丸長方形 |
| 帯のスピーカー | x 0.379・0.396 角（帯の中で縦中央） | 実物の SVG（`image4.svg`） | slide15 |
| 本文の上端 | 帯あり 1.983／題だけ 0.841 | — | 報告ページの content placeholder／レイアウトの body placeholder |
| フッター: ワードマーク | (0.256, 7.166)・2.647 × 0.166 | 実物の SVG（`image1.svg`・青 `#005BAB`／灰 `#595757`） | スライドマスター |
| フッター: タグ | (2.988, 7.149)・3.952 × 0.204 | 10pt 白・地 `#005AAC`・中央 | 各ページの「コンテンツ プレースホルダー 2」 |
| フッター: Strictly confidential | (9.234, 7.099)・3.442 × 0.303 | 12pt `#C00000` 右寄せ | スライドマスター |
| フッター: ページ番号 | (12.427, 7.046)・0.677 × 0.404 | 24pt 太字 `#808080`（bg1 の lumMod 50%）右寄せ。スライド番号のフィールド | スライドマスター |
| 表紙: 会議名 | (0, 1.5)・13.333 × 1.142 | 72pt 太字 黒 中央 | レイアウト「タイトルページ」 |
| 表紙: 部署・プロジェクト名と日付 | (0, 2.898)・13.333 × 1.453（上下 2 段に割って使う） | 40pt 中央 | 同 |
| 表紙: 青い箱（フォーマットの版） | (1.559, 4.717)・10.215 × 0.968 | 24pt 白・地 `#005BAC`・中央・2 行 | 同 |
| 表紙: 進行の注意書き | (0.87, 5.947)・12.14 × 1.01 | 20pt 左・3 行 | 同 |
| Appendix | (0.367, 3.12)・12.385 × 1.01 | 54pt `#005AAC` 左（太字ではない） | レイアウト「Appendix」 |
| 締め「すべての人にインターネット」 | (2.461, 2.548)・8.412 × 2.403 | 実物の SVG（`image2.svg`・青 `#005BAC`） | レイアウト「すべての人にインターネット」 |

色は `FORMAT_COLORS`: 題 `#005BAC`（マスターの titleStyle）・メイン（ポジ情報）`#005AAC`・アクセント（ネガ情報）`#D62825`・
マーカー `#FFFF00`・帯の青 `#BFD7FF`（＝テーマの accent1 `#0047C0` に lumMod 20%／lumOff 80%。HSL で計算）・帯の緑 `#D2ECD8`・
本文 `#262626`・Strictly confidential `#C00000`・ページ番号 `#808080`。書体は Noto Sans JP（`FORMAT_FONT`）。

**絵（ロゴ）は描き直さず実物の SVG をそのまま持つ** — `shared/src/keepReport/formatAssets.ts`（base64・`formatAssetDataUri()` で
`<img src>` に渡す）と server の写し `keep-format-assets.ts`。server は **PNG の代替**（Playwright の Chromium で SVG を 5 倍に描いたもの。
ワードマーク 1912×120・タグライン 1384×396・アイコン 288×288）も持つ。理由は pptxgenjs が Node では SVG の代替 PNG を作れず
壊れた画像を入れるため（dist の `isSvgPng` → `IMG_BROKEN`）。pptx には PNG を置いて `altText` に `gmo-format:<key>` の印を入れ、
出力後に `keep-pptx-svg.service.ts` が実物のテンプレと同じ形（`<a:blip r:embed=PNG><a:extLst><asvg:svgBlip r:embed=SVG/>`）で
元の SVG を結びつける。PowerPoint 2016 以降・LibreOffice は SVG を、Keynote・Google スライド・古い版は PNG を読む。

この取り込みで変えたこと: 罫線を消した（実物に無い）／フッターを表紙にも出す／帯を実物の 2 本（0.449in）にして
...
| 種類 | 口 | 権限 |
|---|---|---|
| GET | `/dailyops/keep/pack?meeting=YYYY-MM-DD&entity_code=all&segment=all` — いまの数字（凍結版があればそれ）→ `{ pack, frozen, pack_id, meeting_frozen }`。`entity_code` は SCS／GSS／GMO か `all`（main の帳簿の列と同じ名前。旧 `entity` は廃止） | `dailyops` or `sales` reader |
...
| 項目 | 決まり |
|---|---|
| 計上会社 | グループ内のお客様のイベント → GMOサムライスタジオ（GSS）、グループ外 → GMOサムライコンテンツスタジオ（SCS）、プロジェクト → グループ本体（GMO）。値は main の `entity_code`・決め方も main の規則（§4）。収支は計上会社別 ＋ 全体（統合） |
