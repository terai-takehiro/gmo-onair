...
| --- | --- | --- |
| 13 | **`/qsheet` を「案件を選ぶ」に変えてよいか** | ✅ 2026-08-22 解決。`routeSwitch.ts` の `QSHEET_ROOT` は第三の答え `'top'`（アプリ全体のトップ・`ProductionTopPage.tsx`）に切り替え済み。「案件を選ぶ」(`'home'`)・「今の一覧」(`'list'`) も後方互換URLとして生きている |
