...
> 機器側の事実（Assistant が何を読むか）は [`../../qsheet-recording-streaming.md`](../../qsheet-recording-streaming.md)（PR #279）が正です。
...
**実測の最大は `211_drop_techsheet_schema.sql` なので、次に空いているのは `212` です。**

⚠️ **README §6 の採番表は「現在の最大が 210」を前提にしており、1つずれています。**
211 は既に `211_drop_techsheet_schema.sql` が使っています。README §6 の 211〜217 は
**そのまま 212〜218 にずらす**か、この段のほうが先にマージされるなら**この段が 212 を取り、README 側が 213 以降にずれる**か、
どちらかです。**`migrate.ts` はファイル名順に流すだけで、番号が重なっても CI は落ちません**（黙って想定と違う順に流れます）。

**したがって、この文書は具体的な番号を固定しません。** 着手時に

```bash
ls server/src/shared/db/migrations/ | sed -n '$p'
```

で最大を確かめ、**その＋1** を取ってください。**この文書の中では `NNN` と書きます。**
（2026-08-21 現在に着手するなら `NNN = 212`、ファイル名は `212_qsheet_device_settings.sql`。）

**08 の DDL は1ファイルにまとめます**（2表とも同じ機能で、片方だけ入った状態に意味が無いため）。
