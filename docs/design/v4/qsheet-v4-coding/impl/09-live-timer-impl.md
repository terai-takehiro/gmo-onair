...
> ✅ **2026-08-22 追記: 実装済み**（PR1〜7すべて実装済み。PR1〜4はPR #288の段9、
> PR5〜7は当初未着手のまま残っていたことが利用者の指摘で判明し、PR #290で実装。
> ただしPR7は§12-11のとおりスコープを絞った）。
> ⚠️ **§12-11 がPR7で見送った「導線・改称」は、[`../12-live-timer-decision.md`](../12-live-timer-decision.md)
> であらためて設計し直しました。** §12-11 が見送り理由の1つに挙げた「制作技術支援自身が
> まだ共通シェルに載せ替えていない」は、その後の載せ替え完了によりもう成立しません。
...
| 過去を入れたら | **400**（「終了時刻が過去です」）。開始できてしまうと1サイクルで止まる |
