...
> **状態**: 設計（未実装）
> **最終確認**: 2026-09-13
> **位置づけ**: 制作技術支援に足す9つ目のミニアプリ「会場図面」の正（何をする道具か・利用者に見せる語・画面・縮尺の担保・データの持ち方・運営マニュアルとの連携・作る順）。**実装はまだ1行も書いていない。** §14 の8件は **2026-09-13 に全件ご判断をいただき**、本文はそれに合わせてある（Q6 だけ推奨と違い「推定のまま進める」）。あわせて**トラス脚などの固定物は描かない**ことも決まった（§4-7）。着手できる状態
...
### 4-7. 固定物は描かない（2026-09-13 のご判断）

「図面に点在する黒い長方形（トラスの脚 10 本）は完全に消す」というご判断。固定物の扱いを次のとおり決める。

| 固定物 | 図面（②）・紙（③・冊子） | データ |
| --- | --- | --- |
| トラス脚（450角×10）・自動ドア・ワインセラー・ELV のかごなど**床の上のもの** | **描かない**。下敷き画像に描かれている □■ も**取り込み時に消し込む**（`underlay.masks`＝白で塗る矩形の一覧を階に持ち、下敷きを描くときに当てる。用賀 26F の WORLD STUDIO は 15 か所） | `fixtures[]` に**残す**。使い道は §4-5 のはみ出し検査だけ（「トラス脚に重なる」を数える）。画面には出さない |
| LED ウォール（壁）・エリアの内法（壁の内側の線） | 描く | 同上 |
| トラスの外形（天井にあるもの） | 冊子の線画では細い点線だけ。②では描かない | `fixtures[]` |

理由: 椅子を並べる面が「何も無い床」に見えるほうが PowerPoint の手つきに近く、実在する障害物への配慮は**検査で数える**ことで担保する（置くのは止めない・§4-5）。
...
手順 6 のあとに**下敷きの消し込み**を置く（§4-7）: 下敷きに描かれている固定物の記号（用賀 26F はトラス脚の □■ 10 組と黒い矩形 5 つ）を矩形で囲んで白く塗る。塗った矩形は `underlay.masks[]` に残し、下敷きを描くたびに当てる（画像そのものは書き換えない。取り消せるようにするため）。
...
| 7 | **A**: 冊子に載せるのは線画だけ |
| 8 | **A**: 図面にも確定と版を持たせる |
| 9 | **固定物は描かない**（追加の判断・§4-7）: トラス脚などの床の上の固定物は図面にも紙にも描かず、下敷きからも消し込む。データは検査にだけ使う |
