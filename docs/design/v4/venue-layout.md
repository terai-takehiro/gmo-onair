...
> **状態**: 設計（未実装）
> **最終確認**: 2026-09-13
> **位置づけ**: 制作技術支援に足す9つ目のミニアプリ「会場図面」の正（何をする道具か・利用者に見せる語・画面・縮尺の担保・データの持ち方・運営マニュアルとの連携・作る順）。**実装はまだ1行も書いていない。** §14 の8件は **2026-09-13 に全件ご判断をいただき**、本文はそれに合わせてある（Q6 だけ推奨と違い「推定のまま進める」）。あわせて**下敷きに描かれたトラス脚の横の黒い矩形は消し込み、トラス脚（柱）は白抜きの柱記号で残す**ことも決まった（§4-7）。クレーンの機種は **TK-53L**。着手できる状態
...
- 追加: 昭特（shotoku.tv）の**ペデスタル TP-90B**・**クレーン TK-53L**（ロングアーム。国内サイトの型番表記は TK-53A／53AL で L がロングアーム。台車 **TI-04B**）を、俯瞰イラストと忠実なサイズ感で図面に投入したい
...
| レイヤー／z-index | **「重ね順」**（最前面へ・最背面へ）。数字は出さない |
...
残す語（PowerPoint・Figma・Illustrator と同じで画面の外でも通用する — ルール6・10）:
**ズーム・グループ化・整列・複製・スナップ・グリッド・ルーラー**。
執筆時（2026-09-13）は運営マニュアルにまだ合わせて「すいつき」「方眼」「ものさし」と和語に言い換えていたが、
同日の [docs/wording.md](../../wording.md) ルール10「通じるカタカナ語は、言い換えずそのまま使う」の決定
（運営マニュアルは §4-1 で対応済み）をこちらは見落として実装してしまっていた。**スナップ・グリッド・ルーラー**に統一する
（2026-09-14 の利用者のご指摘で判明・修正）。
...
### 4-7. 固定物の描き方（2026-09-13 のご判断）

「図面に点在する黒い長方形は消す。ただしトラス（脚）は残す」というご判断。用賀の 26F 図面では、トラス脚 10 本が **□（450角の柱）＋ その横の ■（黒い矩形）** の組で描かれている。■ は消し、□ は残す。

| 固定物 | 図面（②）・紙（③・冊子） | 下敷き | データ |
| --- | --- | --- | --- |
| トラス脚（450角×10） | **白抜きの柱記号**（塗らない □）で描く。黒く塗らない | □ はそのまま。**横の ■ は取り込み時に消し込む**（`underlay.masks`＝白で塗る矩形の一覧を階に持ち、下敷きを描くときに当てる。画像そのものは書き換えない） | `fixtures[]`。§4-5 のはみ出し検査で「トラス脚に重なる」を数える |
| そのほか下敷きに点在する黒い矩形（用賀 26F は WORLD STUDIO の四隅と前室の 5 つ） | — | 消し込む | — |
| 建築図面の寸法線と数字（9810・8530.2・4876.8・500・2000・2645・1600・4970・1780・2250・1040 など）| — | **消し込む**（2026-09-13 のご判断: 図面の中に尺の表示は要らない。寸法は品目の札・右パネル・ルーラー・1m バーで足りる） | — |
| 自動ドア・ワインセラー・ELV のかごなど | 白抜きの薄い枠で描く | そのまま | `fixtures[]` |
| LED ウォール（壁）・エリアの内法（壁の内側の線） | 描く | そのまま | `fixtures[]` |
| トラスの外形（天井にあるもの） | 冊子の線画では細い点線だけ。②では描かない | そのまま | `fixtures[]` |

理由: 実在する障害物（柱）は見えていないと椅子を置いてしまうが、黒く塗ると図面が汚れる。**線で示す・面で塗らない**は冊子の規律（`production-manual.md` §6-6）とも同じ。
...
**道具の帯**（置く・並べる／図形5／整列6／重ね順／元に戻す／スナップ／グリッド／ルーラー／ズーム −・%・＋／階・エリアの切替／階全体を表示）→
...
| 中央 | mm 空間の SVG（`viewBox` を mm）。重ね順は 下敷き → グリッド・通り芯 → エリアの内法（薄い塗り）→ 固定物 → 品目 → 選択枠と札。上と左にルーラー、右下に 1m バー |
| 右 | 名前・寸法（mm。図形だけ編集可）・位置（エリア左上からの mm／通り芯からの距離 `X13 +1,200` に切替）・向き（度・反転）・高さ（ステージ H200／H400）・アームの角度（クレーン）・重ね順・固定。グループなら §7 の入力欄と「並べ直す」「グループ解除」。数字は `.num`・3桁区切り。**px は出さない** |
...
| つかんで動かす | スナップ先は 壁（内法）・通り芯・他の品目の端と中心・1m グリッド（任意）。閾値は画面 8px 相当 | `snapPosition` |
...
| 整列・等間隔・最前面／最背面 | 同じ | `alignBlocks`／`distributeBlocks`／`reorderZ` |
| Ctrl+G／解除 | 並べた結果と任意の複数選択をグループに。ダブルクリックで中の1つ | 新規 |
| スペース＋ドラッグ／Ctrl＋ホイール | 図面をつかむ／ズーム（25〜400%） | `useManualPan` |
| 右クリック・Delete・Esc | 複製・削除・最前面へ・最背面へ・向きを反転・グループ解除／削除／選択解除 | 新規 |
...
| 見せ方 | 中身 |
| --- | --- |
| ルーラー | 図面の上と左。1m／5m の目盛。ズームに追従 |
| グリッドと通り芯 | 1m の点はズーム 50% 以上で表示。通り芯 6,400 の細線に X16〜X9・Y16〜Y9 のラベル（3,200／1,600 の補助線は任意） |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `world-26f-v2.png`（v4.6.16・migration 301） | 26F | (12,193, 10,995) | 897×1083 | 200 | 0.043953／0.045855 | 98,187 | `world-studio`（X14〜X11 と LED ウォール・出入口前室） |
| `lounge-27f.png` | 27F | (27,296, 10,402) | 600×1232 | 220.177 | 0.048387／0.050481 | 54,004 | `lounge-studio`・`pantry`（自動ドアを含む） |
| `floor-26f-v2.png`（v4.6.16・migration 300） | 26F | (4,493, −1,525) | 1187×1650 | 150 | 0.032973／0.03439 | 193,939 | 階全体（`*`） |
| `floor-27f-v2.png`（v4.6.16・migration 300） | 27F | (4,493, −1,525) | 1187×1650 | 150 | 0.032973／0.03439 | 403,977 | 階全体（`*`） |

- **`floor-26f.png`／`floor-27f.png`（420×584）は v4.6.16 で `-v2` に差し替えて廃止**（旧ファイルは削除済み）。利用者が用意した清書版 PDF から作り直したもので、通り芯の PDF 座標が旧原図と一致したため `calibration`（§8）はそのまま、`originMm` も同じ mm 窓（旧下敷きと同じ範囲）で高解像度に再切り出しした。`server/src/app.ts` の静的配信が1年キャッシュ（`immutable`）を返すため、ファイル名自体を変えてキャッシュバストしている。`underlay`（`widthPx`／`heightPx`／`pxPerMmX`／`pxPerMmY`）は migration 300 の UPDATE 文で反映（299 は `ON CONFLICT DO NOTHING` のため既に流れた環境には効かない）
- **`world-26f.png`（800×966）も v4.6.16 で `-v2` に差し替えて廃止**（利用者から「WORLD STUDIO を個別に開くと旧図面のまま」との指摘。300 では階全体の下敷きだけ差し替え、エリア別の下敷きが対象外になっていた）。同じ新原図・同じ校正から、mm 窓（`originMm`）はそのまま高解像度で再切り出し。migration 301 の UPDATE 文で `qsheet_venue_areas.underlay` に反映。`lounge-27f.png`（LOUNGE STUDIO・パントリー）はこの時点で既に判読できていたため対象外
- エリアの行に `underlay` があればそれを、無ければ階の `underlay`（`floor-*-v2.png`）を使う（§5-1）。ROOM A などの小さなエリアは v1 では階全体の下敷きを拡大して見る
...
手順 6 のあとに**下敷きの消し込み**を置く（§4-7）: 下敷きに描かれている不要な記号（用賀 26F はトラス脚の横の ■ 10 個、四隅・前室の黒い矩形 5 つ、寸法線と数字。柱の □ は残す）を矩形で囲んで白く塗る。塗った矩形は `underlay.masks[]` に残し、下敷きを描くたびに当てる（画像そのものは書き換えない。取り消せるようにするため）。
...
**2026-09-13 のご判断（§14 #6）: 推定のまま進める。** 確定は後回しで、札の「寸法は推定」は確定値が入るまで消さない。
...
| 検査 | 会場図面で当たるもの | どうするか |
| --- | --- | --- |
| 1ファイル 400 行（`check-file-size.mjs`。`client-techops/src`・`shared/src` の `.ts`／`.tsx`） | 盤・品目の描画・置く／並べる／右のパネル・幾何 | **最初から割る**（名前は目安）: `VenueBoard.tsx`（盤・render-prop）／`VenueItemView.tsx`（品目1個の描画とつまみ）／`VenueSymbols.tsx`（絵記号）／`VenuePlacePanel.tsx`（置く）／`VenueArrangePanel.tsx`（並べる）／`VenueInspector.tsx`（右）／`VenueUnderlay.tsx`（下敷き・グリッド・通り芯・ルーラー）／`shared/src/venue/` の `scale.ts`・`arrange.ts`・`overflow.ts`（純粋関数） |
...
| 禁止語（`check-ui-tokens.mjs` の `forbidden-wording`・[wording.md](../../wording.md) ルール5） | 画面の文言 | §4-1 の隠す語を使う。機械の正規表現に「レイヤー」「キャンバス」「テンプレート」「オブジェクト」「シミュレーター」は**無い**ので、レビューで人が見る |
...
| --- | --- | --- | --- |
| **A 器** | レジストリ `venue`／`VL`／`prod_doc_vl`（§5-3）・migration 298（表4本）・299（用賀: 階 2・エリア 22・固定物 19・カタログ 36）・下敷き PNG 4 枚・①一覧と作成ダイアログ・空の②（下敷きを2軸で貼る・グリッド・通り芯・ルーラー・1m バー）・タイル・左メニュー・PC専用宣言・確認済みの記録の前倒し（§12-2） | 用賀の図面が案件から辿れ、縮尺の合った下敷きが見える | `npx tsc -b client-techops`・`npm run typecheck -w server`・`npm run lint`・`npm run test`（縮尺変換の Vitest） |
...
**2026-09-13 に全件ご判断をいただいた。** 決定は下の一覧。Q6 だけ推奨と違う。本文はすでに決定に合わせてある。

| # | 決定（2026-09-13） |
| --- | --- |
| 1 | **A**: 「会場図面」・接頭辞 `VL` |
| 2 | **A**: 備品・人・カメラは実寸固定（伸ばすなら「四角」） |
| 3 | **A**: 「並べる」の結果はグループ |
| 4 | **A**: 複数案は複製して A案・B案（別の行） |
| 5 | **A**: スマホは閲覧のみ |
| 6 | **C**: **推定のまま進める**（いまの仮置き＝ベース Ø900・アーム 2,500／3,200・台車 900×1,200 で段D まで作る。確定は後回しで、確定するまで札の「寸法は推定」は消さない。確定値が来たら⑤で差し替える） |
| 7 | **A**: 冊子に載せるのは線画だけ |
| 8 | **A**: 図面にも確定と版を持たせる |
| 9 | **黒い矩形は消し、トラス脚は残す**（追加の判断・§4-7）: 下敷きのトラス脚の横の ■ と点在する黒い矩形は消し込む。トラス脚（□）は白抜きの柱記号で図面にも紙にも残す |
| 10 | **クレーンは TK-53L**（追加の判断）: モックと既定の品目は TK-53L（ロングアーム。最高レンズ軸高 3,850・アーム 3,200・届く範囲 3,800 は推定）。TK-53A はカタログに残すだけ |
...
| 5 | スマホ | A. 閲覧のみ（編集・仕上がり・管理は PC専用宣言）／B. スマホでも編集 | **A** | つまみ・スナップ・右パネルは 375px で組めない（§6 ②）。当日の1列追加は PC で |
