...
| `floor-26f-v2.png`（v4.6.15・migration 300） | 26F | (4,493, −1,525) | 1187×1650 | 150 | 0.032973／0.03439 | 193,939 | 階全体（`*`） |
| `floor-27f-v2.png`（v4.6.15・migration 300） | 27F | (4,493, −1,525) | 1187×1650 | 150 | 0.032973／0.03439 | 403,977 | 階全体（`*`） |

- **`floor-26f.png`／`floor-27f.png`（420×584）は v4.6.15 で `-v2` に差し替えて廃止**（旧ファイルは削除済み）。利用者が用意した清書版 PDF から作り直したもので、通り芯の PDF 座標が旧原図と一致したため `calibration`（§8）はそのまま、`originMm` も同じ mm 窓（旧下敷きと同じ範囲）で高解像度に再切り出しした。`server/src/app.ts` の静的配信が1年キャッシュ（`immutable`）を返すため、ファイル名自体を変えてキャッシュバストしている。`underlay`（`widthPx`／`heightPx`／`pxPerMmX`／`pxPerMmY`）は migration 300 の UPDATE 文で反映（299 は `ON CONFLICT DO NOTHING` のため既に流れた環境には効かない）
