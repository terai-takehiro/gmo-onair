...
手順 6 のあとに**下敷きの消し込み**を置く（§4-7）: 下敷きに描かれている不要な記号（用賀 26F はトラス脚の横の ■ 10 個と、四隅・前室の黒い矩形 5 つ。柱の □ は残す）を矩形で囲んで白く塗る。塗った矩形は `underlay.masks[]` に残し、下敷きを描くたびに当てる（画像そのものは書き換えない。取り消せるようにするため）。
