...
| [`wiki.md`](wiki.md) | **Wiki**（新しいブロックアプリ）。Markdown で書いた文章を木に並べ、探せて、AI が読める。手入力を「AI で整える」で Markdown に。見直し期限と「足りないページ」で古びない | **設計（未実装・判断待ち）**。§10 の10件にご判断をいただいてから着手（スマホでも編集は決定済み） |
...
| [`mockups/native/wiki/`](mockups/native/wiki/README.md) | Wiki のモック（PC 6・スマホ 2・地図 1） | 設計（未実装・判断待ち） |
