...
> - **モック自身が食い違っていることがあります。** その場合も勝手に選ばず、
>   **どちらに寄せるかを訊く**。実例と、決まった答え:
>
>   | 食い違い | ご判断 |
>   | --- | --- |
>   | 案件作成の警告文は「まだ**つくれません**」なのに、同じ画面のボタンは「**案件にする**（与件化）」 | **「案件にする（与件化）」に統一。** 警告文は「まだ**案件にできません**」— できないと言われている操作と、押すボタンの名前は同じ言葉にする |
>
>   なお**画面を開くボタン**（一覧の「案件をつくる」）と**画面の名前**（「案件作成」）は
>   これとは別で、**モックと一致しています**（実測 4回 / 6回）。
>   「つくる」＝画面へ行く、「案件にする」＝実際に登録する、で役割が違います
...
- **数字だけ** `font-variant-numeric: tabular-nums` ＋ `'palt' 0, 'tnum' 1`
  - ⚠️ **いまの書体では等幅になりません（実測・2026-08）。** 数字の subset を
    読み込ませて測ると、`palt 1` / `palt 0` / `tnum 1` の**どれでも同じ幅**でした
    （`1111111111` = 77.56px ／ `8888888888` = 114.20px。`monospace` は 99.33px で
    同じなので、測り方は等幅を検出できます）。**LINE Seed JP の数字は
    プロポーショナル**です
  - **揃うのは「金額の列の右端」**です（`MoneyCell` の固定幅 ＋ `Money` の
    `justify-between`）。**桁ごとには揃いません**
  - **指定は残してあります。** いまの書体では効きませんが、**書体を替えた日に効きます**
