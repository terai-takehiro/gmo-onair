...
> **状態**: 設計（未実装・**判断待ち**）— §10 の 10 件にご判断をいただいてから着手する。同日にいただいた4点のご指示（編集は手入力の使いやすさ優先／手入力したものを AI で Markdown に整える／スマホでも書ける／UI の語は取り決めを徹底）は反映済み
> **最終確認**: 2026-09-22
...
モックは Claude Design のキャンバス（押せる試作・PC 6枚＋スマホ 2枚＋考え方の地図 1枚）:
...
| 2 | **Markdown で書けないブロックは作らない** | GFM（見出し・段落・箇条書き・番号・チェック・表・コード・引用・リンク・画像・区切り）＋拡張3つ（注意書き `> [!NOTE]`・折りたたみ `<details>`・ONAiR カード＝ただのリンク）だけ。Notion のデータベース・ボード・カレンダーは作らない（構造データは案件・機材・タスクの既存アプリにある） |
| 3 | **外から同じものが読める・書ける** | REST `GET /api/v1/wiki/pages/:id.md`（text/markdown）・MCP ツール5本・スペースまるごとの書き出し（.md の木＋YAML の見出し）と取り込み |

利用者に見せる語は **6つ**（§4-1）: **スペース／ページ／ブロック／テンプレート／履歴／見直し期限**。
...
```
トップページのタイル「Wiki」
 └ ① ホーム              /wiki                  ← 検索・AI に聞く・スペース・最近更新・自分の見直し期限
     ├ ② ページ          /wiki/p/:id            ← 左=木／中央=本文／右=情報・目次・履歴・コメント
     │   ├ ③ 編集        /wiki/p/:id/edit       ← 手入力優先＋「AI で整える」。PC・スマホ（⑨）
     │   └ ⑥ 履歴        /wiki/p/:id/history    ← ②の右パネルと同じ。版の比較はここ
     ├ ④ 検索            /wiki/search?q=        ← 絞り込み＋抜粋。上に「この質問を AI に聞く」
     ├ ⑤ AI に聞く       /wiki/ask              ← 出典つきの回答。出典が無ければ「足りないページ」に積む
     ├ ⑦ 見直し          /wiki/review           ← 期限切れ／足りないページ／AI の直され方（月1回の場）
     ├ ⑧ 閲覧（スマホ）  /wiki/p/:id            ← 同じURL。読む・検索・聞く・コメント
     └ ⑨ 編集（スマホ）  /wiki/p/:id/edit       ← 手入力＋「AI で整える」に絞る（道具の帯は5つ）
```

**この設計でいちばん大きい判断は「Notion のデータベースを作らない」（§10-3）と
「出典の無い質問には答えず、足りないページとして積む」（§10-8）の2つ。**
前者で作る量が半分になり、後者で AI がマニュアルを育てる側に回る。
...
| **育てる** | スペースの担当（月1回） | PC | 見直し期限切れ・答えられなかった質問・AI 下書きの直され方を見て、ページを直す・書く |

Notion との違いを1行で言うと: **「何でも書ける場所」ではなく「会社の決めごとを、古びない形で置く場所」**。
だから見直し期限（§6-⑦）と AI の「足りないページ」（§7-1）を、Notion に無い中核として持つ。
...
| **ブロック** | ページの中の段落・見出し・表・画像・注意書き。`/` で足す | Notion・WordPress と同じ語（ルール10）。運営マニュアルの「ブロック」とは指すものが違うがアプリが別なので衝突しない |
...
| **見直し期限** | そのページを担当が次に見直す日。過ぎると「見直し期限切れ」の札が付き、担当に通知 | 造語ではなく説明語。ルール7（12文字以内） |
...
**使わない語**: プロパティ（→ ページの「情報」欄: 担当・見直し期限・タグ）・データベース・ワークスペース・
...
3. `GET /api/v1/wiki/export?space=sales` — スペースまるごとを **zip（`.md` の木）** で書き出す。`POST /api/v1/wiki/import` は逆（Obsidian・Notion の Markdown 書き出しを受ける）
...
意味検索（ベクトル）は v1 で作らない。**AI に聞く**の出典探しもこの検索を使う（§7-1）。検索が弱いと AI の出典も弱いので、段C（検索）を段D（AI）より先に作る（§9）。
...
| `server/src/contexts/mcp/tools/wiki.tools.ts` | MCP ツール5本（§7-6） |
...
- 最近更新（10件）・お気に入り・**自分が担当のページで見直し期限が切れているもの**（0件なら出さない）
- 「ページを追加」はホームにも置く（スペースを選んでから作る）

### ② ページ `/wiki/p/:id`

- 左＝スペースの木（折りたたみ・ドラッグで並べ替え・`+` で子ページ）／中央＝本文／右＝**情報・目次・履歴・コメント**のタブ
- 見出し行: パンくず（スペース ＞ 親 ＞ …）・状態の札（下書き／公開／見直し期限切れ）・「AI作成」の札（AI が下書きしたページ。[wording.md](../../wording.md) ルール1）・更新者と日時
...
- **右パネル「情報」**: 担当・見直し期限・タグ・スペース・作成／更新。ここだけは閲覧画面で直せる（本文の編集ロックとは別）
- バックリンク（このページを指しているページ）は「情報」の下に出す

### ③ 編集 `/wiki/p/:id/edit`（PC・スマホ）

**手入力の使いやすさを最優先にする**（2026-09-22 のご指示）。記号や `/` を覚えなくても、
メモ帳と同じ手つきで打てて、あとから **「AI で整える」** で Markdown の形になる。

| 決めごと | 中身 |
| --- | --- |
| **まず打てる** | 開いた瞬間から本文に打てる。段落は Enter で増える。Markdown の記号（`#`・`-`）は**知っていれば効く**が、知らなくても困らない |
| **道具の帯は6つだけ** | 見出し／箇条書き／チェックリスト／表／画像／リンク。文字を選ぶと 太字・リンク が浮かぶ。それ以外のブロック（注意書き・折りたたみ・コード・ONAiR カード）は「その他のブロック」（`/` でも開く） |
...
| **自動保存** | 打つのを止めて 1.5 秒で保存（版が +1）。「保存しました 14:02」を帯に出す。「公開する」で `status='published'`（下書きのうちは検索・AI の出典に出ない） |
...
- 上＝戻る・題・「公開する」／中央＝本文（メモ帳と同じ手つき）／下＝**道具の帯 5つ**（見出し／箇条書き／チェック／写真／**AI で整える**）をキーボードのすぐ上に固定
...
- **出典が無いことは答えない**（§10-8）。「Wiki にはまだ書かれていません」と返し、**足りないページ**に積む（同じ質問が何回来たかを数える）。そこから「ページを作成」を押すと、質問をタイトルにした下書きができ、望めば AI が会話を材料に下書きを書く（§7-2）
...
- 左＝版の一覧（rev・日時・誰が・「何を変えたか」の一言）／右＝**2つの版の違い**（行単位。足した行は緑・消した行は赤）
- 「この版に戻す」は**新しい版として**保存する（履歴は消さない・上書きしない）
- AI が下書きした版には「AI作成」の札。人が直した版との違いが、そのまま条件2の差分（§7-3）
...
| タブ | 中身 | 出どころ |
| --- | --- | --- |
| **見直し期限** | 期限切れ・14日以内・担当が空のページ。1行＝ページ・スペース・担当・期限・最終更新。「見直した」を押すと期限が伸びる（既定 12か月・§10-10） | `wiki_pages.review_by` |
...
- 1画面1目的: **読む**。上＝パンくずと題・本文は縦1列・目次は上部の「目次」ボタンで下シート・木は左上の「≡」で下シート。右上の「編集」から ⑨ へ
...
出典が無く答えられなかった質問は捨てずに `wiki_ai_gaps` に積む（同じ質問は回数を +1）。
...
| **5. レビュー頻度と担当** | △ **月1回・各スペースの担当**（`wiki_spaces.owner_user_id`）が ⑦ で見る。頻度と担当は決めた。**仕組み（通知と「見直した」の記録）は段E で作る** | △ 同左 | △ 同左 |
...
**計測を壊しやすい所**: 公開後の**通常の更新**（見直し期限で直した・組織変更で直した）を AI の誤りとして数えないよう、
...
| 段 | 何を | 終わったときに何ができるか |
| --- | --- | --- |
| **A 器** | ワークスペース `client-wiki`・レジストリ登録・区画 `wiki`・表（§5-1）・スペースと木・ページ閲覧（Markdown の描画・目次・ONAiR カード）・履歴の保存 | ページが**読める**（まだ画面からは書けない。取り込みか API で入れる） |
| **B 書く** | 手入力優先のエディタ（道具の帯6つ・`/`・Markdown で見る）・**スマホの編集**・自動保存と編集ロック・画像・テンプレート・下書き／公開 | **書いて読める。ここで初めて道具になる** |
| **C 探す** | 検索・バックリンク・お気に入り・最近更新・スマホの閲覧・`.md` の REST と書き出し | 探せる。AI 抜きでも Wiki として成立する |
| **D AI** | AI に聞く（出典・3値フィードバック・足りないページ）・下書き・**整える（手入力のメモ→Markdown が主）**・MCP 5本・`ai_outputs`／`ai_corrections`／成果の導出・digest | 使うほど賢くなる経路が閉じる |
| **E 育てる** | 見直し（期限・担当・毎月の通知・「見直した」）・コメント・月次レビューの画面・取り込み・案件詳細と機材台帳への「Wiki」の口 | 古びない |

**段B まで通して初めて道具になる。** 段A だけで止めると「読めるだけの画面」が増える。
**段D は段C の検索が土台**（検索が弱いと出典も弱い）。段D と段E は並行できる。
...
## 10. 判断をいただきたいこと（10件）と決まったこと（1件）

### 決まったこと（2026-09-22・利用者の判断）

| # | 決まったこと | どこに効くか |
| --- | --- | --- |
| 9 | **スマホでも編集できる**（手入力＋「AI で整える」に絞る。表の列の増減・並べ替え・Markdown で見る は PC） | §6-⑨・`pcOnlyScreens.ts` |

あわせて同日のご指示3点は本文に反映済み: 編集画面は**手入力の使いやすさ優先**（§6-③）／
**手入力したものを AI で Markdown に整える**（§6-③・§7-1）／**UI の語は [wording.md](../../wording.md) を徹底**（§4-1「動詞と評価の語」）。

### 判断をいただきたいこと（10件・番号は据え置き）

**先頭の案がこちらの推し。** 決まったら本文をそれに合わせて直し、上の表へ移す
（[`production-manual.md`](production-manual.md) §10 と同じ運用）。

| # | 論点 | 推し | 代替 | どこに効くか |
| --- | --- | --- | --- | --- |
| 1 | **名前と入口** | 「**Wiki**」（`key: 'wiki'`・`/wiki`・トップの「日々の業務」側のタイル） | 「ナレッジ」／「ドキュメント」。「マニュアル」は運営マニュアル（OM）と衝突するので避ける | §4-1・`apps.ts` |
| 2 | **本文の正** | **DB の列 `body_md`**。ファイルは API・MCP・書き出し／取り込みで出す | サーバー上の `.md` ファイル（git 管理）を正にする — 権限・検索・バックアップ・同時編集を自前で作ることになる | §5-2 |
| 3 | **Notion のデータベース** | **作らない**。表ビュー・ボード・カレンダーは案件・機材・タスクの既存アプリに任せ、Wiki は文章とリンクに徹する | 「属性つきページ一覧」（タグ・担当で絞れる軽い表）だけ作る | §1・§4-2・§11 |
| 4 | **同時編集** | v1 は**ページ単位の編集ロック**（運営マニュアルと同じ・10分で自動解除・引き継ぎ可）。v2 で Yjs（部屋の管理 `roomManager.ts` は共有済み） | 最初から Yjs の同時編集（Notion と同じ体験。エディタとの結線とサーバー側の Markdown 化が最大の作業） | §6-③・§5-3 |
| 5 | **権限の区画** | 新しい区画 **`wiki`**（reader を全員の既定に）。閲覧範囲はスペース単位（全員／指定メンバー） | `dailyops` の区画を流用（5か所を直さずに済むが「日常業務が読めない人は Wiki も読めない」になる）／ページ単位の共有 | §8・§5-6 |
| 6 | **検索** | 拡張なし（`ILIKE`＋Node で点数）で始め、遅くなったら `pg_trgm` | 最初から `pg_trgm`（`CREATE EXTENSION` を本番 DB に1本入れる） | §5-4 |
| 7 | **「AI に聞く」の段** | **heavy 常に**（手順の誤りは読んだだけでは気づけない） | light を既定にし、出典必須で守る（1回あたりの費用は下がるが、要約の段階の誤りが残る） | §7-1 |
| 8 | **出典の無い質問** | **答えない**。「書かれていません」と返し、足りないページに積む | 一般知識で答え「Wiki の出典なし」と明示する（便利だが、会社の決めごとと世間の一般論が混ざる） | §6-⑤・§7-2 |
| 10 | **見直し期限の既定** | **公開から 12か月**（担当に通知。ページごとに変えられる） | 既定なし（ページごとに手で入れる。入れないページは古びても気づけない） | §6-⑦ |
| 11 | **画像・添付の置き場** | 画像と小さなファイルは ONAiR の `uploads/wiki`、大きなものは BOX へのリンク。**`uploads` のバックアップを運用に足す** | 全部 BOX（Wiki の画面に画像が出ない）／全部 ONAiR（バックアップの範囲を広げてから） | §5-5 |

### まだ論点にしていないこと（実装を止めない・使ってから決める）

- 案件・機材の詳細に「Wiki」タブを出す時期（段E。先に Wiki 側のリンクだけ作る）
...
- **Notion のデータベース**（表ビュー・ボード・カレンダー・リレーション）— §10-3
...
- **通知の細分化**（ページの更新を購読する等）— 見直し期限と月次の通知だけ
