...
- **ナビの列は1本だけ**（2026-09-22 のご指摘「サイドタブが増えすぎて、この UI だとすごく窮屈」）。
  スペースのツリー（折りたたみ・ドラッグで並べ替え・`+` で子ページ）は**共通の左メニューの中**に出す
  （`shared/src/client/shell/sideMenuSlot.ts` へ差し込む。予定のミニカレンダーと同じ先例）。
  画面の中に2本目のナビの列を作らない — 共通メニューの「スペース」一覧と重なって同じものを2回出すことになる
- 中央＝本文／右＝**情報・目次・履歴・コメント**のタブ。**右パネルは開閉式で、既定は閉じ**。
  閉じている間は目次と情報を本文の下に続けるので、どちらの状態でも情報に辿り着ける。
  開閉は端末に覚える（`gmo_onair_wiki_info_open`）
- 本文の幅の実測（ページ②）: 1440px で 閉じ **860px**（最大幅）／開き 792px、1280px で 閉じ 860px／開き 632px。
  4列だった頃は 1440px で 608px しか残っていなかった
...
- 上＝戻る・題・「公開する」／中央＝本文（メモ帳と同じ操作）／下＝**ツールバー 5つ**（見出し／箇条書き／チェック／写真／**AI で整える**）をキーボードのすぐ上に固定
  - ⚠️ **段B の時点の5つめは「その他」**（「その他のブロック」の一覧を開く）。「AI で整える」は段E で作るので、それまで押せるボタンを出さない（作っていない機能の入口は出さない・`client-wiki/CLAUDE.md`）。段E で差し替える
...
- 1画面1目的: **読む**。上＝パンくずと題・本文は縦1列・目次と情報は本文の下に続ける。
  **ツリーは上辺バーの `☰`（共通メニューの引き出し）から開く** — 画面の中にもう1つ `☰` を置かない
  （同じものを開くボタンが横に2つ並ぶ）。右上の「編集」から ⑨ へ
