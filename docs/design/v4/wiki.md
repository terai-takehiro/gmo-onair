...
| **見直し期限** | そのページを担当が次に見直す日。過ぎると「見直し期限切れ」と表示され、担当に通知 | 造語ではなく説明語。ルール7（12文字以内） |
...
- 見出し行: パンくず（スペース ＞ 親 ＞ …）・状態のバッジ（下書き／公開／見直し期限切れ）・「AI作成」のバッジ（AI が下書きしたページ。[wording.md](../../wording.md) ルール1）・更新者と日時
...
| **B 書く** | 手入力優先のエディタ（ツールバー6つ・`/`・Markdown で見る）・**スマホの編集**・自動保存と編集ロック・画像・テンプレート・下書き／公開 | **書いて読める。ここで初めて道具になる** |
...
| 8 | **出典の無い質問** | **答えない**。「書かれていません」と返し、足りないページに登録する | 一般知識で答え「Wiki の出典なし」と明示する（便利だが、会社の決めごとと世間の一般論が混ざる） | §6-⑤・§7-2 |
