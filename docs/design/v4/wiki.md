...
利用者に見せる語は **8つ**（§4-1）: **スペース／ページ／ブロック／データベース／ビュー／テンプレート／履歴／見直し期限**。
...
トップページのタイル「Wiki」
 └ ① ホーム              /wiki                  ← 検索・AI に聞く・スペース・最近更新・自分の見直し期限
     ...
     ├ ⑦ 見直し          /wiki/review           ← 期限切れ／足りないページ／AI の修正状況（月1回の場）
...
| **見直し期限** | そのページを担当が次に見直す日（任意・既定なし。入れたページだけ見直しに出る） 。過ぎると「見直し期限切れ」と表示され、担当に通知 | 造語ではなく説明語。ルール7（12文字以内） |
...
| タブ | 中身 | 出どころ |
| --- | --- | --- |
| **見直し期限** | 期限切れ・14日以内・担当が空のページ。1行＝ページ・スペース・担当・期限・最終更新。「見直した」を押すと次の期限を入れ直す（期限は任意・既定なし・§10-10。入れたページだけここに出る） | `wiki_pages.review_by` |
...
| **F 見直し** | 見直し（期限・担当・毎月の通知・「見直した」）・コメント・月次レビューの画面・取り込み・案件詳細と機材台帳への「Wiki」のリンク | 期限切れにならない |
...
| 10 | 見直し期限は **既定なし**（ページごとに手で入れる。入れたページだけ見直しに出る） | §6-⑦ |
...
- 見直し期限をスペースごとの既定で入れるか（#10 は「既定なし」で始め、入れ忘れが目立てば検討）
