...
- **#726**（`chore(techops): テロップCGの画面の言葉を対応表どおりに統一した`・2026-09-22）——
  テロップCGの画面文字列を `docs/design/v4/graphics-redesign.md` §7「言葉の対応表（旧 → 新）」と
  モックに合わせて統一した（53ファイル・+253/−228・1コミット）。段A〜Fで新しい語に寄せたのは
  主な導線だけで、確認ダイアログ・お知らせ・空状態・入力欄のラベル・サーバーのエラー文には
  旧語が残っており、同じものが2〜3通りで出ていた（依頼が「未作画の発注」/「テロップの発注（テロ原）」/
  「ページ化済み」、1枚が「ページ」/「テロップ」、分類が「部品」/「パーツ」/「種類」）。
  **Code Review は Codex の usage limits で実行されず**（#663 以降くり返し起きている既知の状況）、
  Security Review は `c69660a` で完走し findings 無し。**レビュースレッドは0件。表に移す未対応の
  指摘は無い**。CI は `checks`・`build` とも初回で緑、`mergeable_state: clean`。
  ⚠️ **意図して残した判断・未検証のまま入れたもの**（レビュー0件のため自分で書き出す）:
  - **「やめる」「絵」「出す順」はモックにある語なので残した。** `docs/wording.md` ルール8
    （動詞は標準語系＝やめる → キャンセル）とは食い違うが、`docs/v4-plan.md` の「大前提」
    （モックが v4 の正・用語ルールが衝突したらルールのほうを直す）に従った。**この判断が違うなら
    直すのは wording.md 側か、モックのほうになる**
  - **`PageFormDialog.tsx`（383行）は旧語のまま残した。** どこからも import されていない死んだコード
    （段Aで右パネル `TelopEditorPanel.tsx` に置き換わった名残）で、直しても画面には出ないため。
    **ただし旧語（発注・ページ・部品）で grep するとこのファイルが今も引っかかる**。削除は別PR
  - **`SLOT_LABELS`（フルスクリーン／下部テロップ／サイドスーパー／ティッカー／時計・カウント／速報）は
    そのまま。** 設計は「スロットは利用者に出さない」だが、設定「同時に出せないもの」の表と
    バッジでは位置の名前として必要なため、語だけ「出る位置」に寄せて名前は残した
  - **左メニューの「テロップCG設定」は画面の名前（「設定」）と揃えていない。** 同じメニューに
    「収録設定」「配信設定」が並ぶため、単に「設定」にすると何の設定か読めなくなる
  - **`npm run verify:ui` は未実施。** `scripts/verify-ui.mjs` の対象一覧にテロップCGの
    `:ownerKey` 付き URL が無い（`awards-migration` だけが入っている）。代わりに検証用 Postgres へ
    案件1件・依頼2件・テロップ3枚を入れ、Chromium で 1440px / 375px から5画面＋設定4タブを開いて
    横はみ出し0・JSエラー0・バッジの左右端の揃いを実測した。**バッジの右端の自動計測（`verify:ui` の
    G4）は通っていない**
  - **権限の無い利用者で 403 / 非表示になることは未確認**（権限まわりのコードは触っていないため）
  - サーバー側の文言変更（`テロップCGが見つかりません` ほか）は、**その文字列を当てにしている
    テストが無いことを grep で確かめただけ**で、テストで固定はしていない
