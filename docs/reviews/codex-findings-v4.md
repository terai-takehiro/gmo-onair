...
- **#716**（`fix(server): 決算取込で新規作成した案件の番号履歴をproject_numbersに残した`・
  2026-09-22）—— #714のフォローアップ（マージ後に届いた5件目のCodex指摘への対応。3ファイル・
  +40）。決算取込（CLI版・Web版とも）が新規作成する案件は`projects.gls_number`に番号を書く
  だけで`project_numbers`に1行も残しておらず、後日`renumberProject()`で改番すると「退役させる
  現役の番号」が見つからず旧番号が`gls_number`（新番号に上書き）からも`project_numbers`からも
  消える不具合を直した。新規作成の直後に`issueGls()`と同じ形（`entity_code=NULL`・
  `scheme='gls'`）で1行残すようにした（固定原価の疑似案件は対象外）。**Code Reviewで2件
  （P1×1・P2×1）**が付き、**いずれも同PR内で対応・返信・解決済み**。P1: 元帳の摘要が改番後の
  新方式番号（`SCS-0001`等）を含む場合でも一律`scheme='gls'`で記録しており、
  `listRenumberCandidates()`（移行センター）が「未改番」と誤検知しうる→番号の接頭辞から
  `scheme`/`entity_code`を判定するよう修正（`f5701ae`）。P2: `ensureProject`が既存案件を
  解決する経路（見つかった／削除済みから復活）では新設のバックフィル分岐を素通りしていた→
  どの経路で解決してもバックフィルするよう整理（`ffe3ec3`）。**表に移す未対応の指摘は無い**。
  ⚠️ **P2の返信で保留した「過去に既にこの穴を踏んで旧番号が失われた案件が無いか」の実データ監査**
  ——2026-09-22、terai-takehiro氏の依頼で本番DB（`onair_prod`）を実機確認した。①決算取込
  (`kessan_marker`)起因でまだ改番されておらず要バックフィルな案件: 0件。②実際に改番され
  （`entity_source='manual'`）、かつ`project_numbers`の行数が2未満＝旧番号の退役記録が無い
  （＝旧番号が失われた確度が高い）案件: **0件**。**本番で実害は確認されなかった。**
  調査に使った読み取り専用クエリ／スクリプトはブランチ`claude/great-einstein-0r2q8l`の
  `server/scripts/check-project-number-history-gaps.mjs`。
  ⚠️ **同じ調査で、決算取込以外にも同型の穴を1件新規発見（未対応・要判断）**: `excel.routes.ts`
  （案件Excel一括登録・`PROJECTS_CONFIG.insert`）は旧GLS番号付きの過去データ取込を許しながら
  （テンプレート例示行`gls_number: 'GLS001'`が明示的に案内している）、`project_numbers`への
  記録を一切していない。#714/#716はどちらも決算取込しか直していないため、この経路は今のところ
  未修正のまま。
  検証: 上記スクリプトによる本番DBの実機確認（読み取り専用・書き込みなし）。
