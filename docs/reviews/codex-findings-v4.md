...
⚠️ **上の5件は、#727 の宿題を片づける PR（この PR）で次のように扱いました**（上の段落はマージ時点の記録として残す）:

| 宿題 | この PR でどうしたか | 状態 |
| --- | --- | --- |
| ①チップの件数が案件数 | `GET /activity-logs/by-project` の返りに `summary`（`actions`＝次のアクションの件数・`projects`＝案件の数。各 `overdue`/`today`/`week`/`none`/`all`）を足し、総数と同じ CTE の上で `FILTER` 付きの集計1本にした（`summary` は `due` を無視し `search`・`user_id` だけを効かせる）。画面は5回投げていた `useDueCounts` と鍵 `activity-by-project-counts` を廃止し、一覧の返りを1回読むだけにした（`DueChips.tsx`。大きい数字＝件数・小さい数字＝案件数）。試験 `server/tests/activity-by-project-review.test.mjs` | ⭕️ この PR |
| ②`?due=` が効かない・担当者の絞り込みが無い | `?due=` / `?user=` を URL 引数にした（既定のときは引数を消す・`logParams.ts` の純関数＋`shared/tests/activityLogParams.test.ts`）。**記録者**の欄を案件別・時系列の両方に出した（最初は「担当者」と出していたが、営業担当が「担当者」と読むと**案件の担当**を思い、「自分の案件なのに出ない」になるので、中身に合わせて「記録者」に直した）。**記録者＝活動を記録した人（`activity_logs.user_id`）**で、時系列の一覧・「今後の予定」・MCP の `list_activity_logs` と同じ意味に揃えた（案件別だけ案件の担当者 `projects.assigned_to` に変える案もあったが、並びを切り替えた瞬間に同じ `?user=` で別の集まりが出るので採らなかった。要るなら別の引数名で足す）。人の一覧は `GET /users`（`/auth/users` は `AUTH_MODE=password` で 400 のため使わない） | ⭕️ この PR（案件の担当者での絞り込みは未対応・要望待ち） |
| ③「あと3日」「今日が期限」などの口語 | 全アプリの期限の注記を「残りN日／本日／明日／N日超過」に置き換え、`scripts/check-ui-tokens.mjs` の `overdue-wording` に口語の型を足した（言い換えは `docs/wording.md` の表）。**「過ぎた／過ぎている」系の画面文言も同じ PR で片づけた**（「期限を過ぎたもの」「返事の期限が過ぎているもの」「期日を過ぎたものが上に来ます」ほか15か所を「期限超過」「期日超過」「超過している」に。検査も「期限／期日／予定日／見直し日／本番日／締め日／返す予定の日 ＋ を・が ＋ 過ぎ」の型まで広げ、「言い過ぎ」「多過ぎ」のような期限と関係ない語には当たらないことを実測した）。**残したもの**: 「期限なし」（約20か所。正は「期限未設定」だが口語ではなく範囲が広い） | ⭕️ この PR（「期限なし」だけ 🔺・次の PR で置き換えと検査の追加を同時に行う） |
| ④migration 177 の通知テンプレートの「過ぎています」 | migration 306 で `inv_late`・`eq_return`・`inv_send_todo` の本文を、**177 の元の文字列と完全に一致する行だけ**置き換えた（「知らせと文面」で人が書き換えた本文は上書きしない・2回目以降は0行）。条件の文字列が 177 とずれて黙って0行になるのを試験で止めた（`server/tests/notification-template-wording-review.test.mjs`）。本番・検証で人が書き換えた行があるかは未確認 | ⭕️ この PR |
| ⑤権限のない利用者での 403・非表示 | v-none・v-keiri・v-reader・v-sales・v-manager・v-admin で API（9つの口）と実ブラウザ（375px・1440px）を測った。API は契約どおり。画面は**押せるのに 403** が4か所あった: 営業活動記録の「完了」「延期」（reader の全行）・やり取りタブの書く枠・お客様詳細のやり取りの履歴の「完了」「延期」・案件詳細のステージの帯（確認ダイアログの先で 403）。前の2つは完了・延期の口と書く枠を editor にだけ渡す形に、後の2つは統合時に同じ形（お客様詳細）と「押せなくして理由を `title` で添える」形（ステージの帯。表示を兼ねるので隠さない）にした。`shared/tests/clickable403.test.ts` に2件足した。あわせて、営業活動記録の「完了」「延期」のボタンが**32px でタップ領域 44px に届いていなかった**のを `min-h-tap`（PC は元の高さ）に、延期の取り消しが「×」1字だったのを「キャンセル」に直した。後の2つも実ブラウザ（375px・1440px）で reader は「完了」「延期」0件・ステージの帯8つがすべて押せない（押しても書き込みの通信0件・403 は0件）、editor は従来どおり押せることを確かめた | ⭕️ この PR |
