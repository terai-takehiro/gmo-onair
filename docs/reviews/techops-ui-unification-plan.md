...
> **種類**: 現役の計画
> **状態**: 段0＋ブロック A〜G 実施済み（2026-09-08・PR #647）。残り H〜O（表本体・本番4画面・AI/Excel 部品・`EditorPage`・shared の `input.tsx`・`page-shell-missing` の他アプリ展開・`verify:ui` の対象追加）は判断待ち／別の段
...
### あえて塞いでいない抜け道（自己監査・2026-09-08）

Codex の指摘が4回とも「その決まりが禁じたはずのものを、その決まりが許す」形だったため、
残りの抜け道を自分で洗った。**塞いだもの**と**あえて塞がなかったもの**を分けて記す。

| 抜け道 | どうしたか |
| --- | --- |
| `pb-[env(safe-area-inset-bottom)]`（Tailwind の任意値で書いた同じ壊れ方） | **塞いだ。** 使用箇所は 0 だが、別の綴りで通る状態を残さない。`calc(… + env(…))` の足し算はシェル側の正しい形なので当てない |
| `components/` 配下の `…Page.tsx`（ルートではない部品） | **塞いだ。** `page-shell-missing` を `pages/` 配下に限った。いま該当は 0 だが、置かれた日に誤検知になる |
| **`max-w-[900px]` のような任意値の幅** | **あえて塞がない。** 対象の `…Page.tsx` を調べると、当たるのは列やセルの幅（`max-w-[240px]` `max-w-[160px]` `max-w-[200px]` の5件）ばかりで、**画面の外枠ではない**。一律に止めると「ダイアログを広げただけで見当違いの案内が出る」という最初の指摘と同じ誤りを繰り返す。制作技術支援では `page-shell-missing` が外枠側を押さえる（`max-w-[900px]` を手書きしても `<PageShell>` が無ければ止まる）。他3アプリは下表の N で扱う |
| 文字列に `'<PageShell>'` と書いて検出をだます | **塞いだ。** `blankComments(text, { strings: true })` で**文字列の中身も潰した写し**を作って判定する。テンプレートの `${…}` はコードなので潰さない |
| `import { PageShell as Shell }` と別名で使う | **塞がない。** 検出できないが、いま該当は 0 で、起きても誤検知（止まりすぎ）側に倒れる |
| `const x = <PageShell/>;` と書いて**返さない** | **塞がない（正規表現の原理的な限界）。** この検査が言えるのは「部品がコードに現れる」までで、「外枠として包んでいる」ではない。構文解析（TypeScript の AST）にすれば言えるが、いまは正規表現1本の作りなので入口を2つにしない。**間違えて踏む種類の誤りではない**（意図的に回避しないと起きない）と判断した |
