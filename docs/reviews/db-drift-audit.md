...
1. ✅ **行数確認（dev・本番の両方・2026-08-19 完了）**: ユーザーがVPSで実行。
   **dev・本番で結果が完全に一致**（23テーブル中18テーブル＋未追跡列2つは**dev・本番とも0行**。
   ...
   `manual_parts=12` は `docs/roadmap.md` の「部品12種」の定義と一致するため
   **固定マスタ値の可能性が高い**（消えても実害が小さい）。一方
   `project_changes=16`（案件の変更履歴16件）・`manuals=1`＋`manual_layouts=1`
   （運営マニュアル1冊とそのレイアウト図1件）・`slack_dm_settings=1`
   （誰か1人の個人設定）は**中身を見るまで業務記録かどうか判断できない**。
