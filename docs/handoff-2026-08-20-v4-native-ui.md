...
## ⚠️ 最優先で確認すること — `main` に重複・競合する作業が既にマージされている
...
1. `main` を fetch し、`git diff f94ca97..origin/main -- <ファイル>` で上記4点を
   実際に開いて比較する
2. **すぐにマージしない。** まずユーザーに「2セッションが同じ画面を別々に直した」ことを
   報告し、顧客360とタスクガントの方針（どちらを採用するか）を決めてもらう
3. 方針が決まったら、このブランチに `main` を取り込み（`git merge origin/main`）、
   コンフリクトを解消してから動作確認・pushする
4. その後で初めてPRを検討する（現時点でPRは出していない）
...
⚠️ 上記はすべて `main` との重複を発見する**前**の検証。`main` を取り込んだ後は
再度フルの検証（typecheck/lint/test/build/check:frozen）をやり直すこと。
