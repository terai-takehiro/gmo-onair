...
APPS = [
    ("案件管理", "Project & Finance", icon_chart, GMO_BLUE, BLUE_LIGHT,
     "提案 → 受注 → 制作 → 請求まで、案件のお金と進捗をまるごと管理",
     ["売上・仕入・損益をリアルタイム集計", "ヨミ〜完了のステージ管理", "GLS番号の自動発番・請求書PDF"]),
...
def s_safety(n):
    ...
    text(s, 1.25, 5.42, 11.0, 0.4, [("ルールで守る本番環境", 13, GREEN, True)])
    text(s, 1.25, 5.82, 11.0, 0.6,
         [[("● 本番と検証のデータベースは完全分離   ● 「本番に入れて」の明示指示があるまで本番反映しない", 12, INK, True)],
          [("● 3時間ごとに自動バックアップ   ● いつでも以前のバージョンに戻せる", 12, INK, True)]],
         line_spacing=1.2)
    footer(s, n)
    return s
...
def s_numbers(n):
    ...
    text(s, 1.2, 5.15, 11.0, 1.1,
         [[("見積から本番・請求まで、", 17, INK, True),
           ("バラバラだった制作業務を“1つのOS”に。", 17, GMO_BLUE, True)],
          [("しかも、その進化は社長や現場との対話そのものから生まれ続けています。", 14, INK, False)]],
