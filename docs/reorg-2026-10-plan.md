...
>   **SCS**（コンテンツスタジオ）／ **GSS**（サムライスタジオ）に変える。すでに GLS が付いているものは
>   安全のため**通知して手動で変更**する
...
2. **案件番号は `SCS-0001` / `GSS-0001` / `GMO-0001`（決定）。回コードは末尾に `-001`（`SCS-0001-001`）、請求キーはさらに税枝番（`SCS-0001-001-1`）。**
...
| 売上を立てる会社 | グローバルスタジオ | グループ外の案件 → **コンテンツスタジオ（SCS）**／ グループ内の案件 → **サムライスタジオ（GSS）** |
| 案件番号 | `GLS-A###`（スタジオ案件）／ `GLS-B###`（プロジェクト） | `SCS-…`／`GSS-…`（旧A）／ `GMO-…`（旧B） |
| GLS-B（プロジェクト管理） | 自社の事業。売上・仕入・見積・請求あり | **グループ本体の事業担当へ移管。コストセンター**（売上なし・コスト管理のみ） |
| 財務管理 | 1社ぶん | **2社ぶんをタブで切替**。既存データは GSS が引き継ぐ。SCS は新規に立ち上げ |
| 資本関係 | — | コンテンツスタジオ 100% → サムライスタジオ |

2026-09-06 に確認できたこと（§9 の ✅）: 番号は `SCS-0001`・回コードは末尾 `-001`、**2社間の社内取引は発生する**
（SCS が受けた案件を GSS のスタジオ・人員で作る → ONAiR に載せる・§4.12）、GLS-B は**走っているもの全部**を GMO へ、
GMO のコスト管理は最小案（稟議・固定資産は本体の別システム）、**コンテンツスタジオは 2026-01 設立の既存法人で
切替前に SCS 名義の見積を出してよい**、請求書番号は発行者ごとに分ける。

同日の2回目で決まったこと: **社内取引の金額は手入力（見積の原価行の合計を初期値）で、GSS → SCS の請求書 PDF も
...
| 概念 | 画面に出す語（候補） | 理由 |
|---|---|---|
| SCS／GSS／GMO の区分 | **計上会社**（副題「売上・費用をどの会社の帳簿に載せるか」） | 会計の標準語・4文字。「会社」だけだと取引先（`companies`）と衝突する |
| 各社の呼び名 | コンテンツスタジオ／サムライスタジオ／**グループ本体** | 12文字以内。正式名は初出で1度だけ。「（コスト）」は付けない —— `legal_entities.short_name`（migration 284 で seed 済み）が既に「グループ本体」で、設定「会社と切替」画面もこれを表示している。実装（2026-09-06・案件一覧/詳細/台帳の計上会社表示）でこの食い違いが見つかり、**既に出ている値に合わせた**（同じ概念が画面によって違う文字列で出るのを避ける） |
| 案件の番号 | **案件番号**（旧「GLS番号」。説明文「10月から SCS／GSS／GMO で始まる番号。それより前の案件は GLS のまま」） | GLS は特定の社名の略。帳簿・請求に出る語なので**旧番号の表示では GLS を消さない** |
...
| 列 | 例 | 用途 |
|---|---|---|
| `code` PK | `SCS` / `GSS` / `GMO` | 案件番号の prefix と同じ文字列 |
...
| `parent_code` | GSS → SCS | 連結の見方を将来足すときの親子（§9-F） |
| `number_prefix` UNIQUE | `SCS-` | 採番。`sequences.seq_name = project_{code}` |
...
「会社と切替」・`system_admin` のみ編集可）から SCS・GSS とも入力・変更できる**ようにする。
...
- **SCS** は既存の値を持たないので**空欄で作成**。**SCS 名義で見積・請求書を1通でも出す前に**、
...
- **ガード**: PDF 生成時に発行者情報が不足（住所または登録番号が空）なら、その計上会社の見積・請求書は
  **発行を止め**「発行者情報が未設定です。設定 ＞ 会社と切替 で入力してください」と画面に出す
  ...
  発行者情報の有無を条件にしない**——先取り（§5）で SCS の案件番号を採ることは発行者情報が無くても進められる

### 4.3 案件番号 — 形式・採番・履歴

**形式（決定・2026-09-06）**: `{prefix}{4桁通し}` ＝ `SCS-0001` / `GSS-0001` / `GMO-0001`。3段で1本に揃える:

| 段 | 形 | 出る場所 |
|---|---|---|
| 案件番号 | `SCS-0001` | 案件一覧・詳細・BOX フォルダ名・カレンダーの題名・検索 |
| 回コード | `SCS-0001-001`（GMO の月次ユニットは `GMO-0001-2610`） | 回・Qシート（連携キー）・見積書の件名コード |
| 請求キー | `SCS-0001-001-1`（末尾は税枝番） | 売上・仕入の行・請求書／検収書 PDF の名前 |

- **A/B の1字は無くす**（会社の prefix が意味を持つ。旧 A は SCS か GSS、旧 B は GMO）
...
- **統一の中身（決定・§9-N）: 売上・仕入は必ず回に紐づける。** 理由: いまは回を持たない売上だけ別経路で
  ...
  - **A（SCS／GSS）**: 受注（`a_won`）で回が無ければ**第1回を1件だけ自動で作る**（レギュラー・単発とも。
...
- `projects.gls_number` は**列名を変えずに「現在の案件番号」を持ち続ける**（`SCS-0001` も入る）。
...
- **先取り**（決定）: コンテンツスタジオは 2026-01 設立の既存法人なので、**切替日より前でも「実施日 ≥ 切替日」のグループ外の案件は
  最初から SCS の番号を採り、見積書も SCS 名義で出す**（§5 の `preparing`）。9月に受注する10月以降の案件が
...
- **切替日をまたぐ案件**（例: 9月と11月に回があるレギュラー）:
  ...
  - **SCS になるもの（グループ外）だけ、法人が変わるので 9/30 以前の回と 10/1 以降の回を別案件にする**
...
- `monthly_budgets` / `monthly_actual_overrides` の PK を `(entity_code, year_month)` に、
  `money_rules` は `CHECK (id='default')` を外して会社ごとの行に（`default` → GSS、SCS は GSS の写しで初期化）
...
- **2社間の社内取引**（GSS → SCS の売上と SCS の仕入）も同じ表の行として持つ。両側の対応は §4.12
- 按分グループ（`project_groups`）が SCS と GSS の案件をまたぐ場合、按分先の行は各案件の会社で書く
  （行レベルなので自然に分かれる）

### 4.6 財務管理の2社タブ

- **画面全体の切替**として `PeriodBar` の左に置く（`ForecastModeToggle.tsx` の教訓: カードの中に置くと
  カードの設定に見える）。部品は `FilterChips` 系の1本のセグメント（`SCS ／ GSS ／ GMO（コスト）`、
...
2. **改番の対象一覧**: 発番済みで「実施日 ≥ 切替日」または「B で進行中」の案件。列＝現在の番号・実施日・
   ...
   （SCS）へ請求する客先の仕事**（導出規則どおり＝客先がグループ内 → GSS で正しい）だが、
   **10月以降はコンテンツスタジオ自身が制作する**予定（現時点）——つまり計上会社が GSS から SCS へ
   **変わる**。導出規則は「客先の `is_gmo_group`」しか見ないため、この切り替わりを規則だけでは検出できない。
   **対象一覧には「客先＝自社行（SCS／GSS）になり得る取引先」を含む案件を目立たせ**（客先名に
   ...
   **人が§4.4の上書き（理由必須）で SCS に直す**運用にする。同じ客先の案件が今後も出ることを見込み、
...
3. **通知**: 主担当（`assigned_to`）へ既存の通知基盤（`notifications`＋メール）。「`GLS-A008` は 10/8 実施のため
   コンテンツスタジオ（SCS）の番号へ変更が必要です」＋ボタン。定時実行は **`scheduled_job_runs` ＋
...
| `comp-self-gms` | 名前を「自社（GMOサムライスタジオ）」に、`legal_entity_code='GSS'`。SCS の自社行 `comp-self-gjv` を追加。**取引先「GMOサムライコンテンツスタジオ」2行の名寄せ**（`3d099e40…` に統合・§2.6）を同じマイグレーションで先に行う |
...
- **取込の正規表現**: 決算取込・X-point・PDF 名修復の `GLS…` を `GLS…|SCS-…|GSS-…|GMO-…` に広げ、
...
-- sequences: seq_name 'project_SCS' 'project_GSS' 'project_GMO' / 'invoice_SCS_2026' …
```
...
SCS が受けたグループ外の案件を GSS のスタジオ・人員・機材で作るとき、**GSS → SCS の売上（社内）と SCS の仕入（社内）**が
法人ごとの帳簿に要る。載せないと SCS は売上だけ・GSS は費用だけの損益になる。

**考え方: 社内取引は「同じ案件に付く、会社の違う2行」。写しの案件は作らない。**

| 側 | 行 | 計上会社 | 相手先 |
|---|---|---|---|
| 売り手 | `revenues`（社内売上） | GSS | `companies` の自社行「GMOサムライコンテンツスタジオ」（`legal_entity_code='SCS'`） |
| 買い手 | `purchases`（社内仕入） | SCS | `companies` の自社行「GMOサムライスタジオ」（`legal_entity_code='GSS'`） |
...
- **GSS 側の請求・検収・入金は通常の流れ**（決定・§9-M）。締め画面に「社内」の印を付けて分けて見せ、
  **請求書 PDF も ONAiR から発行する**（別法人なので適格請求書。発行者は GSS・宛先は SCS の自社行）
- **案件の粗利は2通り**: 「案件全体（社内取引を除く）」＝外部売上−外部仕入 が営業の見る数字（いまの粗利と同じ意味）。
  「会社別」＝ SCS: 外部売上−外部仕入−社内仕入 ／ GSS: 社内売上−GSS の仕入。案件詳細の収支サマリーは前者を既定にし、
...
- 営業見通し（パイプライン）は外部の売上・仕入だけを数える（社内行は除外。SCS と GSS の合計が二重にならない）
...
> をそれぞれ UNIQUE で1対1に結ぶ。migration 289）を新設し、`createIntercompanyPurchase`/
...
| `preparing`（先取り） | `system_admin`（検証環境で先に） | 移行センターが開く。会社マスター・切替日を入れる。**新規発番は導出規則どおり**（実施日 ≥ 切替日のグループ外 → `SCS-`、グループ内 → `GSS-`、実施日が切替日より前 → 従来の `GLS-A`）。SCS の案件は見積書も SCS 名義。**A の改番と通知が動く**（発番済みで実施日 ≥ 切替日のもの）。財務のセグメントが出る（SCS はほぼ空）。B は一覧に出るが改番は切替日以降 |
...
- **検証環境（dev.gmo-onair.jp）は 9月中に `cutover` で動かす**。本番は P0＋P1 が入り次第 `preparing` にして、
  10月以降の受注を最初から新番号・SCS 名義で出す。切替前に、会社の導出が違う案件を直せる
  （案件一覧・詳細に「計上会社: コンテンツスタジオ（SCS）」を出す。帳簿はまだ GSS のまま）
...
| **P2 財務の2社＋社内取引** **✅ 実装済み（2026-09-06・Round 1+2）** | 8画面のセグメント（URL）・集計／締め／Excel／MCP の `entity_code`・月次予算と `money_rules` の会社化・請求書番号の系列・**社内取引**（`intercompany_links`・仕入タブの入口・案件の粗利2通り） | 10月中旬まで（最初の SCS 案件の請求が10月下旬） | 5 PR |
...
> `kessan-import.service.ts` のトークン正規表現を `SCS-`/`GSS-`/`GMO-` にも広げた
...
>
> - **migration 288**: `money_rules` を `id='default'` 固定の1行から、会社ごとの複数行
>   （`id=entity_code`）に変更。SCS/GMO は GSS の設定値の写しで初期化（§4.5 の決めごとどおり）。
...
| 社内取引の片側だけが直る・消える | SCS の仕入と GSS の売上がずれ、2社の帳簿が合わない | `intercompany_links` で1対1に結び、単独の更新・削除を 409 で止める（§4.12） |
...
- **A. 番号の形式** ✅ `SCS-0001`。回があるものは末尾に回コード `SCS-0001-001`。GMO も `GMO-0001`（ハイフンで統一）
- **B. 請求書番号** ✅ 発行者ごとの系列 `INV-SCS-2026-0001` / `INV-GSS-2026-0001`
...
- **K. コンテンツスタジオ** ✅ 2026-01 設立の既存法人。**切替前に SCS 名義の見積を出してよい** → 先取り（§4.4・§5）。
  補足（ご確認）: 9月まではサムライスタジオ（GSS）からコンテンツスタジオ（SCS）へ請求書を出す取引が実在する
...
- **M. 社内取引の値決めと請求** ✅ 金額は手入力（見積の原価行の合計を初期値）。GSS → SCS の請求書 PDF は ONAiR から発行し、同じ締めの流れに乗せる
...
マイグレーションは **280 番から**（2026-09-06 時点の最新は 279。**後日の改番（2回）で最終的に
284番からになった** — 下の実装結果の注記参照）。すべて追加のみ。

> **実装結果（マルチエージェント・2026-09-06）**: 0a〜0d とも実装し、
> `claude/2026-oct-system-migration-hu4a58` ブランチに commit 済み（PR は未作成）。
> 計画からの差分:
> - マイグレーションは **284（0a）／285（entity_code 列・NULL 可のまま追加）／
>   286（0b: 全 INSERT の配線後に NOT NULL 化）／287（0c: notification_templates の件名）**
>   の4本になった（原案の番号は280〜283だったが、後日の改番（2回）で最終的に284〜287に。
>   経緯は次の項）。285 は「列を足すだけ」に留め、NOT NULL 化は配線を終えた286で分離した
>   （`DEFAULT` は一度も持たせていない——286 が来るまでは列がただ NULL 可なだけで、
...
>   ファイル名と本文中の参照番号だけの変更
> - ⚠️ **さらに後日（PR統合後、`main`との再合流時・2026-09-06）同じ7本を282〜288番から
>   284〜290番へ、もう一度相対順序を保ったまま一律+2で改番した。** 上の改番のあと`main`へ
>   このPRをpushしてCIを回したところ、`main`側で別のPR（テロップCG再設計）が**同じ
>   282・283番**（こちらも`main`側の280・281番との衝突を解消した結果の番号）を独立に
>   使っており、CIの`check-migration-numbers`が新たな重複を検出した。この`main`側PRは
>   既に`main`へマージ済み＝検証環境へデプロイ済みだったため、前回と同じ判断基準
>   （まだどの環境にも流れていない自分の番号を動かす）でこちら側を改番した。前回の
>   教訓（依存関係のある一連のマイグレーションは相対順序を保ったまま一律にずらす）を
>   最初から適用したため、今回は`column does not exist`のような二次的な破壊は起きて
>   いない。最終番号: `legal_entities`=284・`entity_code_columns`=285・
>   `entity_code_not_null`=286・`notification_template_issuer_var`=287・
>   `money_rules_per_entity`=288・`intercompany_links`=289・`renumber_notify_template`=290
...
|---|---|---|---|
| **0a** `feat(server): 計上会社マスターと切替状態の表を足した` | 取引先「GMOサムライコンテンツスタジオ」2行の名寄せ（`ff64b0e1…` を `3d099e40…` へ吸収）。`legal_entities` に SCS／GSS／GMO の3行——**GSS は `pdf.service.ts` の `COMPANY` 定数の値（住所・登録番号）をそのまま初期値に**、`former_name`＝GMOグローバルスタジオ株式会社・`renamed_on`＝切替日。**SCS は発行者情報が空欄**（設定画面から後で入れる）。`org_transition` を `off` の1行、`companies.legal_entity_code`（`comp-self-gms` → GSS、SCS の自社行 `comp-self-gjv` を追加。名前の書き換えは切替時）。`GET/PUT /legal-entities`（発行者情報の編集含む・`system_admin`）・`GET/PUT /org-transition`（`cutover` は `cutover_date` 以降だけ・`done` は残件 0 だけ）。`migrate.ts` の起動時チェックに「3行と1行がある」を足す | 280→282→**284**（後日2回改番。上の実装結果の注記参照） | `server/src/contexts/platform/{services,routes}/legal-entity.*`・`shared/db/migrate.ts` |
| **0b** `feat(server): 帳簿の行と案件に計上会社の列を足した` | `projects.entity_code/entity_source/entity_note`・`project_numbers`（既存の `gls_number` を `scheme='gls'`・`GSS` で流し込む）・`revenues/purchases/sga_expenses/estimates.entity_code`（`NOT NULL DEFAULT 'GSS'` で埋め戻し）・`finance_docs.entity_code`（NULL 可）・`monthly_budgets/monthly_actual_overrides/money_rules.entity_code`（**列だけ**。PK の付け替えと複数行化は P2）。サーバーの **INSERT を全部 `entity_code` 明示**にして（案件の `entity_code ?? 'GSS'`）、`shared/tests/entityCodeInserts.test.ts`（`droppedColumns.test.ts` と同じ走査で `INSERT INTO revenues|purchases|sga_expenses|estimates` に列があるか）が通ったら **同じ PR で `DROP DEFAULT`** | 281→283→**285**（後日2回改番） | `sales/services/{project,estimate}.service.ts`・`finance/routes/*`・`platform/services/{kessan,xpoint}-import*`・`mcp/tools/{finance,budget}.tools.ts`・`gpm/*` |
| **0c** `feat(server,client): 帳票とメールの発行者を計上会社マスターから読むようにした` | `pdf.service.ts` の `COMPANY` 定数を `PdfRevenueData.issuer` に置き換え、呼び出し側（`revenues.routes.ts:68`・`estimate-pdf.service.ts`）が行の `entity_code` から引く。名前は `nameAsOf(entity, 発行日)`（`renamed_on` より前なら旧社名・以後は新社名＋「（旧 …）」の併記＝§9-E の提案どおり）。`notification_templates` の初期4行の件名を `【{発行会社}】` に（手で直した行＝`updated_at` が初期値でない行は触らない）・送信時に変数を解決。機材レンタルの文面・GPM の「自社（…）」・マニュアル・案件作成のメール下書きをマスター読みに | 283→285→**287**（後日2回改番） | `shared/services/pdf.service.ts`・`sales/services/estimate-pdf.service.ts`・`platform/services/notification*`・`qsheet/services/rental.service.ts`・`client/src/contexts/gpm/pages/projectForm/BasicStep.tsx` ほか |
| **0d** `feat(client): 設定に「会社と切替」の画面を足した` | 設定トップ「ルール」に新カード（`/settings/reorg`・PC 専用・`CLIENT_PC_ONLY` に登録）。**会社ごとに（SCS・GSS の2枚。GMO は請求書を出さないので発行者情報欄は無し）**発行者情報の編集フォーム（住所1/2・適格請求書発行事業者登録番号・振込先・ロゴ）、切替日、状態ボタン（P0 で押せるのは `off → preparing` だけ。`cutover` は P1 で活性化）。**GSS は最初から値が入った状態で開く**（0a で流し込み済み）・**SCS は空欄から入力**。`system_admin` 以外は読むだけ | — | `client/src/contexts/platform/pages/reorg/`・`pages/settings/hubCards.ts`・`App.tsx`・`pcOnlyScreens.ts` |
...
**コードの外でやる作業（P0 と並行）**:
- **SCS の発行者情報**（住所・適格請求書発行事業者登録番号・振込先・ロゴの有無）を、0d の画面から
  `system_admin` に入力していただく（このセッションでは値を預からない・チャットでは受け取らない）。
  **SCS 名義で最初の見積・請求書を出す前まで**に間に合えばよい（§9-K の先取り運用）
...
- 2026-09-06（同日・4回目）: 発行者情報は設定画面から入力（GSS は今の値を初期値に・SCS は空欄）に確定。
...
- 2026-09-06（同日・7回目）: **P2 Round 1（財務の多社対応。§6）をマルチエージェントで実装した。**
  ...
  2ラウンドへ分割。核（migration 288・`money_rules`/`monthly_budgets`/`monthly_actual_overrides`
...
- 2026-09-06（同日・8回目）: **P2 Round 2（社内取引 SCS⇄GSS。§4.12）をマルチエージェントで
  実装した。** 核（`intercompany_links`・migration 289・作成/編集/削除の専用経路・
...
- 2026-09-06（同日・12回目）: **PR #599 の push 後、CIの`checks`が
  `check-migration-numbers`で再び赤くなった。** `main`に別PR（テロップCG再設計）が
  マージされ、そちらも`main`側の280・281番衝突を解消した結果として282・283番
  （`282_graphics_qsheet_import.sql`/`283_graphics_follow_script.sql`）を使っており、
  前回の改番で使った282〜288番とちょうど重なった。`main`を取り込み（コンフリクトなし）、
  前回の教訓どおり今回は最初から依存する7本すべてを相対順序を保ったまま一律+2で
  284〜290番へ改番し（`legal_entities`=284・`entity_code_columns`=285・
  `entity_code_not_null`=286・`notification_template_issuer_var`=287・
  `money_rules_per_entity`=288・`intercompany_links`=289・`renumber_notify_template`=290）、
  参照番号もコード・ドキュメント全箇所（`main`側のグラフィックス関連の282/283番言及は
  対象外と正しく識別して除外）を追随させた。検証: `server`/`client`/`client-techops`の
  型チェック、`shared`のVitest 2112件、検証DBのフレッシュ再構築でmigration 1〜290番を
  通しで適用、`npm run lint`一式（`check-migration-numbers`含む）すべてgreen。
