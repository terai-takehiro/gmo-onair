...
やり残しとして意図的に残したもの (理由つき):

| 見送ったもの | 理由 |
| --- | --- |
| 案件の在席アバターと409の競合バナー | サーバー基盤 (`project_collab`) はあるが `PUT /projects/:id` が `expected_updated_at` を受けない。保存の約束に触るので別途 |
...
| 「直近7日で N件 終わらせました」 | 正しく出せる集計が無い。当てずっぽうの数字を置くほうが害が大きい |
| 権限の変更履歴 (誰が変えたか) | 監査テーブルを持っていない。「いつ」だけ出し、持っていないことを画面に書いた |
| 配布URLの無効化 | 番組終了後にURLを失効させる仕組みが無い。画面に明記した |
