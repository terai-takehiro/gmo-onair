...
| 論点 | 決定 |
| --- | --- |
| 凍結4アプリ（制作資料/Qシート・技術資料・計時LIVE・リアルタイムCG） | **対象外のまま**。CLAUDE.md の凍結方針（URLは生かし見た目は今のまま）を維持する。※その後、制作資料・計時LIVE は凍結を解いて対象になり、リアルタイムCG は 2026-09-06 に「移行中」（後継＝制作技術支援＞テロップCG）へ整理した。ネイティブUI化の対象は**後継のテロップCG側**（[design/v4/graphics-redesign.md](design/v4/graphics-redesign.md)） |
