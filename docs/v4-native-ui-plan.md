...
- **残り15画面**（中規模12・大規模3）: 取引先マスター・取引先(仕入先/パートナー)・
  探す(案件管理)・設定トップ・GPMプロジェクト一覧・GPM全タスク・ウィークリー活動報告・
  デイリーニュース・セキュリティカード・機材ダッシュボード・機材メンテナンス・貸出返却
  → 中規模。⑥案件詳細・タスク依頼(daily)・機材台帳 → 大規模。詳細は監査結果の表を参照
