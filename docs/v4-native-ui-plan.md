...
3. **`pc_only_questionable` 9画面の理由を実測で書き直す。** 顧客・取引先マスターの前例と同じで、
   実装は既にモバイル対応可能な形（`stackOnMobile` 済み等）になっているものが多い。
   1画面ずつ実機（Playwright・375px）で確かめ、開けるものは `CLIENT_MOBILE_OK` へ移す
   （小: 費用を分け合うグループ一覧・拠点部屋・権限とメンバー・通知とテンプレート・
   GPM標準工程テンプレート／中: お金のルール・休日営業時間／大: 顧客360・GPMプロジェクト詳細）

### B. 旧カレンダー2画面の機能吸収（このドキュメント上部で決定済み）

- 香盤ビュー・予約を直す導線を ① 予定に吸収 → 旧スタジオカレンダーを退役
- 取込元の詳しい色分け・外部連携設定を ① 予定／④ 設定に吸収 → 旧自分の予定を退役
- ② 部屋の空きに専用モバイルレイアウトを追加（上記「見つかった重要な誤り」参照。①予定と対になる画面なので優先度を上げる）
