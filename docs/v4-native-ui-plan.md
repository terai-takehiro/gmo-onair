...
> **状態**: 現役の計画（進行中）— バックログ A〜D（2026-08-20〜21・第1〜12バッチ）は完了。残る宿題は C・D の末尾に書いたもの
> **最終確認**: 2026-09-08（v4.6.10）
> **位置づけ**: [docs/v4-plan.md](v4-plan.md)（元の v4 刷新計画）を進めた先に、2026-08-20 にユーザーから出た新しい指示（全画面を macOS/iOS ネイティブ級に、PC 専用は原則廃止）を受けて始めた追加の取り組みの記録。v4-plan.md 自体は書き換えず、ここに積む。PC 専用の例外の正は `client/src/pcOnlyScreens.ts`。監査の元データは [v4-native-ui-audit-2026-08-20.md](v4-native-ui-audit-2026-08-20.md)、引き継ぎ書は [archive/2026/handoff-2026-08-20-v4-native-ui.md](archive/2026/handoff-2026-08-20-v4-native-ui.md)（役目を終えたので 2026-09-08 にアーカイブへ移した）。

## 決定事項（2026-08-20・ユーザー判断）

| 論点 | 決定 |
| --- | --- |
| 凍結4アプリ（制作資料/Qシート・技術資料・計時LIVE・リアルタイムCG） | **対象外のまま**。CLAUDE.md の凍結方針（URLは生かし見た目は今のまま）を維持する。※その後、制作資料（現・制作技術支援）・計時LIVE は凍結を解いて対象になり、リアルタイムCG は 2026-09-06 に「移行中」を経て段F（畳み込み）で「廃止」にした（後継＝制作技術支援＞テロップCG。用語は [v4-plan.md](v4-plan.md) の「用語」）。ネイティブUI化の対象は**後継のテロップCG側**（[design/v4/graphics-redesign.md](design/v4/graphics-redesign.md)） |
...
（詳細は [docs/archive/2026/handoff-2026-08-20-v4-native-ui.md](archive/2026/handoff-2026-08-20-v4-native-ui.md)）。
