...
**Phase C（設計変更・持ち場所の一本化）に着手した**（`docs/project-ledger-phase-c-design.md`）。
マルチエージェントで金額・受注/失注日時・実施日・書く口集約の4テーマを深掘り調査し、
金額の集計統一と実施日のDATE型化は**本番データの事前監査が必須**（このセッションからは
本番DBへアクセスできない）と判明したため保留し、それ以外の安全な項目を実装した。
`recordStageTransition()`共有関数を新設して案件・GPMのステージ変更（履歴・
`won_at`/`lost_at`書き込み）を1本に集約し、**GPM経由の見送りが`lost_at`/`lost_reason`を
一度も書いていなかったバグ**（Phase Aで`won_at`だけ直し`lost_at`が未修正のまま残っていた）
を修正した。GPM新規作成の初回ステージ履歴も追加。`project.service.create()`を
`createCore(tx, ...)`＋薄いラッパーに分割し（既存呼び出し元は無変更）、投入口（`neta`起票）を
これに移行——**投入口が独自に`customer_type`を`'external'`固定していたバグ**
（グループ会社からの投入がグループ外扱いになっていた）も副産物として修正した。
台帳の一括編集に受注/失注への直接変更を拒否するガードを追加。ダッシュボード
（`sales-board`/`ai-inbox`）と週報の行ごとの金額表示を、他の一覧と同じ「見積があれば見積・
無ければ想定」に揃えた。Excel取込の想定金額0上書きバグ（セルが空だと既存値を無条件で0にする）
と実施日の型バグ（`asString`のまま取り込みExcelの日付セルが数値文字列化する）も修正した。
検証: `npm run typecheck:all`・`npm run lint`・`npm run test`（1522件）緑。migrationは無し
（列の型・存在は変えない）。実サーバー（検証用Postgres）で案件作成・受注/失注ステージ変更・
台帳一括編集のガード・GPMの受注/失注（`lost_at`修正の実地確認込み）・投入口の
`customer_type`修正（`is_gmo_group`会社での実地確認）・ダッシュボード/週報への
`estimate_amount`追加を確認済み。
