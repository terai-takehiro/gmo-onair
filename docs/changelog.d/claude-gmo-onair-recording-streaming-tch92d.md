...
載せ替えは別 PR に切ることを推奨として書いた。⑥**モックアップのレイアウトを実ブラウザで実測して直した**（ご指摘「レイアウト崩れには細心の注意を／2行になってはみづらいもの／長体も活用しレイアウトを維持」）。
Playwright で枠のサイズどおりに描き、①値が2行になっていないか ②…で切れていないか
③枠から下が切れていないか ④タップ対象44px、を機械で数えたところ **49 件**の不備があった。
⚠️ いちばん大きかったのは **`height: 100%` が効いていなかった**こと（`support.js` の受け皿
`#dc-root` に高さが無い）で、スマホ画面が枠から **196px あふれていた** — 根の要素を `100vh` にした。
⚠️ **`transform: scaleX()` だけの長体は文字数を増やさない**ことも実測で判明した
（`text-overflow: ellipsis` は変形前の幅で切る位置を決めるため、字が細くなるだけで
収録先が「ネットワ…」のまま切れていた）。**枠を 1/縮小率 だけ広げてから縮める**実装に直し、
横並びでは受け皿でくるむ形にした。あわせて列幅（ファイル名 140→168px・収録先 136→154px）を
実測値から決め直し、タップ対象 44px 未満を **107 件 → 0 件**にした。残りは **4 件**で、
いずれも注記が2行になっているだけ（値の折り返しは 0 件）。決めごとは
`docs/design/qsheet-recording-streaming/mockups/README.md` に残した。
検証: 文書とモックアップのみでコード変更なし。`npm run lint` の自作検査
