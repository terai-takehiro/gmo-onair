**案件フェーズに「実施済（財務処理中）」を追加し、財務処理前に完了フォルダへ入らないようにした**。
①⚠️ **受注後は日次ジョブが `event_end` の翌朝に無条件で「完了」へ繰り上げ、
同じジョブが直後に BOX の `98_終了案件` フォルダへ移していた** — 請求・入金など
財務処理が済んでいなくても翌朝には完了フォルダに入ってしまい、財務側が
「完了フォルダにあるのにまだ処理していない」と分かりづらい状態だった
（`project-health.ts` の `completeElapsedWonProjects()`）。
②新ステージ `r_delivered`（R 実施済）を「A 受注済」と「S 完了」の間に追加。
受注確定の扱い（`WON_STAGES`）には含めつつ、BOX の `98_終了案件` への移動は
従来どおり `stage = 's_completed'` の完全一致でしか発火しない状態を維持し、
日次ジョブの繰り上げ先を `s_completed` から `r_delivered` に変更した
（`s_completed` への昇格は財務が案件詳細から手動で行う運用にした）。
③一覧のバッジ・チップ・案件詳細のステージ変更ボタン・GPM（プロジェクト管理）の
同種のUI・ファネル分析・ダッシュボードの各種「進行中案件」集計・営業活動記録の
次回アクション自動クローズ・DB の CHECK 制約（migration 271）を一通り追随させた。
「進行中」「行を薄くする終了扱い」からは意図的に除外し、独立した警告色のバッジで見せる。
検証: `npm run typecheck`（client/client-daily/client-equipment/server）・
`npm run test`（shared、1964件）全通過。
④仕上げのレビューで見つけた見落とし2件を追加で直した:
AIフィードバック集計（`ai-feedback.service.ts` の `get_ai_feedback_digest`）の
受注/進行中の集計SQLに `r_delivered` が入っておらず、実施済（財務処理中）の
案件だけ「進行中」に数えられていたのを他箇所（`WON_STAGES`）と揃えて修正。
検証環境のシードデータ（`seed.ts` の GLS-A001）を `s_completed` から
`r_delivered` に変更し、新ステージのバッジ・フォルダ挙動を実ブラウザで
見分けられるサンプルを用意した（GLS-A008 は `s_completed` のまま残し、
BOX の `98_終了案件` フォルダ移動の確認用に維持）。

---

**見積作成時、グループ内案件などの定価差から値引き項目行を自動生成できるようにした**。
これまでカタログ選択時の定価と実額の差（`list_unit_price` > `unit_price`）は
画面のカテゴリ小計・見積書PDFだけがその場で動的に合成する「表示専用のゴースト値引き」で、
`estimate_items` には実体を持たなかったため、**見積を受注して売上に変換すると
（`revenue_items` に定価列が無く）値引きがどこにも残らず、請求書・検収書には
定価差の値引きが一切出ていなかった**。既存の「カテゴリに値引き行を追加」ボタン
（`EstimateItems.tsx` の `addCategoryDiscount`。中身は新設の
`estimateCategoryDiscount.ts` に切り出し）を拡張し、押した時点でそのカテゴリの
定価差合計を自動計算して単価に入れ、説明欄を「グループ価格による値引き」で埋める。
**同時に、差額のもとになった行の単価を定価まで引き上げる**（値引きは単価を
下げず別建て、という既存方針どおりに組み替えるだけで、カテゴリの合計・粗利は
1円も変わらない — 引き上げないと実額の行＋新しい値引き行で二重に値引いたことになる）。
生成後は他の行と全く同じ通常の明細行として保存されるので、既存の保存・見積PDF・
売上変換（`convertToRevenue`）・請求書/検収書PDFの経路がそのまま通り、金額・項目名も
自由に編集できる（サーバー側の変更は無し）。ボタンの文言に自動計算した金額を出し、
差が無いカテゴリでは従来どおり空の値引き行（手入力用）にフォールバックする。
自動計算は押した瞬間の初期値の提案のみで、保存のたびに再計算はしない（編集・削除した
値引き行が復活しない）。⚠️ **実装中にテストで見つけて直した実害**: 「押したら入る額」の
判定を画面のカテゴリ小計と同じ `categoryTotals()`（定価小計−実額小計）に頼ると、
一度自動生成した値引き行そのもの（単価マイナス）も実額側に混ざって同じ差額を
「まだ材料化していない」と誤認し、**ボタンを2回押すと値引き行がもう1本増えて
実質二重値引きになる**不具合があったため、`list_unit_price > unit_price` の行だけを
直接合算する専用の判定に直した（`shared/tests/estimateCategoryDiscount.test.ts` で固定）。
検証: `npx tsc -b client`・`npm run typecheck -w server`・`npm run test`（shared、
1971件）全通過。

---

**「案件を直す」で無関係な項目まで保存できないケースを直した**（継続区分など）。
保存ボタンの disabled 判定（`missingOf()`）は、客入れの有無・案件分類の
**片方だけ**入っている（古いデータ・他経路の書き込みなど）案件を開くと、
継続区分のような全く無関係な項目だけ直そうとしてもこの2項目を埋めない限り
常に押せないままだった。サーバー（`resolveClassification`）は2段が渡されな
ければ今の値を保つだけで、渡された側だけでは導けず旧 `project_type` から
毎回導き直す設計のため、触っていない古い分類を送らずに他の項目だけ保存して
も壊れない。`missingOf()` に `touchedClassification` を追加し、
react-hook-form の `dirtyFields` から「このセッションで実際に触ったか」を
渡すようにした。触っていなければ片方だけの状態でも通し、触ったときは今まで
どおり両方を要求する。第3引数の既定は `true`（今までどおり常に見る）なので
案件作成画面・既存呼び出しの挙動は変えていない。

---

**GLS発番の番組種別に「リアルイベント」を追加した**。配信・収録を伴わない
リアルだけのイベント向けに `BroadcastType` へ `real` を追加。
`projects.broadcast_type` は CHECK制約の無い TEXT列（カンマ結合の複数選択）
のため migration は不要。`shared/src/enums.ts` と `client/src/types/index.ts`
の両方に追加した（GLS発番ダイアログ・案件フォームの番組種別トグルは
`BroadcastTypeLabels` を動的展開しているため、ラベル追加だけで選択肢が
自動的に出る）。`real` は生放送判定（`broadcastTypeIncludes(...,'live')`）
の対象外のため、回（エピソード）の放送日は既存の「収録」相当の計算
（収録日+オフセット日数）に流れる——リアルイベントの放送日の扱いは別途の
仕様判断が必要なため、今回は番組種別への追加のみに留めた。

---

**案件分類・継続区分のリロード反映漏れ（再発）を横断的に塞いだ**。
v4.5.20 で「案件詳細・案件を直す・案件台帳」の3画面を直したはずが、保存
しても次に開いたときに反映されず、もう一度リロードすると反映される不具合
が再発していた。原因は一元化された `invalidateProjectQueries()`
（`contexts/sales/projectQueries.ts`）を使わず自前で鍵を並べたまま漏れが
残っていた箇所が複数あったこと: ①案件作成画面（受付レールから既存の
「ネタ」案件を選んで保存する経路・`useCreateProject.ts`）②案件台帳の一括
編集・1件削除（`useLedgerState.ts`）③案件台帳のその場で直す・貼り付け
（`useLedgerGrid.ts`）。いずれも `invalidateProjectQueries()` を呼ぶよう
統一し、鍵を手書きしていた箇所を残らず一元化した。あわせて
`invalidateProjectQueries()` 自体が一覧・1件ぶんの鍵を漏れなく落とすことを
固定する検査（`shared/tests/projectQueries.test.ts`）を新設した。
検証: `npx tsc -b client`・`npm run typecheck`（既定3アプリ+server）・
`npm run test`（shared、146ファイル・1996件）全通過。

---

**取引先マスターに「与信限度額」「最新与信確認日」を登録できるようにした**
（登録は任意）。対象は `companies` テーブルを直接編集する `/sales/companies`
（`CompanyListPage.tsx`）— `customers`/`vendors` は migration 208 で削除済みで
`companies` が唯一の正のため、`credit_limit_amount INTEGER NULL`（円）・
`credit_check_date DATE NULL` を追加する単純な1本（migration 272）で足りた。
金額欄は既存の `CurrencyInput` を使うと空欄でも 0 が送られ「未設定」と
「与信枠0円」を区別できない実装上の欠陥があるため使わず、`DiscountLimits.tsx`
と同じ方式（プレーンな `<Input>` ＋ 文字列 state、送信時に空文字→null へ
変換）で実装した。サーバーは他の任意項目と同じ「渡されなければ現在値を保つ」
`keep()` パターンを踏襲。一覧（PC行・スマホカード）は固定列の幅設計が
既に実測で詰められているため追加せず、`CompanySummaryDialog`（収支サマリー）
にのみ表示する。

---

**財務ダッシュボードの仕入・販管費の内訳行から、そのまま編集できるようにした**。
以前は行を押すと台帳（明細一覧）へ遷移するだけで、そこから改めて対象の行を
探して編集する必要があった。ダッシュボードに編集フォームを作り直すのではなく、
遷移先の URL に `?edit=<id>` を足し、**台帳側の既存の編集ダイアログを開いた
状態**で着地するようにした（`ledgerOpenQuery` に `editId` 引数を追加）。
`SgaListPage` は元々 PDF取込レビュー画面用にこの仕組みを持っていたため
そのまま乗り、`PurchaseListPage` には同じ形で `?edit=` ハンドラを新設した。
「行＝台帳へ遷移」という直前の決定（仕様変更 #4）はそのまま保ち、遷移した
先で即座に編集できるようにしただけなので、売上・仕入・販管費で挙動が
また食い違う、という後戻りは起きていない（売上は編集要望が無いため対象外）。

---

**仮押さえスケジュールに「何番手の仮押さえか」を登録できるようにした**。
`studio_bookings` は同じ部屋・同じ時間帯を複数の仮押さえ（`status='tentative'`）
が取り合うことを妨げない設計（一意制約なし）で、営業が「うちは2番手」を
把握する手段が無かった。`hold_rank INTEGER`（1以上、任意）を追加
（migration 273）。**人が都度手入力する単独の整数値**として持たせるだけに
留め、自動算出・自動繰り上げは行わない — 同一部屋・重なる期間の件数から
自動算出する案は、先着の仮押さえが確定/削除されたときに後続の番手を
自動で繰り上げるかどうかという別の問題を増やし、データの正が二重化する
ため。予約作成・編集ダイアログ（`StudioBookingDialog.tsx`）の「予約状態」
セクションに、`status==='tentative'` のときだけ表示される数値入力欄を追加。
サーバーは既存の部分更新原則（`PUT /studios/bookings/:id`）に1行で乗せ、
渡さなければ今の値を保つ・明示の空値/0以下だけ NULL に落とす。確定
（`confirmed`）に切り替えても値は自動で消さない（過去に何番手だったかの
記録として残せる）が、番手は仮押さえ一覧（`HoldListPage`/`HoldCards`）と
このダイアログの tentative 時にしか出さないため、確定後に画面上へ
残って混乱を招くことはない。MCP `create_studio_booking` にも
`hold_rank`（tentative のときのみ有効）を追加し、`list_studio_bookings`
の要約列にも含めた。
検証: `npm run typecheck`（既定3アプリ+server）・`npm run test`
（shared、146ファイル・1996件）全通過。
