...
## 現在のバージョン
v2.8.34 (dev) — **モバイル スクロール不能 Fix + レスポンシブ強化**: ユーザー報告「スマホで編集画面がスクロールできない」+「レスポンシブが残念」に対応。①EditorPage の body 親 div を `overflow-hidden` から `overflow-y-auto lg:overflow-hidden` に変更 — lg 未満では親自身がスクロール担当、lg 以上では従来通り CueTable 内 `<main overflow-auto>` が縦スクロールを担当。これでモバイル CueCardList が縦長になっても縦スクロール可能に。②CueCardList のセクションヘッダ ↑↓ 削除ボタンを `p-1 size-3.5` → `size-9 inline-flex items-center justify-center size-4` に拡大、iOS HIG の最小タップ領域 (44px ≒ size-9 = 36px+ padding) を確保。③row 右サイドツールバーを `px-2 flex-1` → `w-11 shrink-0 min-h-9` に変更し、3 ボタン (↑/↓/削除) それぞれが 36px 以上のタップ領域を持つ独立 area に。アイコンも `size-3.5` → `size-4`。④CueCardList のセクションヘッダで input に `min-w-0 flex-1` を付与し、ラベルが長くてもボタン領域が圧迫されない構造に修正、各バッジ・ボタン群に `shrink-0` を明示。データ・Socket.IO 影響なし。

(v2.8.33: InsertGap UI 安定化) ユーザー報告「UI が崩れる時がある」に対応。原因: v2.8.32 の InsertGap が `h-1.5 hover:h-9 transition-[height]` で hover 時に高さが変化していたため、layout shift + `group-focus-within` での opacity 残留により「ボタンが出っぱなし + 周囲レイアウトがガタつく」状態が発生していた。修正: ①InsertGap を**固定高 28px** (`h-7`) に変更、layout shift を完全排除。②デフォルト表示は中央に小さな `+` アイコン (`size-5 rounded-full`) のみ、hover/focus 時に `opacity` のみで 3 種ボタン (ロール/CM/VTR) を切替表示。③点線 (border-dashed) で挿入位置を常時可視化、hover 時は `border-primary/40` に。④モバイル CueCardList の InsertGap も同じ視覚言語 (固定 28px、点線 + 3 ボタン常時表示) に統一。インデックス計算と挿入機能は v2.8.32 と同等。

(v2.8.32: CM/VTR 任意位置挿入 + ロール/行 DnD) ユーザー要望「CM/VTR を一番下だけでなく、ロールの間にも常に入れられるようにしたい / ロール単位でドラッグ&ドロップ / ロール内の行単位でドラッグ&ドロップで移動」に対応。①CueTableLg (デスクトップ) の各セクション間に **`InsertGap`** を導入: ホバーで 1.5px → 36px に拡張し、ロール/CM/VTR 3 種ボタンが現れる。先頭にも 1 つ配置（最上部への挿入）。`insertSectionAt(idx)` 新設、既存 `addBreak/addVtr(afterIndex)` を流用。②セクションヘッダ全種 (CM / VTR / ロール) を **HTML5 native draggable** 化: `dataTransfer.setData("text/x-section-idx", ...)` + `moveSection(from, to)` で `splice` ベースのリオーダー。drop ターゲットには `outline outline-2 outline-primary` で視覚フィードバック。③`CueRow` に行 DnD prop を追加 (`isRowDragged`/`isRowDropTarget`/`onRowDrag*`)、内部の `<tr>` に dragHandlers + cursor-grab + drop ターゲット時 `outline-primary` 表示。`CueTable` 側で `moveRowTo(si, fromRi, toRi)` をハンドルし同一ロール内で並び替え。④CueCardList (モバイル) にも同等の `InsertGap` 配置 + セクションヘッダに **↑↓ ボタン**追加（タッチ DnD は不安定なため明示ボタン）。CM/VTR セクションは `bg-warning/5` / `bg-info/5` で識別、行リストと行追加 footer は非表示にして代わりに尺 input を表示。doc.data JSONB 構造完全維持、Socket.IO 影響なし、データキー名変更なし。 ①EditorSidebar の `LED` タブを廃止し、4 タブ → 3 タブ (`列 / マスター / メタ`) に再編成。LED/XR シーン・立ち位置図・マスタデータを `マスター` タブ内で **`ResourceCard`** という統一カード UI で並列表示（アイコン + タイトル + 件数バッジ + アクション）。②`MasterSection` の pill デザインを refined（旧 `font-bold` → `font-medium` + `shadow-sm` + `pl-2.5 pr-1`、削除ボタンは `size-3.5 rounded` + `aria-label`）。③`LedSceneSection` 壁/床表示を 2 カラムグリッドの**ミニカード**化: `bg-primary/5 border-primary/20`（壁）と `bg-warning/5 border-warning/20`（床）の semantic 反対色で識別、`9px uppercase tracking-wider` ラベル + 中身を inline 編集。シーン番号バッジを `size-5 bg-primary/20` 円形に。④CueRow.tsx 壁/床インライン表示も同じ 2 カラムグリッド形式に統一（`8px` ラベル）。⑤CueRowMobileEditor.tsx LedXrEntry に**壁/床表示を追加**（従来は欠落していた）— シーン選択時に自動で読み出し表示。Socket.IO + doc.data JSONB 構造完全維持、機能変更なし。
