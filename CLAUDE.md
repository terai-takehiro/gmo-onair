...
## 現在のバージョン
v2.8.95 — 表彰CG/ランキングCG 出力 URL のログイン不要アクセスを完全修復 + 表彰CG output を viewport スケール対応化。ユーザー報告 3 件への対応。①**根本原因 = Express router middleware leak**: `events.routes.ts` / `categories.routes.ts` / `entries.routes.ts` / `oneshot.routes.ts` が `router.use(requireAuth, requirePermission('awards'))` (パス無し) で auth ガードを掛けていた。この書き方だと **router 内のすべてのリクエスト**で middleware が発火するため、`/awards/images/...` (imageRoutes 担当) 等の他 router 担当のパスがこれら router を通過する際に matching route が無くても requireAuth が走り、未ログインだと 401 で蹴られていた。これにより a) ランキングCG の画像 (`<img src="/api/v1/internal/awards/images/X.jpg">`) が未ログインで表示されない、b) 表彰CG の output URL (`/awards/output/:id/oneshot` → `fetch('/awards/events/:id/oneshot/output')`) が**そもそも 401 で event=null** → 何も描画されない、という 2 つの異なる症状の同根バグになっていた。 ②**修正**: 4 router の `router.use(requireAuth, ...)` を **path-scoped 配列形式**に変更: `router.use(['/events', '/box-backups'], requireAuth, requirePermission('awards'))` (events.routes), `['/events', '/categories']` (categories.routes), `['/categories', '/entries']` (entries.routes), `['/events', '/entries']` (oneshot.routes)。これでパスが一致しないリクエストは middleware を素通りして次の router に流れる → imageRoutes が `/awards/images/...` を正常に処理可能。`cues.routes.ts` は per-route で `router.post('/...', requireAuth, ...)` 形式のため漏れなし、無修正。 ③**OneShotOutputPage を viewport スケール対応**: ユーザー指摘「下位置のCGはランキングCGのように1920×1080で出力できていない気がします」に対応。v2.8.84 で position:fixed + 1920×1080 固定にしていたが、これでは OBS の 1920×1080 browser source 以外 (通常ブラウザで開いてプレビューする等) では viewport からはみ出して見える範囲が無かった。ranking CG `OutputPage`/`Stage` と同じ方式 (`Math.min(w/CG_W, h/CG_H)` で scale 計算 + offset で中央寄せ) に変更。OBS の 1920×1080 source では scale=1 / offset=0 で 1:1 ネイティブ表示 (従来挙動を維持)、それ以外のブラウザでは自動縮小プレビューが効く。`window.resize` で再計算。
