...
## 現在のバージョン
v2.9.54 — **案件管理一覧: 開催月絞り込み (既定=今月) + 表示順を「提案中 → 受注済 → その他」に変更**。①**開催月絞り込み** (`ProjectListPage` + `project.service.ts`): `<input type="month">` を検索バー隣に追加し、既定で今月に絞り込み (「解除」で全件)。サーバーは新 `event_month` param (YYYY-MM) で、イベント期間 `[event_start, event_end]` が対象月に**重なる**案件を抽出 (`event_start < 翌月初` かつ `COALESCE(NULLIF(event_end,''), event_start) >= 月初`、TEXT 日付の文字列比較)。`event_start` 未設定の案件は月絞り込み中は非表示。②**既定ソート変更** (`DEFAULT_SORT_SQL`): 旧「完了/失注を最後 + イベント日順」→ **ステージ順位 = B口頭(1) → C提案(2) → D保留(3) → S完了(4) → A受注(5) → E失注(6) → ネタ(7)**、同順位内はイベント開始日 ASC NULLS LAST → 作成日 DESC。③**一覧の 3 区分表示**: 旧「進行中 / 完了・失注」の 2 分割を「**提案中** (B/C/D) / **受注済・完了** (S/A) / **その他（失注・ネタ）**」の 3 セクション (各区切りヘッダー付き) に再構成。薄表示 (terminal) は失注のみに適用。ソートプルダウンの既定ラベルも「おすすめ (提案中 → 受注済 → その他)」に更新。DB スキーマ変更なし。)
