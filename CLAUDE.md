...
## 現在のバージョン
v2.8.93 — ランキングCG ステップピッカー説明テキストの視認性改善 + PROGRAM OFF オーバーレイの読みやすさ向上。ユーザー報告「ランキングCG コントロールページのステップピッカー (IDLE / TITLE / NOMINEES / RANKS 5→2 / BEST 3 / WINNER BAR / ONE SHOT) の各ボタン下の日本語説明 (タイトルカード / ノミネート一覧 / ランキングバー / 一覧から TOP3 一気発表 / 大賞引きバー / 大賞フルスクリーン) が極めて読みづらい」(スクリーンショット添付) に対応。`ControlPage.tsx` line 299 の `<span className="text-[9px] text-slate-500 mt-0.5 leading-tight">{desc}</span>` を `text-[10px] font-medium text-slate-300 mt-1 leading-tight` に変更: ①フォントサイズ 9px → **10px** で 1px 拡大、②色 `text-slate-500` (#64748b、AA 不適合) → **`text-slate-300`** (#cbd5e1、AAA 準拠の高コントラスト)、③`font-medium` (500 weight) を追加してウェイト底上げ、④`mt-0.5` → `mt-1` で英ラベルとの行間を 2px → 4px に広げて視覚分離。ついでに `OneShotControlPage` の PROGRAM OFF オーバーレイ ("Press TAKE to send") も `text-slate-500` → `text-slate-300` + `font-medium` で読みやすく改善。光面 (light bg) UI には触らず据え置き。
