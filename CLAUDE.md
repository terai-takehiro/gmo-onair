...
## 現在のバージョン
v2.8.33 (dev) — **InsertGap UI 安定化** (Fix): ユーザー報告「UI が崩れる時がある」に対応。原因: v2.8.32 の InsertGap が `h-1.5 hover:h-9 transition-[height]` で hover 時に高さが変化していたため、layout shift + `group-focus-within` での opacity 残留により「ボタンが出っぱなし + 周囲レイアウトがガタつく」状態が発生していた。修正: ①InsertGap を**固定高 28px** (`h-7`) に変更、layout shift を完全排除。②デフォルト表示は中央に小さな `+` アイコン (`size-5 rounded-full`) のみ、hover/focus 時に `opacity` のみで 3 種ボタン (ロール/CM/VTR) を切替表示。③点線 (border-dashed) で挿入位置を常時可視化、hover 時は `border-primary/40` に。④モバイル CueCardList の InsertGap も同じ視覚言語 (固定 28px、点線 + 3 ボタン常時表示) に統一。インデックス計算と挿入機能は v2.8.32 と同等。
