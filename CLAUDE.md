...
v4.6.8 — **PR #650 が Codex のレビュー0件のままマージされたことを記録に残した**。レビューの有無は見た目ではなく `get_reviews` と要約表で判定する、と `pr-watch` の教訓に足した。あわせて: `check-changelog.mjs` のリリース枝判定を「枝のいずれかのセグメントの先頭が `release`」に緩めた（`claude/release-…` でも CI が通る）／案件作成でスタジオ予約の作成失敗を黙って握りつぶしていたのを通知するようにした／GLS-B006・B009・B010 の3件を番号はそのまま案件（GLS-A）扱いに直す特例マイグレーションを追加した。
