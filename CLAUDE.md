...
## 現在のバージョン
v4.7.2 — **BOXの案件フォルダ名の頭に実施日を付け、BOXの名前順がそのまま実施日順になるようにした**（利用者からのご依頼）。形は `【社内】2026.12.03_GLS-A001_案件名`（実施日が未定なら `未定_`）で、案件名・実施日・番号が変わるとその場で付け直し、既存フォルダは日次ジョブが少しずつ付け直す（migration 310）。あわせて技術資料の映像パッチで候補が表の枠で切れて選べなかったのを直し、#745 が Code Review 0件のままマージされたことを棚卸しに記録した。詳細・検証は全文を`docs/version-history.md`のこの版のアーカイブに置いた。
