...
## 現在のバージョン
v2.8.57 — アワードCG TOP3 演出のレイアウト調整: 1 位を中央拡大配置から「左から 1 位 / 2 位 / 3 位」均等サイズ横並びに変更。1 位は引き続き金枠 + boxShadow で視覚的に区別するが、サイズ階層は撤廃。あわせて「会場投票で大賞を決定 / LIVE VOTING」テロップを削除（演出はカード表示のみとシンプルに）。
