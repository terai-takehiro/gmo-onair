...
## 現在のバージョン
v2.8.58 — アワードCG BEST3 演出のリファイン: ①`RANKS 5→4` ステップを撤去、運用フローを「ノミネート一覧 → 一気に BEST3 一斉発表」に簡素化。②BEST3 表示中は 4 位以下/圏外の strip 写真を非表示にし、ポイント数字との重なりを解消。③カード意匠を NOMINEES 一覧と同じ「写真 4:3 portrait + 下ラベル + コーナーブラケット」構造に統一し、1 位はブラケット + ソフトグローで区別（サイズ階層なし）。`migration 080` で CHECK 制約から ranks54 を削除。
