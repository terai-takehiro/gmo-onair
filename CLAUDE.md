...
## 現在のバージョン
v4.7.1 — **やり取りの AI 整形で、社内メモ（予定表・シフト・体制案）の中身がほとんど落ちていたのを直した**（利用者からのご指摘「要約があまりに適当すぎる」）。本文の構造に節（見出し＋行）を足し、原文の見出しごとに時刻・人数・機材名を原文のまま残すようにした（プロンプト `activity-v5`・既存の記録は「整え直す」で作り直せる）。あわせて #741・#742 が Codex の利用上限で Code Review 0件のままマージされたことを棚卸しに記録した。詳細・検証は全文を`docs/version-history.md`のこの版のアーカイブに置いた。
