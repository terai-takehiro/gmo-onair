...
## 現在のバージョン
v2.8.59 — アワードCG BEST3 遷移時のネタバレ修正: NOMINEES → BEST3 切替時、共通 `PhotoStage` の写真が CSS transition でグリッド位置から TOP3 位置へ「見えながら移動」してしまい、観客に順位がバレていた。BEST3 の写真本体を `StepTop3` 側で独立描画する形に変更し、`PhotoStage` 側の写真は top3 中はその場で fade out のみ（移動しない）に。これで 1〜3 位の写真は TOP3 位置に「新規に立ち現れる」演出になりネタバレを解消。
