...
## 現在のバージョン
v2.8.113 — インタラクティブ ↔ 表彰CG 連携 API + 切り出しプラン整備

(v2.8.113: インタラクティブ演出を別 VPS に切り出す準備として、表彰CG (Awards) との **HTTP/X-API-Key 連携 API** を実装 + 段階移行プラン文書を整備。①**Interactive 外部公開 API**: `/api/v1/external/interactive/events/:eventId/questions` (問題一覧 + 多言語テキスト + 回答数) と `/api/v1/external/interactive/questions/:questionId/results` (選択肢ごとの集計 count + percent)。認証は `X-API-Key` ヘッダーのみ (cookie/セッション非依存、サーバー間通信前提)。`interactive_api_keys` テーブル (SHA-256 ハッシュ保存、key_prefix で識別) + `apiKey.service.ts` (発行/取消/検証 + last_used_at 更新) + 管理 API + Interactive アプリ内の管理 UI `/interactive/api-keys` (manager+ のみ、発行直後の 1 回のみ平文表示、取消可能)。 ②**Awards 側ブリッジ**: `interactive-bridge.service.ts` を新設し、baseUrl が設定されていれば HTTPS で外部 fetch、空欄なら同プロセスの `questionService` を直接呼ぶデュアル経路 (切り出し前後どちらでも動く)。`/awards/events/:id/interactive-link/*` に GET / PUT / DELETE / preview / ingest を集約。`awards_events.interactive_link JSONB` に `{ baseUrl, apiKeySecret, apiKeyPrefix, eventId, mapping }` を保存。マッピングモードは `choices-as-entries` (各選択肢 → awards_entries.id、書き込み先は points / own_points を選択可能)。 ③**Awards UI**: `client-awards/src/components/InteractiveLinkSection.tsx` を EventEditorPage 末尾に挿入 — base URL / API キー / Interactive eventId 入力 + 「問題一覧を取得」ボタン + 問題ごとに「選択肢 → ノミネート」のドロップダウンマッピング editor + 「結果を取り込み」+ 取り込みサマリー (どの選択肢がどのノミネートに何票流入したかのテーブル) を表示。`ControlPage` ヘッダーには「Interactive 取込」ボタンを追加 (本番中のワンタップ再集計、`canIngest` で mapping 設定済時のみ表示)。 ④**マイグレーション**: `086_interactive_external_api.sql` で `interactive_api_keys` 作成 + `awards_events.interactive_link JSONB` を追加。 ⑤**プラン文書**: `docs/interactive-extraction-plan.md` に Phase 1〜5 (新リポジトリ雛形 → 新 VPS 構築 → 連携 API = 本リリース → ONAiR 側 Interactive 削除 + 外部リンク化 → SSO 共通認証) の段階移行手順 + 新リポジトリ構成 + Nginx/docker-compose 雛形 + cutover チェックリスト + API 仕様 (request/response 例) を記載。 ⑥**既知の制約**: API キーは v1 で Awards DB に平文保存 (社内限定運用前提、Phase 5 でシークレットマネージャ化検討)、マッピングは `choices-as-entries` のみ (`correct-as-vote` 等は必要に応じて追加)、リアルタイム連動 (投票中のライブ集計) は未実装。)
