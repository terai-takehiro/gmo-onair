...
### 同居している独立アプリ
ONAiR ブロックアプリではないが、同じリポジトリ内に**コードのみ同居**しているシステム。
運用・デプロイ先・DB・認証は ONAiR とは完全に独立している。

| アプリ | ディレクトリ | デプロイ先 | DB | 認証 | 連携 |
|---|---|---|---|---|---|
| リアルタイム多言語同時通訳配信 | `interpretation/` | GCP (別プロジェクト, Cloud Run) | Cloud SQL (専用) | 独自 JWT | なし (将来 GLS 番号で連携余地) |

> 詳細: `interpretation/docs/REQUIREMENTS.md` (要件定義書 v1.0)
> 将来「別リポに切り出す」「ONAiR ブロックアプリ化する」のどちらにも振り替え可能な構造で保持。
...
## 現在のバージョン
v2.8.18 — OneShotカードレイアウト刷新（NO.1右配置・部門名大型化）、プロポーショナルフォント対応
