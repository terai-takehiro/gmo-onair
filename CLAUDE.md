...
## 現在のバージョン
v2.8.43 (dev) — **アワードCG ノミネートインポート不具合 2 件修正 (英語列が読めない / 画像フォルダで 0 件マッチ)**: ユーザー報告「英語系がうまく読み取れない」「フォルダインポートで一致しても 0 件になる」に対応。①**Excel ヘッダーマッチング強化**: `server/src/contexts/awards/services/excel-import.service.ts` の `NAME_EN_HEADERS` に `ノミネート者氏名（英語）` 系を追加（従来は `ノミネート名（英語）` しか拾えず、ユーザー実シートの `ノミネート者氏名（英語）` が `ノミネート名（英語）` を **substring に含まないため** ヒット 0 だった）。②`findCol` を「**完全一致 → 部分一致**」の 2 段階に変更し、`ノミネート名` が `ノミネート名（英語）` を誤拾いするリスクも排除。③`normalize` に全角英数→半角・全角括弧→半角の変換を追加。④**画像フォルダインポートを真の folder picker 化 + 多段マッチ**: `client-awards/src/pages/EventEditorPage.tsx` の `<input>` を `webkitdirectory/directory/mozdirectory` 属性に変更しフォルダ単位選択可能に。サーバー `/events/:id/import-images` は (a) `path.basename` で webkitRelativePath 由来のパスを除去、(b) `.DS_Store` / `Thumbs.db` / `__MACOSX/` を skip、(c) マッチング戦略を「画像ID 完全一致 → 画像ID 正規化一致 (全角半角・空白・leading zero 除去) → 氏名/英語名 一致」の順に拡張。レスポンスに `skipped` を追加してアラートに表示。これでユーザーがファイル名をエントリ名にした場合でも一括登録できる。
