...
## 現在のバージョン
v2.8.64 — アワード下部テロップCG: ページ名を「1S CG」→「**下位置CG**」にリネーム (UI 表示のみ。ルートパス `/oneshot/control` `/output/.../oneshot` および内部コード `oneshot/`、socket イベント `oneshot:set`/`oneshot:sync`、DB テーブル `awards_oneshot_cue_state` 等の内部名は維持)。`OneShotControlPage` ヘッダー / `EventEditorPage` の遷移ボタン / 出力 URL セクションのラベルを更新。
