...
## 現在のバージョン
v2.8.123 — 表彰CG (ranking) に演出パターン 2 種を追加: **「No.1発表」** (direct, 既存フロー) と **「投票No.1決定」** (vote, 新規)。 ①**DB**: migration 089 で `awards_categories` に `award_pattern ('direct'|'vote')` + `poll_title/poll_title_en/poll_question/poll_question_en`、`awards_entries` に `vote_count`、`awards_cue_state` に `vote_display ('count'|'percent') / poll_started_at / reveal_phase` を追加 + step CHECK 制約に `'poll'` / `'vote-reveal'` を許容。 ②**新規ステップ**: `poll` (アンケート投票画面 — タイトル / 質問文 / TOP3 の3択カード / 30秒カウントダウン / 中央にカメラ合成用アルファ透過枠) と `vote-reveal` (TOP3 縦棒グラフを TAKE 連打で phase 0=ランダム揺れ → 1=実値へ 0 から伸張 → 2=大賞フルスクリーン演出 まで統合)。 ③**ControlPage**: NEXT 部門の `award_pattern` で送出ステップ一覧を切替 — direct は `idle/title/nominees/ranks52/winner-bar/oneshot`、vote は `idle/title/nominees/top3/poll/vote-reveal`。`poll` TAKE で `pollStartedAt=Date.now()` をセット + 30s + 3s 後に自動で `top3` に復帰するタイマーをスケジュール。`vote-reveal` 中の TAKE 連打で `revealPhase` 0→1→2 進行。ヘッダーに「投票」ボタン (vote パターン時のみ表示) を追加、`VoteSettingsDialog` でマニュアル投票数入力 (TOP3 各エントリ) と表示モード切替 (票数/パーセント) を提供 — 操作 UI が散らからないよう投票関連 UI は別ダイアログに集約。 ④**EventEditorPage**: 部門カードに `PatternBlock` を追加 — 「No.1発表」/「投票No.1決定」トグル + vote 選択時のみ展開する `poll_title (JA/EN)` / `poll_question (JA/EN)` の 4 フィールド (collapsible)。 ⑤**Server**: `socket.ts` の `cue:set` / `cue:sync` / `cue:nextSet` / `cue:nextSync` payload + DB UPSERT を `voteDisplay` / `pollStartedAt` (epoch ms ↔ to_timestamp) / `revealPhase` で拡張。`categories.routes.ts` PUT 経路に `award_pattern` + poll_* を追加 + 新規 `PUT /awards/categories/:id/vote-counts` (TOP3 のみ vote_count を一括更新)。 ⑥**CG レンダラ**: `CGSequence` が `poll` / `vote-reveal` を新コンポーネント (`StepPoll` / `StepVoteReveal`) に分岐、両ステップ中は `PersistentHeader` を非表示にして専用全画面レイアウトを優先。`mapEntry` に `voteCount` を伝搬、vote-reveal の winner は `vote_count` 最大値で決定 (TOP3 内優先 → 全体)。

(v2.8.122: 下位置CG カウントダウン UX 改善 (4 点)。①**数字の上下重なり解消**: digit container を w 96→**116**px / h 132→**176**px に拡大、glyph を `0.82*h → 0.62*h` に縮小、上下の黒マスクを 28%→**42%** + グラデ強化 (95% 不透明 → 0%) して隣接桁の覗き込みを完全に隠す。 ②**コントロール UI をタブ化**: `OneShotControlPage` の controls 列を「送出」(Module + Ticker) / 「カウントダウン」の 2 タブに分割、カウントダウン ON 時はタブ右にゴールド点滅 dot。TAKE/CLEAR (`SendActionRow`) は両タブ共通で常時表示してアクセス維持。 ③**分秒指定で開始**: `CountdownControlPanel` に開始モード切替 (`指定日時` / `分秒指定`) を追加。`分秒指定` モードでは「今から M 分 S 秒」を入力 → 「開始」ボタンで `target = now + duration` をセット (同時に countdownOn=true)。30s/1m/3m/10m のプリセット ボタンも併設。`指定日時` の +1m〜+60m プリセットも同様に countdownOn=true で発火するよう統一。 ④**枕詞にベース**: `.oscg-countdown-prefix` に半透明黒の pill 背景 (`rgba(8,11,16,0.72→0.88)` + `border 1px solid var(--line)` + `border-radius: 999px` + inner glow) を追加し、明るい背景でも視認性確保。 ⑤**00:00 でフェードアウト**: 到達後 1 秒ホールド → 1.2 秒で opacity 0 (`.oscg-countdown--fade` クラス)。target 再設定で復活。)
