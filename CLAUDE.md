...
## 現在のバージョン
v2.8.62 — アワード1S CG: TAKE/CLEAR 挙動修正 + DB 紐付け改善。①**PROGRAM/PREVIEW の分離**: ユーザー報告「OFFにしてもONになる」(CLEAR ボタンを押しても大プレビューが消えず逆に preview がフェードインして再表示される) に対応。大プレビュー (左) を「PROGRAM (実出力ミラー)」専用に変更し、`liveFlow.mounted` が false になった時点で lower-third を unmount して画面を空にする (実出力と完全に一致)。送出予定の確認用に小サイズの「PREVIEW · NEXT TAKE」サムネ (320×180) を下段操作エリア左に追加し、現在選択中のノミネート/モジュール/ティッカー状態を常時表示。②**DB エントリ紐付け改善**: ユーザー報告「DBとの紐づけができていない」に対応。`mapEventToNominees` が `oneshot_data` JSONB 不在のエントリを skip していたため、既存ランキングCG向けに登録した普通のエントリが 1S CG で表示されない問題を修正。`oneshot_data` 任意化 — 不在時は `awards_entries` の base 列 (name / org / photo_url / rank / categories.name) から最低限の Nominee を組み立てるよう変更。`oneshot_data` がある場合はその値を優先 (override)。③**Nominee.id 形式統一**: DB-backed エントリは常に `entry-{id}` 形式に統一、`nomineeDbId()` ヘルパーで int への往復変換。output 側の `cue.entryId → nominee` lookup が確実に動作する。
