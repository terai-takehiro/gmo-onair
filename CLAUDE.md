...
## 現在のバージョン
v2.8.67 — 下位置CG: マッピング Inspector/Editor を**ビジュアル連動型**に作り直し + 全体フォントサイズ見直し。①**ビジュアルマッピング Inspector/Editor**: ユーザー要望「DB のクエリがわからないので紐付けがしずらい / ビジュアライズ的にできるように (ドラッグ&ドロップなど)」に対応。OneShotDataEditor を**2 ペイン構成**にリニューアル: 左ペイン = CG プレビュー (1920×1080 を letterbox 表示) に **番号付きホットスポット** を 各フィールドの実描画位置 (氏名/会社/写真/賞名/部門名/エントリーNo/部署/モジュールスロット 等) にオーバーレイ、source 別に色分けバッジ (🟢DB列 / 🟡oneshot_data / 🔵i18n / 🔴未設定)。右ペイン = フィールド一覧をセクション (基本情報 / 賞・部門 / プロフィール / ノミネート内容 / チーム情報 / 推薦者) でグループ化し、**ホバー連動**: 右の項目にホバーすると左 CG 上の該当領域が amber 半透明枠でハイライト、逆にホットスポットにホバーすると右の該当行がハイライト。各フィールドに DB ヒント (`awards_entries.org / .org_en` 等) を mono フォントで表示、編集対象は JA/EN を 2 カラムで inline 入力。**充足率%バッジ** (緑100%/黄50%↑/赤<50%) でデータ補完進捗が一目で分かる。タブ切替を廃止して「INSPECT + EDIT を同画面で同時に」できる UX に。②**フォントサイズ全体引き上げ**: 「全体的に文字サイズが小さくて分かりずらい」要望に対応。Modal は base 16px / section header 14px bold / field label 14px / input 14px に統一。NomineePanel の 賞 chip / 部門 chip / 人リスト name を text-xs → text-sm に、photo サムネを 40px → 48px に拡大。ModulePickerRow / TickerControlRow / SendActionRow / ShortcutHints も全て text-xs → text-sm を基準に、kbd / ボタンサイズも 1〜2px 増し。I18nDictDialog 内も text-sm 基準に統一。
