...
## 現在のバージョン
v2.9.6 — 計時LIVE 視聴者カウンターに Zoom（ミーティング + ウェビナー合算）と Microsoft Teams（Graph API Change Notification によるリアルタイム参加者数）を追加。DBマイグレーション 095 で liveops_programs に zoom_meeting_id / zoom_webinar_id / teams_meeting_url、liveops_settings に6つの認証情報フィールド、liveops_snapshots に zoom_count / teams_count（total_count を4プラットフォーム合算に再定義）、liveops_teams_subscriptions テーブルを追加。サーバー側は Zoom S2S OAuth / Azure AD Client Credentials のトークンキャッシュ（Inflight Promise パターン）、Teams Graph API Webhook の受信・サブスクリプション永続化・自動更新（40分ごと）。クライアント側は設定ページに Zoom/Teams 認証情報セクション、番組設定ページに各IDフィールド、ダッシュボードと表示画面で Zoom/Teams カード表示（番組に設定がある場合のみ）、グラフにも Zoom/Teams 系列を追加。
