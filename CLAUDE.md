...
## 現在のバージョン
v3.1.5 — **案件の「見積」が案件化しても消えないようにし、見積＝売上＝仕入を1つのデータで扱えるようにした。あわせて税区分に【不課税】を足し、仕入・販管費をお金トップから入力できるようにした（利用者依頼6件）**。①**カレンダーの「直近の予定」を畳んだ**（既定は閉じる）。今日から7日ぶんを日付ごとに全部並べるので、予約の多い週はカレンダー本体が画面の外まで押し下げられていた。件数は閉じたままでも見出しの右に出す。**開いた状態は覚えない** — カレンダーの表示状態に足すと「既定は閉じる」という依頼が「一度開いた人には以後ずっと開く」に化ける。②**案件化（GLS発番）で見積が消えていた原因は、見積を引く条件が `status = 'estimate'` の1本だったこと**。発番すると `migrateEstimates` が同じ行の status を `'confirmed'` に変えるので、**発番した瞬間に「見積が無い案件」になり画面が空になる**。空の画面で組み直されると `status='estimate'` の行がもう1本でき、分類を変えて再発番したときにそれも確定売上へ変換されて**同じ金額の売上が2件**立つ（金額が二重に乗る）。`revenues.is_estimate_origin` を足し（migration 157・埋め戻しつき）、**status に依らず同じ行を見積として開く**。発番後も明細・区分・並び・行ごとの期間・備考がそのまま残り、同じ画面で直せる（直した先は確定売上そのもの。計上日・請求日はお金＞売上の持ち物なので触らない）。あわせて**お金タブが発番後に「見積書PDFを出す」を押せなくなっていた**のと、そのボタンが**存在しない口（`/estimates/pdf`）を GET で開いていて何も出てこなかった**のを直した。③**売上の編集画面が見積の明細を1行も出していなかった**。`handleEditRevenue` が `setItems([])` で捨てていたため、案件化した見積の並びと区分が「反映されていない」ように見え、しかも**1行足して保存すると残りが全部消えた**（サーバーは明細が来たら全置換する）。読んだ明細をそのまま出し、そのまま送り返す。④**売上を1度保存すると、見積が入れた列が消えていた**。`revenue_items` の `unit` / `cost_amount` / `cost_vendor_id` / `is_ai_suggested` が **INSERT の列に入っていなかった**（DELETE→INSERT で入れ替えるので毎回消える）。列の並びを1か所に寄せて4列を足した。**この画面には仕入の欄が無いので、消えても誰も気づけない**形だった。カテゴリの候補も見積と同じ3区分（スタジオ / 技術・人員 / 制作・その他）に揃えた — 別系統の候補（制作費・機材費…）を出していたので、見積書と請求書で区分の見出しが変わっていた。⑤**見積と仕入を同じ画面で編集できるようにした**。行ごとの**仕入先**を見積の画面で選べるようにし（仕入先の一覧は `budget` 権限なので、選択肢は見積の口から返す。営業は `/vendors` を叩けない）、受注のとき作った見込み仕入を**見積を保存するたび追随**させた（金額・品目名・仕入先・計上日）。見積から消えた行・仕入を0にした行は**仮のままなら消す**。**人が確定した仕入（`is_provisional = FALSE`）には触らない** — 確定した支払いを勝手に書き換えたり消したりしない。⑥**見積の末尾に「備考」を足した**（品目に紐づかない自由記述）。`revenues.notes` に入り、見積書 PDF の明細表の下の枠に**打った改行のまま**出る（pdfkit の折り返しに任せず1行ずつ描く。`\r` が残ると豆腐が出て、空行が詰まる = v3.1.4 で品目内補足を直したのと同じ理由）。**空で送れば消せる** — `COALESCE` で寄せると消せない欄になるので、送ってこなかったときと空で送ってきたときを分けた。⑦**税区分に【不課税】を足した**（migration 156 で CHECK を貼り替え）。**非課税と不課税は別物**（非課税＝消費税の対象だが法令で課税しない／不課税＝そもそも対象外）で、税額はどちらも0円だが帳簿と申告では区別する。選択肢が「非課税」しかないため不課税の支払いを非課税として入れるほかなく、**あとから分けられなかった**。決算取込と楽楽精算の取込も分けて入れるようにした（「不課税」は「課税」を含むので判定の順番が要る）。⑧**税区分の扱いを1か所に寄せた**。税枝番（`billing_key` の末尾）を出す式が**4か所にあって、それぞれ違っていた** — `billing-key.service` だけ非課税を `1` にしており、同じ非課税の売上が作った入口によって `...-1` と `...-0` に分かれていた。税率の表も3か所にあった。`tax-category.service` 1本にし、選択肢は `TaxCategoryLabels` から出す（画面ごとに `<SelectItem>` を並べると足した選択肢が漏れる。実際に販管費だけ非課税が無かった）。⑨**仕入・販管費をお金トップから入力できるようにした**。上辺に「仕入を登録」「販管費を登録」を置き、押すとそれぞれの一覧で**登録の欄が開いた状態**になる（`?new=1`）。内訳が0件のときはその場にも同じボタンを出す。**フォームは一覧ページのまま** — 同じ入力欄を2か所に置くと片方だけ項目が増える。⑩**検証**: 全9ワークスペースの型チェック・ビルド通過、`eslint` 0 errors / 77 warnings（着手前と同数）、`check-ui-tokens.mjs` 違反0。**実 HTTP + 実 Postgres 57項目**（不課税が CHECK を通り消費税0円で保存できる・請求キーの枝番が `3`・備考の `\r\n` が `\n` に寄って空行が残る・空白だけは null・送らなければ残る／**受注で見込み仕入が2件立ち計上日が行の期間の開始日になる**・見積を直すと金額が追随し0にした行は消える・二度打ちしない・確定済みは書き換えも削除もされない／**GLS発番のあとも明細3行と区分と並びと備考が残り、売上の行は増えない**・発番後の保存で同じ id が更新され status は confirmed のまま・請求キーは変わらない・発番後も見積書PDFが出る・見るだけの権限では 403／**売上の口で保存しても単位・行ごとの仕入・仕入先・区分と並びが残る**）。**実 PDF 11項目**（描画を1件ずつ追い、備考の3行が別々に刷られ、空行が1行ぶんの高さを取り、`\r` が残らず、明細表より下に出る）。**実ブラウザ 96項目（PC 1440px / スマホ 375px）**（直近の予定が既定で閉じていて押すと開き件数は閉じたままでも読める・見出しのタップ領域44px以上／見積の税区分に不課税が非課税と別に並ぶ・備考が明細の下に出て打ち替えて保存し**読み直しても改行が残る**・仕入を入れた行だけ仕入先が出て営業の権限でも候補が読める・案件化済みの札／**売上の編集に見積の3行が並びと区分のまま出る**・「見積の画面で編集」から行ける／お金トップの2つのボタンから登録の欄が開き仕入・販管費の両方に不課税がある／横はみ出し0px・JSエラー0件）。
...
## ブランチ運用 (v4 で全面刷新 — 詳細: [`docs/branching.md`](docs/branching.md))

**GitHub Flow + リリースタグ**。社内独自ルールは無くしてある (他のプロジェクトから来た人がそのまま働ける形にするため)。

- **長命ブランチは `main` だけ**。ここが唯一の正で、常に「次に出せる状態」。**直接 push は ruleset で止まる**
- **作業は必ず `main` から短いブランチを切って PR で戻す**: `feature/<Issue番号>-<名前>` / `fix/` / `chore/` / `docs/`
  (Claude Code が自動で作る `claude/*` もそのまま PR にしてよい。**名前より寿命が大事** — 数日で PR にする)
- **マージは Squash 固定**、マージ後にブランチは自動削除
- **`main` にマージ → 検証環境 (dev.gmo-onair.jp) に自動デプロイ**。本番には出ない
- **本番は GitHub で Release を公開したときだけ** (タグ `vX.Y.Z` / `production` 環境の承認つき)
- **`release/v3`** — v3 系の保守専用 (v3.1.5 で凍結)。本番が止まる緊急修正だけ。**`main` にマージしない**
- **`archive/*` タグ** — 2026-07-31 に整理した旧 `claude/*` ブランチの退避先。ブランチとして復元できる
- **検査は PR のマージ前** (`checks` = 型/Lint/整合性、`build` = Docker ビルド)。以前は push 後にしか走らず、PR は無検査でマージできた

> ⚠️ **Claude への注意**: `main` への直接 push は禁止 (必ず PR)。
> **本番リリース (Release の公開) は、ユーザーが明示的に指示したときだけ**。いかなる理由があっても勝手に出さない。

- **バージョン管理**: インクリメンタル（v4.0.1, v4.0.2...）、大きくジャンプしない。
  **メジャー/マイナー番号 (先頭2つ) はユーザーが明示的に指示したときだけ上げる**
  （例: v3.0.0 は「現場の道具の共通基盤統合＋UI刷新」という節目をユーザーが指定した版）。
- **バージョンを上げるのは「リリースを出すとき」だけ (v4 で変更)**。
  以前は「push するたびパッチを上げる」運用だったが、機能ごとにブランチを分けると
  **複数のブランチが同時に `package.json` を触って必ず衝突する**。上げるのは以下の3か所だけで、
  `npm run check:version` が一致を機械的に検査する (CI の必須チェック):
  1. ルート `package.json` の `"version"` ← **バージョンの唯一の情報源**
  2. `CLAUDE.md` の「現在のバージョン」
  3. `README.md` の「現在のバージョン」＋「バージョン履歴（抜粋）」に新バージョン行を追記
  - ~~各ワークスペース `package.json`~~ → **更新しない (v2.9.238 で方針変更)**。
  ...
  - リリースの Release ノート (タグ `vX.Y.Z`) に変更点を書く
  - **公開完了後、チャットでバージョン番号とデプロイ先 (検証/本番) をユーザーに必ず報告すること**
...
- **Claudeは必ず PR → `main` (検証環境) までで止め、ユーザーが「本番に入れて」と明示するまで Release を公開しない**

### デプロイワークフロー
1. **開発 → 検証**: `main` から切ったブランチを PR → CI が通ったら Squash マージ → GitHub Actions が検証環境 (`dev.gmo-onair.jp`) に自動デプロイ
2. **検証で動作確認**: 検証環境で全機能テスト → OKならユーザーに通知して承認を待つ
3. **本番リリース**: ユーザーがチャットで「本番に入れて」と明示的に指示してから、バージョンを上げる PR をマージ → **GitHub で Release を公開 (タグ `vX.Y.Z`)** → `production` 環境の承認 → 本番デプロイ
4. **緊急ロールバック**: Releases から1つ前のタグの Deploy を再実行する (イメージも `ghcr.io/<repo>:vX.Y.Z` と `:sha-<commit>` で残っている)

> ⚠️ Claudeへの注意: ユーザーの明示的な本番指示なしに **Release を公開する / `production` へデプロイする**ことは**いかなる理由があっても禁止**。
> `main` への直接 push も禁止 (ruleset で止まるが、そもそもやらない)。
...
## デプロイフロー（必須手順）
1. **検証環境 (dev.gmo-onair.jp)** — `main` へ PR をマージ → 自動デプロイ
   - **直接 push はしない** (ruleset で止まる)。必ず PR を出して CI (`checks` / `build`) を通す
   - バージョン番号は**ここでは上げない** (リリース時にまとめて上げる)
   - デプロイ完了後、チャットでユーザーに通知すること
2. **本番環境 (gmo-onair.jp)** — ユーザーからチャットで承認を受けてから **GitHub で Release を公開**
   - 勝手に本番デプロイしない。必ずユーザーの明示的な指示を待つ
   - Release の**前に**バージョンを上げる PR をマージする (3か所 / `npm run check:version` が検査)
   - タグは `vX.Y.Z`、ターゲットは `main`。公開すると `production` 環境の承認待ちになる
   - デプロイ完了後、チャットでバージョン番号とデプロイ先を通知すること
