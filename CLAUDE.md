...
## 現在のバージョン
v4.7.0 — **新しいブロックアプリ「Wiki」を作り、制作技術支援に「技術資料」を足した。あわせて OpenAI を SDK 7・GPT-6 世代（Node 22）に上げた**。Wiki は設計の段A〜段F（書く・データベース・検索と `.md` の出入口・出典つきの AI・見直しとコメント・スペース管理）がそろい、Codex の再レビューで付いた指摘（同時操作で閲覧範囲をすり抜ける競合など）もすべて直した。ほかに営業活動記録の案件別表示と担当者での絞り込み、決算取込（CLI版を含む）の失敗と番号履歴の抜け、議事録の AI まとめが短すぎた不具合、テロップCG・旧モックの画面の言葉の統一を直した。詳細・検証は全文を `docs/version-history.md` のこの版のアーカイブに置いた。

v4.6.19 — **営業活動記録の案件プルダウンで、一部案件が出ない・終了案件が混ざる・並び順が案件日と無関係だった不具合を直した**（利用者指摘）。終了（完了・失注）を除く全件を案件日が近い順に返す専用口 `GET /projects/activity-log-projects` を新設し、同じ穴を持っていたお客様詳細「やり取りを記録」ダイアログも合わせて直した（Codexレビュー指摘4件——編集時の値消失・無効化漏れ2か所・空文字の案件日ソート——も同PRで対応済み）。あわせてPR #700〜#708のレビュー状況を棚卸しに記録した（未対応2件は表に❌として残る）。詳細・検証は全文を`docs/version-history.md`のこの版のアーカイブに置いた。

v4.6.18 — **会場図面「並べ方」の配置ずれ修正で、2段のP1回帰が連鎖したのを直した**（Codexレビュー指摘）。v4.6.17でプレビュー下端切れを直した際に足した補正を「追加」の配置・「並べ直す」にも使ってしまい、①`frontClearanceMm`・`sideAisleMm`が盤上の位置に効かなくなる、②数値を変えずに「並べ直す」を押すだけでグループが動く、という2つのP1指摘（それぞれ次のPRで対応）を招いた。盤に置く側の計算を元の式に戻し、補正はプレビューの中央寄せ・「並べ直す」時の原点逆算だけに使うよう整理した。詳細・検証は全文を`docs/version-history.md`のこの版のアーカイブに置いた。
