...
## 現在のバージョン
v2.8.114 — 経理データ取込み Phase 1 (Layer A: 生データ取込み API + DB スキーマ)

(v2.8.114: 経理データ取込み Phase 1 / Layer A 生データ層): 月次会計確定後の経理 CSV (損益計算書 + 残高試算表 + 総勘定元帳、CP932) を ONAiR に取込む機能の第 1 弾。Phase 0 既存スキーマ調査 (`docs/accounting-import/phase0-schema-survey.md`) のレビュー結論「A 案 = 既存テーブル + source='accounting'」をベースに実装。①**migration 087**: a) `accounting_import_batches` (バッチ管理、`staged`/`confirmed`/`superseded` の 3 状態、SHA-256 ハッシュで冪等性、CP932 原本を `source_file_bytea` に BYTEA 保管、Box ミラー用 `source_box_file_id` も確保)、b) `accounting_pl_lines_raw` (P/L を level 0-3 階層 + 勘定/補助/細目コードに分解保存、`raw_row` JSONB に元 CSV 行)、c) `accounting_ledger_entries_raw` (元帳を `voucher_no + line_seq` で一意化、`extracted_gls_codes` / `extracted_xp_codes` TEXT[] に摘要から抽出した GLS/XP 配列を GIN index で高速検索)、d) 既存 `revenues` / `purchases` に **`source TEXT CHECK('staff','accounting') DEFAULT 'staff'`** と `accounting_ledger_entry_id BIGINT FK` を追加 (`sga_expenses.source` は migration 001b 時点から既存)。すべて `IF NOT EXISTS` で冪等。 ②**Parser** (`server/src/contexts/accounting/parsers/`): a) `pl-parser.ts` は `iconv-lite` で CP932→UTF-8 → `csv-parse/sync` で RFC4180 解釈、ヘッダ動的部分 `開始月:YYYY-MM-DD` / `終了月:YYYY-MM-DD` から period_ym 抽出、補助科目の二段表記 (`8000 ｸﾞﾙｰﾌﾟ業務委託 :100 GP`) を `^(\d+)\s+(.+?)(?:\s*:\s*(\d+)\s+(.+))?$` で分解、階層判定は (col0 のみ + 金額空 → level 0, "合計"系ラベル → level 3, col1 のみ → level 1, col2 ある → level 2) で実装。 b) `ledger-parser.ts` は同じ CP932 デコード + `csv-parse` (relax_quotes / relax_column_count) でメモのセル内改行に対応、摘要から `GLS[\s\-]*?\d{3,}` (ハイフン許容、正規化して `GLSnnn` に統一) と `XP\d{6,}` を抽出、`voucher_no` 別に `line_seq` を取込時採番、取引日 `YYYY/MM/DD` → `YYYY-MM-DD` 正規化。 ③**Service** (`services/import.service.ts`): `importCsv()` 関数で SHA-256 ハッシュ計算 → 同一 (source_type, period_ym, hash) のバッチを検索 (`status<>'superseded'`) → 既存ならそのまま返却 (200 duplicate)、新規ならトランザクション内で `accounting_import_batches` (status='staged') + 各生データ行を INSERT。source_type 自動判定はファイル名 (`損益計算書` / `総勘定元帳`) + 内容 (`取引No` / `構成比` のヘッダ) の二段。元帳の period_ym は entries の `transaction_date` の最頻 YYYY-MM (fallback はファイル名の YYYYMMDD)。 ④**API** (`routes/imports.routes.ts`, `/api/v1/internal/accounting/` 配下): a) `POST /imports` (multer 5MB 上限 memoryStorage、form-data の `file` 必須 + `source_type?` / `period_ym?` / `notes?` 任意、editor 権限)、b) `GET /imports` (期間/種類/状態で絞り込み)、c) `GET /imports/:id` (バッチ詳細 + サマリー: P/L は level 別件数、元帳は 取引数/voucher 数/GLS 配列/XP 配列/日付範囲)、d) `GET /imports/:id/pl-lines` (P/L 行を row_index 順)、e) `GET /imports/:id/ledger-entries` (ページング + `?gls=` / `?xp=` / `?account_code=` フィルタ、ANY() で GIN index 活用)、f) `POST /imports/:id/confirm` (staged→confirmed)、g) `POST /imports/:id/supersede` (manager 権限のみ)。 ⑤**権限**: 既存 `budget` モジュールを拡張 (新規 `accounting` モジュールは作らない、レビュー結論)。`requireAuth + requirePermission('budget')` で全 endpoint をガード、書込みは `editor` / 廃棄は `manager` を追加要求。 ⑥**依存追加**: `server/package.json` に `csv-parse ^5.6.0` + `iconv-lite ^0.6.3` を追加 (既存に CP932 デコーダなし)。 ⑦**Phase 1 で意図的に未実装** (Phase 2 以降): ETL ジョブ (生データ→revenues/purchases/sga_expenses への source='accounting' INSERT)、勘定/GLS/XP/ベンダーの解決、未マッピング対応 UI、Box ミラーアップロード、P/L 表示画面、予算 vs 実績比較画面、編集監査ログ。 ⑧**実 CSV 検体**: Box folder `381450796408` から取得 (15 GLS / 115+ XP / 6 税区分 / 90 勘定科目を実データで確認済、要件書 §10 の GLS127/140/148/150/151 すべて実在)。検証用ファイルは `.gitignore` の `docs/accounting-import/samples/` で git 管理外。
