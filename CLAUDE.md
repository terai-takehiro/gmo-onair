...
(v2.9.4 — アンケート/クイズCG: 3 モードパターン (アンケート質問のみ / 結果発表あり / クイズ) + **スタック方式の送出** (event 単位 1 出力 URL + NEXT プルダウン切替 + TAKE で昇格) + カウントダウン スライドアニメ復活 + is_correct チェックボックス保存バグ修正。①**3 モード**: migration 095 で `quizzes.mode` CHECK 制約に `'survey-only'` を追加 → `survey-only`/`survey`/`quiz`。`survey-only` は idle → poll → idle (結果発表なし)、`survey` は idle → poll → reveal → winner、`quiz` は idle → poll → answer-check → correct-reveal → reveal → winner。`QuizEditPage` のモード select も 3 択化。 ②**スタック送出**: migration 095 で `quiz_stack_state` を新設、socket query `?stackEventId=` で `quizStack:{eventId}` ルーム join、`quizStack:set/sync` で broadcast。新規 `GET /quiz/events/:eventId/quiz-stack/public`、新ページ `QuizStackControlPage` (`/event/:id/quiz-stack/control`) + `QuizStackOutputPage` (`/awards/output/quiz-stack/:eventId`)。QuizListPage の per-quiz 「送出/出力」を撤廃し、ページ上部に event 単位の「スタック送出」+「出力 URL」を配置。 ③**カウントダウン スライドアニメ復活**: `SlideDigits` を `QuizCG` に追加 (qzDigitIn/qzDigitOut keyframes)。 ④**is_correct 保存バグ修正**: `PUT /quizzes/:id/choices/:position` の SQL を `COALESCE(?::boolean, is_correct)` に変更。 ⑤**App.tsx 整理**: 旧 `QuizControlPage`/`QuizOutputPage` を削除しスタック版に置換。
