...
## 現在のバージョン
v4.6.11 — **PR #660のレビュー指摘を棚卸しに移した**（`docs/branching.md`「マージしたら、その PR のレビューを棚卸しに移す」）。#660（案件編集画面のスタジオ日程が保存されない不具合の修正）にCodexから届いた指摘5件（P1×4・P2×1）のうち、4件（P1×3・P2×1）はマージ前にPR内で修正・返信・スレッド解決まで完了した。残り1件（複数人が同一の未予約案件をほぼ同時に保存した場合のサーバー側の原子性）は、ユーザーに確認のうえ対応しないと決定し、理由と何が変われば直すかを`docs/reviews/codex-findings-v4.md`の表に記録した。`npm run reviews:debt`はこの環境のトークンでは401（過去の棚卸しと同じ既知の制約）のため、`get_review_comments`で直接内容を確認して記録した。検証: `node scripts/check-md-links.mjs`。 **ドキュメント全体を現時点の事実に合わせて体系的に書き直した**。版上げのたびに `README.md` が版の全文と「旧 v…」を4件抱えて入口として読めなくなり、`CLAUDE.md`（毎ターン文脈に載る）の6割が版の履歴で、`docs/ops/vps-setup.md` は単一アプリ時代の手順のまま、検証環境の認証を「ユーザーカード選択式」と書いた文書が残るなど、文書ごとに古さの度合いが違っていた。①`README.md` は入口の文書として書き直し、版は `**現在のバージョン**: vX.Y.Z — **見出し**` の1行だけにした（`npm run release:notes` もこの形だけを書くよう `scripts/collect-changelog.mjs` を直し、全文は `CLAUDE.md` の最新3件と `docs/version-history.md` に置く）。②`CLAUDE.md` は「全体に効く決めごとだけ」に整理し、直近3件の版は見出し＋2〜3文の要約にして全文を `docs/version-history.md` へ移した（画面の「バージョン履歴」は同じ版なら長いほうを出すので表示は変わらない）。③`CONTRIBUTING.md`・`docs/README.md`・`docs/branching.md`・`docs/deploy-pipeline.md`・`docs/ops/*`・`docs/guide/*`・`docs/changelog.d/README.md`・`docs/ia.md`・PR テンプレートを現行の仕組みに合わせて書き直し、経緯の語りは `docs/archive/2026/` へ移した。④各アプリの `CLAUDE.md`（client・client-daily・client-equipment・client-techops・client-live・client-awards・shared）を「いま効くルール」だけに絞り、外した経緯は `docs/reviews/*-build-log.md` に記録として残した。⑤計画・設計・レビューの各文書には状態欄（状態・最終確認・位置づけ）を付け、役目を終えた文書（引き継ぎ書・リリース前の作業場・当時のスナップショット）は `docs/archive/2026/` へ移し、`docs/reviews/README.md`・`docs/guide/README.md` の索引を足した。検証: `node scripts/check-md-links.mjs`・`npm run check:version`・`node scripts/generate-version-history.mjs`（513件・v4.6.10 の本文がアーカイブの全文で出ること）・`node scripts/collect-changelog.mjs 4.6.11 --dry`・`npm run lint`・`npm test`。 **総勘定元帳インポート（決算取込）の取込元を BOX から PC アップロードに変更した**。①これまで「財務管理＞決算取込」の画面（`GlTab.tsx`）は Box のフォルダ ID・ファイル ID を入力欄で受け取り、サーバー側で Box から総勘定元帳（freee の CSV または MoneyForward の xlsx）をダウンロードして取り込んでいた。今回この Box 連携をやめ、画面から総勘定元帳ファイルをドラッグ＆ドロップ、またはクリックして選択し、PC から直接アップロードする方式（`multipart/form-data`）に変更した。CLI 版（`import-kessan-dev.mjs`）も`--file=<path>` でローカルファイルを指定する形に合わせた。②取込先の判定・`scope`（販管費／売上／仕入／全部）や「マスタ作成」「固定原価除外」「重複スキップ」といったオプション、抽出・投入のロジック自体（`kessan-import.service.ts`）は変更していない。その上で、**対象月ぶんのデータは取り込むたびに完全に上書きし、元帳に含まれない既存データ（手入力など）はマージされたまま残る**という従来からの挙動が変わっていないことをコードで確認した。**あわせて、取引先・顧客名の名寄せが表記ゆれに弱く「マージ（残す）に失敗する」ケースがあったのを直した**。①利用者から「残す挙動のはずが、少し表記が違うだけで失敗しているケースがあった」というご指摘。調査したところ、取引先・顧客の名寄せ（`findCustomer`/`ensureVendor`）は `companies.name = $1` の完全一致で、元帳側の表記が登録済みマスタと1文字でも違う（全角/半角スペース・`㈱`と`(株)`・連続する空白など）と別会社として新規作成され、同じ取引先が2つに分裂していた。販管費の重複検出（二重計上スクリーニング）も取引先名の完全一致がキーのため、同じ理由で二重計上を見逃す方向に倒れていた（売上・仕入はGLS番号というコード値で見ているため対象外）。②NFKC正規化＋連続空白の圧縮で比較用キーを作る仕組みを追加し、この3か所で使うようにした。表示・保存する名前自体は元の表記のまま変更していない。GLS番号（案件コード）の突き合わせは、表記ゆれではなく別案件との誤結合を避けるため完全一致のまま維持した。検証: 正規化パターンを固定する review test を追加。 **PR #663のレビュー状況を棚卸しに記録した**（`docs/branching.md`「マージしたら、その PR のレビューを棚卸しに移す」）。#663（総勘定元帳インポートをBOXからPCアップロードに変更）はCode Reviewが1巡も欠かさず走り、届いた指摘（P1/P2）は全20スレッドとも修正・返信・解決済みでマージした（表に移す未対応の指摘は無い）。一方でSecurity ReviewはPR作成直後から最後まで一度も完走せず（`⚠️ Failed`のまま、15回のpush・約1時間40分・手動再依頼でも変化なし）、取引先マスタを自動作成する経路や新規のmultipartアップロード口という、まさにセキュリティ観点で見てほしい変更が未実施のまま入った。「指摘なし」ではなく「一度も走っていない」ことを`docs/reviews/codex-findings-v4.md`の「レビューが0件のままマージされた PR」に記録した。検証: `node scripts/check-md-links.mjs`。

v4.6.10 — **案件編集画面の「スタジオの日程」で部屋を選んでチェックを入れても保存されない不具合を直した**。予約が1件も無い案件では編集画面にも「使う部屋・空間」が出るのに、保存側（`useProjectForm.ts`）が編集時は常に予約作成を飛ばしていた。表示と同じ判定（`hasEventBooking`）を保存側にも使い、条件を満たすときだけ新規作成時と同じ `createInitialBookings` で予約を作るようにした。

v4.6.9 — **PR #654・#655のレビュー指摘を棚卸しに移した**（表に移す未対応の指摘は無し）。あわせて2件を直した: 案件台帳の整合性チェック「受注しているのに実施日が無い」が、実施日の概念を持たない GLS-B まで拾っていた（`gls_category = 'A'` に絞った）／グループ会社の案件でリード経路が「グループ案件」に固定されず、画面以外の経路（MCP・取込）で作った案件が整合性チェックに引っかかり続けていた（サーバー側で確定させ、既存データを埋め戻すマイグレーションを追加）。
